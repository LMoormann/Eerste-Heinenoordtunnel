...
/** Automaton "rb_overzicht_systemen_en_detecties_vluchtdeuren". */
public final class Aut_rb_overzicht_systemen_en_detecties_vluchtdeuren extends RuntimeSimpleAutomaton<State> {
    public Aut_rb_overzicht_systemen_en_detecties_vluchtdeuren() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_rb_overzicht_systemen_en_detecties_vluchtdeuren", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "rb_overzicht_systemen_en_detecties_vluchtdeuren";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_rb_overzicht_systemen_en_detecties_vluchtdeuren.lp_rb_overzicht_systemen_en_detecties_vluchtdeuren);
    }
    ...
    public static boolean fillSyncData_evt_rb_overzicht_systemen_en_detecties_vluchtdeuren_open_drop_down_menu(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(145).get(0);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(70);
        fillSyncData(aut, 145, rslt, state.s_rb_overzicht_systemen_en_detecties_vluchtdeuren.lp_rb_overzicht_systemen_en_detecties_vluchtdeuren);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_rb_overzicht_systemen_en_detecties_vluchtdeuren_sluit_drop_down_menu(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(146).get(0);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(70);
        fillSyncData(aut, 146, rslt, state.s_rb_overzicht_systemen_en_detecties_vluchtdeuren.lp_rb_overzicht_systemen_en_detecties_vluchtdeuren);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_sluit_alle_tabs(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(157).get(66);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(70);
        fillSyncData(aut, 157, rslt, state.s_rb_overzicht_systemen_en_detecties_vluchtdeuren.lp_rb_overzicht_systemen_en_detecties_vluchtdeuren);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_rb_overzicht_systemen_en_detecties_vluchtdeuren = target.s_rb_overzicht_systemen_en_detecties_vluchtdeuren.copy();
        target.s_rb_overzicht_systemen_en_detecties_vluchtdeuren.lp_rb_overzicht_systemen_en_detecties_vluchtdeuren = value;
    }
}
