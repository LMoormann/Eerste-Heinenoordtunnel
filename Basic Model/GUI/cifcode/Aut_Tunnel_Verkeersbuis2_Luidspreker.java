...
/** Automaton "Tunnel_Verkeersbuis2_Luidspreker". */
public final class Aut_Tunnel_Verkeersbuis2_Luidspreker extends RuntimeSimpleAutomaton<State> {
    public Aut_Tunnel_Verkeersbuis2_Luidspreker() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_Tunnel_Verkeersbuis2_Luidspreker", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "Tunnel_Verkeersbuis2_Luidspreker";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_Tunnel_Verkeersbuis2_Luidspreker.lp_Tunnel_Verkeersbuis2_Luidspreker);
    }
    ...
    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis2_Luidspreker_c_boodschap(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(933).get(4);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(540);
        fillSyncData(aut, 933, rslt, state.s_Tunnel_Verkeersbuis2_Luidspreker.lp_Tunnel_Verkeersbuis2_Luidspreker);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis2_Luidspreker_c_live(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(934).get(4);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(540);
        fillSyncData(aut, 934, rslt, state.s_Tunnel_Verkeersbuis2_Luidspreker.lp_Tunnel_Verkeersbuis2_Luidspreker);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis2_Luidspreker_c_onderdrukt(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(935).get(4);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(540);
        fillSyncData(aut, 935, rslt, state.s_Tunnel_Verkeersbuis2_Luidspreker.lp_Tunnel_Verkeersbuis2_Luidspreker);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis2_Luidspreker_c_uit(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(936).get(4);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(540);
        fillSyncData(aut, 936, rslt, state.s_Tunnel_Verkeersbuis2_Luidspreker.lp_Tunnel_Verkeersbuis2_Luidspreker);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_Tunnel_Verkeersbuis2_Luidspreker = target.s_Tunnel_Verkeersbuis2_Luidspreker.copy();
        target.s_Tunnel_Verkeersbuis2_Luidspreker.lp_Tunnel_Verkeersbuis2_Luidspreker = value;
    }
}
