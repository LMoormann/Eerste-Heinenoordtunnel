...
/** Automaton "Tunnel_MiddenPompkelder_Regime". */
public final class Aut_Tunnel_MiddenPompkelder_Regime extends RuntimeSimpleAutomaton<State> {
    public Aut_Tunnel_MiddenPompkelder_Regime() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_Tunnel_MiddenPompkelder_Regime", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "Tunnel_MiddenPompkelder_Regime";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_Tunnel_MiddenPompkelder_Regime.lp_Tunnel_MiddenPompkelder_Regime);
    }
    ...
    public static boolean fillSyncData_evt_Tunnel_MiddenPompkelder_Regime_c_bergen(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(271).get(3);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(185);
        fillSyncData(aut, 271, rslt, state.s_Tunnel_MiddenPompkelder_Regime.lp_Tunnel_MiddenPompkelder_Regime);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_MiddenPompkelder_Regime_c_leeghouden(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(272).get(3);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(185);
        fillSyncData(aut, 272, rslt, state.s_Tunnel_MiddenPompkelder_Regime.lp_Tunnel_MiddenPompkelder_Regime);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_MiddenPompkelder_Regime_c_uit(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(273).get(3);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(185);
        fillSyncData(aut, 273, rslt, state.s_Tunnel_MiddenPompkelder_Regime.lp_Tunnel_MiddenPompkelder_Regime);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_Tunnel_MiddenPompkelder_Regime = target.s_Tunnel_MiddenPompkelder_Regime.copy();
        target.s_Tunnel_MiddenPompkelder_Regime.lp_Tunnel_MiddenPompkelder_Regime = value;
    }
}
