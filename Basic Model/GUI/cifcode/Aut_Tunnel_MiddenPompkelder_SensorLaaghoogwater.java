...
/** Automaton "Tunnel_MiddenPompkelder_SensorLaaghoogwater". */
public final class Aut_Tunnel_MiddenPompkelder_SensorLaaghoogwater extends RuntimeSimpleAutomaton<State> {
    public Aut_Tunnel_MiddenPompkelder_SensorLaaghoogwater() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_Tunnel_MiddenPompkelder_SensorLaaghoogwater", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "Tunnel_MiddenPompkelder_SensorLaaghoogwater";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_Tunnel_MiddenPompkelder_SensorLaaghoogwater.lp_Tunnel_MiddenPompkelder_SensorLaaghoogwater);
    }
    ...
    public static boolean fillSyncData_evt_Tunnel_MiddenPompkelder_SensorLaaghoogwater_u_aan(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(276).get(2);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(187);
        fillSyncData(aut, 276, rslt, state.s_Tunnel_MiddenPompkelder_SensorLaaghoogwater.lp_Tunnel_MiddenPompkelder_SensorLaaghoogwater);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_MiddenPompkelder_SensorLaaghoogwater_u_uit(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(277).get(2);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(187);
        fillSyncData(aut, 277, rslt, state.s_Tunnel_MiddenPompkelder_SensorLaaghoogwater.lp_Tunnel_MiddenPompkelder_SensorLaaghoogwater);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_Tunnel_MiddenPompkelder_SensorLaaghoogwater = target.s_Tunnel_MiddenPompkelder_SensorLaaghoogwater.copy();
        target.s_Tunnel_MiddenPompkelder_SensorLaaghoogwater.lp_Tunnel_MiddenPompkelder_SensorLaaghoogwater = value;
    }
}
