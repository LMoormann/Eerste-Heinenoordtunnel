...
/** Automaton "Tunnel_MiddenTunnelKanaal_BedieningOverdruk_Modus". */
public final class Aut_Tunnel_MiddenTunnelKanaal_BedieningOverdruk_Modus extends RuntimeSimpleAutomaton<State> {
    public Aut_Tunnel_MiddenTunnelKanaal_BedieningOverdruk_Modus() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_Tunnel_MiddenTunnelKanaal_BedieningOverdruk_Modus", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "Tunnel_MiddenTunnelKanaal_BedieningOverdruk_Modus";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_Tunnel_MiddenTunnelKanaal_BedieningOverdruk_Modus.lp_Tunnel_MiddenTunnelKanaal_BedieningOverdruk_Modus);
    }
    ...
    public static boolean fillSyncData_evt_Tunnel_MiddenTunnelKanaal_BedieningOverdruk_Modus_c_auto(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(298).get(3);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(200);
        fillSyncData(aut, 298, rslt, state.s_Tunnel_MiddenTunnelKanaal_BedieningOverdruk_Modus.lp_Tunnel_MiddenTunnelKanaal_BedieningOverdruk_Modus);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_MiddenTunnelKanaal_BedieningOverdruk_Modus_c_hand(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(299).get(3);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(200);
        fillSyncData(aut, 299, rslt, state.s_Tunnel_MiddenTunnelKanaal_BedieningOverdruk_Modus.lp_Tunnel_MiddenTunnelKanaal_BedieningOverdruk_Modus);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_Tunnel_MiddenTunnelKanaal_BedieningOverdruk_Modus = target.s_Tunnel_MiddenTunnelKanaal_BedieningOverdruk_Modus.copy();
        target.s_Tunnel_MiddenTunnelKanaal_BedieningOverdruk_Modus.lp_Tunnel_MiddenTunnelKanaal_BedieningOverdruk_Modus = value;
    }
}
