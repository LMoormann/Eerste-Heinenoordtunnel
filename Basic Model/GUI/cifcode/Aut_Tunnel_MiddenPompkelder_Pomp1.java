...
/** Automaton "Tunnel_MiddenPompkelder_Pomp1". */
public final class Aut_Tunnel_MiddenPompkelder_Pomp1 extends RuntimeSimpleAutomaton<State> {
    public Aut_Tunnel_MiddenPompkelder_Pomp1() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_Tunnel_MiddenPompkelder_Pomp1", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "Tunnel_MiddenPompkelder_Pomp1";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_Tunnel_MiddenPompkelder_Pomp1.lp_Tunnel_MiddenPompkelder_Pomp1);
    }
    ...
    public static boolean fillSyncData_evt_Tunnel_MiddenPompkelder_Pomp1_c_aan(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(265).get(1);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(182);
        fillSyncData(aut, 265, rslt, state.s_Tunnel_MiddenPompkelder_Pomp1.lp_Tunnel_MiddenPompkelder_Pomp1);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_MiddenPompkelder_Pomp1_c_uit(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(266).get(1);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(182);
        fillSyncData(aut, 266, rslt, state.s_Tunnel_MiddenPompkelder_Pomp1.lp_Tunnel_MiddenPompkelder_Pomp1);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_Tunnel_MiddenPompkelder_Pomp1 = target.s_Tunnel_MiddenPompkelder_Pomp1.copy();
        target.s_Tunnel_MiddenPompkelder_Pomp1.lp_Tunnel_MiddenPompkelder_Pomp1 = value;
    }
}
