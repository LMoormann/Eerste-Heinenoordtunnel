...
/** Event "Tunnel_Verkeersbuis2_KnopHerstel_Waarnemer_c_onderbroken". */
public final class Event_Tunnel_Verkeersbuis2_KnopHerstel_Waarnemer_c_onderbroken extends RuntimeEvent<State> {
    public Event_Tunnel_Verkeersbuis2_KnopHerstel_Waarnemer_c_onderbroken() {
        super("Tunnel_Verkeersbuis2_KnopHerstel_Waarnemer_c_onderbroken", 824, true);
    }
    ...
    public boolean fillData(State state) {
        ...
        proceed = Aut_sup48.fillSyncData_evt_Tunnel_Verkeersbuis2_KnopHerstel_Waarnemer_c_onderbroken(state);
        if (!proceed) return false;

        // Check automaton "sup6".
        proceed = Aut_sup6.fillSyncData_evt_Tunnel_Verkeersbuis2_KnopHerstel_Waarnemer_c_onderbroken(state);
        if (!proceed) return false;

        // Check automaton "sup72".
        proceed = Aut_sup72.fillSyncData_evt_Tunnel_Verkeersbuis2_KnopHerstel_Waarnemer_c_onderbroken(state);
        if (!proceed) return false;

        // Check automaton "sup76".
        proceed = Aut_sup76.fillSyncData_evt_Tunnel_Verkeersbuis2_KnopHerstel_Waarnemer_c_onderbroken(state);
        if (!proceed) return false;

        // Check automaton "sup80".
        proceed = Aut_sup80.fillSyncData_evt_Tunnel_Verkeersbuis2_KnopHerstel_Waarnemer_c_onderbroken(state);
        if (!proceed) return false;

        // Check automaton "Tunnel_Verkeersbuis2_KnopHerstel_Waarnemer".
        proceed = Aut_Tunnel_Verkeersbuis2_KnopHerstel_Waarnemer.fillSyncData_evt_Tunnel_Verkeersbuis2_KnopHerstel_Waarnemer_c_onderbroken(state);
        if (!proceed) return false;
