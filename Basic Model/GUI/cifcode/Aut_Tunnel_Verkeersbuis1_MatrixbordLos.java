...
/** Automaton "Tunnel_Verkeersbuis1_MatrixbordLos". */
public final class Aut_Tunnel_Verkeersbuis1_MatrixbordLos extends RuntimeSimpleAutomaton<State> {
    public Aut_Tunnel_Verkeersbuis1_MatrixbordLos() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_Tunnel_Verkeersbuis1_MatrixbordLos", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "Tunnel_Verkeersbuis1_MatrixbordLos";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_Tunnel_Verkeersbuis1_MatrixbordLos.lp_Tunnel_Verkeersbuis1_MatrixbordLos);
    }
    ...
    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis1_MatrixbordLos_c_aan(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(579).get(1);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(359);
        fillSyncData(aut, 579, rslt, state.s_Tunnel_Verkeersbuis1_MatrixbordLos.lp_Tunnel_Verkeersbuis1_MatrixbordLos);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis1_MatrixbordLos_c_uit(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(580).get(1);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(359);
        fillSyncData(aut, 580, rslt, state.s_Tunnel_Verkeersbuis1_MatrixbordLos.lp_Tunnel_Verkeersbuis1_MatrixbordLos);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_Tunnel_Verkeersbuis1_MatrixbordLos = target.s_Tunnel_Verkeersbuis1_MatrixbordLos.copy();
        target.s_Tunnel_Verkeersbuis1_MatrixbordLos.lp_Tunnel_Verkeersbuis1_MatrixbordLos = value;
    }
}
