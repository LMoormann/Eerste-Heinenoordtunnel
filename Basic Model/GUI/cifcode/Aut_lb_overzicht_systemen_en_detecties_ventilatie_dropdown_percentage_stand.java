...
/** Automaton "lb_overzicht_systemen_en_detecties_ventilatie_dropdown_percentage_stand". */
public final class Aut_lb_overzicht_systemen_en_detecties_ventilatie_dropdown_percentage_stand extends RuntimeSimpleAutomaton<State> {
    public Aut_lb_overzicht_systemen_en_detecties_ventilatie_dropdown_percentage_stand() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_lb_overzicht_systemen_en_detecties_ventilatie_dropdown_percentage_stand", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "lb_overzicht_systemen_en_detecties_ventilatie_dropdown_percentage_stand";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_lb_overzicht_systemen_en_detecties_ventilatie_dropdown_percentage_stand.lp_lb_overzicht_systemen_en_detecties_ventilatie_dropdown_percentage_stand);
    }
    ...
    public static boolean fillSyncData_evt_lb_overzicht_systemen_en_detecties_ventilatie_dropdown_percentage_stand_open_drop_down_menu(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(55).get(0);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(25);
        fillSyncData(aut, 55, rslt, state.s_lb_overzicht_systemen_en_detecties_ventilatie_dropdown_percentage_stand.lp_lb_overzicht_systemen_en_detecties_ventilatie_dropdown_percentage_stand);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_lb_overzicht_systemen_en_detecties_ventilatie_dropdown_percentage_stand_sluit_drop_down_menu(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(56).get(0);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(25);
        fillSyncData(aut, 56, rslt, state.s_lb_overzicht_systemen_en_detecties_ventilatie_dropdown_percentage_stand.lp_lb_overzicht_systemen_en_detecties_ventilatie_dropdown_percentage_stand);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_sluit_alle_tabs(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(157).get(21);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(25);
        fillSyncData(aut, 157, rslt, state.s_lb_overzicht_systemen_en_detecties_ventilatie_dropdown_percentage_stand.lp_lb_overzicht_systemen_en_detecties_ventilatie_dropdown_percentage_stand);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_lb_overzicht_systemen_en_detecties_ventilatie_dropdown_percentage_stand = target.s_lb_overzicht_systemen_en_detecties_ventilatie_dropdown_percentage_stand.copy();
        target.s_lb_overzicht_systemen_en_detecties_ventilatie_dropdown_percentage_stand.lp_lb_overzicht_systemen_en_detecties_ventilatie_dropdown_percentage_stand = value;
    }
}
