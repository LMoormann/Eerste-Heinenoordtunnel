...
/** Automaton "Tunnel_Verkeersbuis2_KnopHerstel_Knop". */
public final class Aut_Tunnel_Verkeersbuis2_KnopHerstel_Knop extends RuntimeSimpleAutomaton<State> {
    public Aut_Tunnel_Verkeersbuis2_KnopHerstel_Knop() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_Tunnel_Verkeersbuis2_KnopHerstel_Knop", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "Tunnel_Verkeersbuis2_KnopHerstel_Knop";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_Tunnel_Verkeersbuis2_KnopHerstel_Knop.lp_Tunnel_Verkeersbuis2_KnopHerstel_Knop);
    }
    ...
    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis2_KnopHerstel_Knop_u_indrukken(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(822).get(0);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(471);
        fillSyncData(aut, 822, rslt, state.s_Tunnel_Verkeersbuis2_KnopHerstel_Knop.lp_Tunnel_Verkeersbuis2_KnopHerstel_Knop);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_Tunnel_Verkeersbuis2_KnopHerstel_Knop = target.s_Tunnel_Verkeersbuis2_KnopHerstel_Knop.copy();
        target.s_Tunnel_Verkeersbuis2_KnopHerstel_Knop.lp_Tunnel_Verkeersbuis2_KnopHerstel_Knop = value;
    }
}
