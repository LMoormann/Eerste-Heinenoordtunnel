...
/** Automaton "Tunnel_MiddenTunnelKanaal_DynamischeVluchtrouteIndicatie". */
public final class Aut_Tunnel_MiddenTunnelKanaal_DynamischeVluchtrouteIndicatie extends RuntimeSimpleAutomaton<State> {
    public Aut_Tunnel_MiddenTunnelKanaal_DynamischeVluchtrouteIndicatie() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_Tunnel_MiddenTunnelKanaal_DynamischeVluchtrouteIndicatie", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "Tunnel_MiddenTunnelKanaal_DynamischeVluchtrouteIndicatie";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_Tunnel_MiddenTunnelKanaal_DynamischeVluchtrouteIndicatie.lp_Tunnel_MiddenTunnelKanaal_DynamischeVluchtrouteIndicatie);
    }
    ...
    public static boolean fillSyncData_evt_Tunnel_MiddenTunnelKanaal_DynamischeVluchtrouteIndicatie_c_aflopend(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(308).get(3);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(206);
        fillSyncData(aut, 308, rslt, state.s_Tunnel_MiddenTunnelKanaal_DynamischeVluchtrouteIndicatie.lp_Tunnel_MiddenTunnelKanaal_DynamischeVluchtrouteIndicatie);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_MiddenTunnelKanaal_DynamischeVluchtrouteIndicatie_c_oplopend(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(309).get(3);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(206);
        fillSyncData(aut, 309, rslt, state.s_Tunnel_MiddenTunnelKanaal_DynamischeVluchtrouteIndicatie.lp_Tunnel_MiddenTunnelKanaal_DynamischeVluchtrouteIndicatie);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_MiddenTunnelKanaal_DynamischeVluchtrouteIndicatie_c_uit(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(310).get(3);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(206);
        fillSyncData(aut, 310, rslt, state.s_Tunnel_MiddenTunnelKanaal_DynamischeVluchtrouteIndicatie.lp_Tunnel_MiddenTunnelKanaal_DynamischeVluchtrouteIndicatie);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_Tunnel_MiddenTunnelKanaal_DynamischeVluchtrouteIndicatie = target.s_Tunnel_MiddenTunnelKanaal_DynamischeVluchtrouteIndicatie.copy();
        target.s_Tunnel_MiddenTunnelKanaal_DynamischeVluchtrouteIndicatie.lp_Tunnel_MiddenTunnelKanaal_DynamischeVluchtrouteIndicatie = value;
    }
}
