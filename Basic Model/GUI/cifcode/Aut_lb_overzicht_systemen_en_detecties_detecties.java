...
/** Automaton "lb_overzicht_systemen_en_detecties_detecties". */
public final class Aut_lb_overzicht_systemen_en_detecties_detecties extends RuntimeSimpleAutomaton<State> {
    public Aut_lb_overzicht_systemen_en_detecties_detecties() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_lb_overzicht_systemen_en_detecties_detecties", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "lb_overzicht_systemen_en_detecties_detecties";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_lb_overzicht_systemen_en_detecties_detecties.lp_lb_overzicht_systemen_en_detecties_detecties);
    }
    ...
    public static boolean fillSyncData_evt_lb_overzicht_systemen_en_detecties_detecties_open_drop_down_menu(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(27).get(0);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(11);
        fillSyncData(aut, 27, rslt, state.s_lb_overzicht_systemen_en_detecties_detecties.lp_lb_overzicht_systemen_en_detecties_detecties);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_lb_overzicht_systemen_en_detecties_detecties_sluit_drop_down_menu(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(28).get(0);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(11);
        fillSyncData(aut, 28, rslt, state.s_lb_overzicht_systemen_en_detecties_detecties.lp_lb_overzicht_systemen_en_detecties_detecties);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_sluit_alle_tabs(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(157).get(7);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(11);
        fillSyncData(aut, 157, rslt, state.s_lb_overzicht_systemen_en_detecties_detecties.lp_lb_overzicht_systemen_en_detecties_detecties);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_lb_overzicht_systemen_en_detecties_detecties = target.s_lb_overzicht_systemen_en_detecties_detecties.copy();
        target.s_lb_overzicht_systemen_en_detecties_detecties.lp_lb_overzicht_systemen_en_detecties_detecties = value;
    }
}
