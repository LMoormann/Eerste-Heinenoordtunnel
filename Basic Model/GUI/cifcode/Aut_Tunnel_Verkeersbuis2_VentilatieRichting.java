...
/** Automaton "Tunnel_Verkeersbuis2_VentilatieRichting". */
public final class Aut_Tunnel_Verkeersbuis2_VentilatieRichting extends RuntimeSimpleAutomaton<State> {
    public Aut_Tunnel_Verkeersbuis2_VentilatieRichting() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_Tunnel_Verkeersbuis2_VentilatieRichting", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "Tunnel_Verkeersbuis2_VentilatieRichting";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_Tunnel_Verkeersbuis2_VentilatieRichting.lp_Tunnel_Verkeersbuis2_VentilatieRichting);
    }
    ...
    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis2_VentilatieRichting_c_rijrichting(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(991).get(1);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(558);
        fillSyncData(aut, 991, rslt, state.s_Tunnel_Verkeersbuis2_VentilatieRichting.lp_Tunnel_Verkeersbuis2_VentilatieRichting);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis2_VentilatieRichting_c_tegenRijrichting(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(992).get(1);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(558);
        fillSyncData(aut, 992, rslt, state.s_Tunnel_Verkeersbuis2_VentilatieRichting.lp_Tunnel_Verkeersbuis2_VentilatieRichting);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_Tunnel_Verkeersbuis2_VentilatieRichting = target.s_Tunnel_Verkeersbuis2_VentilatieRichting.copy();
        target.s_Tunnel_Verkeersbuis2_VentilatieRichting.lp_Tunnel_Verkeersbuis2_VentilatieRichting = value;
    }
}
