...
/** Event "Tunnel_MiddenPompkelder_Regime_c_leeghouden". */
public final class Event_Tunnel_MiddenPompkelder_Regime_c_leeghouden extends RuntimeEvent<State> {
    public Event_Tunnel_MiddenPompkelder_Regime_c_leeghouden() {
        super("Tunnel_MiddenPompkelder_Regime_c_leeghouden", 272, true);
    }
    ...
    public boolean fillData(State state) {
        ...
        proceed = Aut_sup6.fillSyncData_evt_Tunnel_MiddenPompkelder_Regime_c_leeghouden(state);
        if (!proceed) return false;

        // Check automaton "sup86".
        proceed = Aut_sup86.fillSyncData_evt_Tunnel_MiddenPompkelder_Regime_c_leeghouden(state);
        if (!proceed) return false;

        // Check automaton "sup88".
        proceed = Aut_sup88.fillSyncData_evt_Tunnel_MiddenPompkelder_Regime_c_leeghouden(state);
        if (!proceed) return false;

        // Check automaton "Tunnel_MiddenPompkelder_Regime".
        proceed = Aut_Tunnel_MiddenPompkelder_Regime.fillSyncData_evt_Tunnel_MiddenPompkelder_Regime_c_leeghouden(state);
        if (!proceed) return false;
    ...
    public boolean allowedByInvs(State state) {
        // Invariants for "".
        try {
            if (!(((state.s_Tunnel_MiddenPompkelder_Bediening_Modus.lp_Tunnel_MiddenPompkelder_Bediening_Modus == 0) && ((state.s_Tunnel_Verkeersbuis1_StandVerkeersbuis.lp_Tunnel_Verkeersbuis1_StandVerkeersbuis == 0) && (state.s_Tunnel_Verkeersbuis2_StandVerkeersbuis.lp_Tunnel_Verkeersbuis2_StandVerkeersbuis == 0))) || ((state.s_Tunnel_MiddenPompkelder_Bediening_Modus.lp_Tunnel_MiddenPompkelder_Bediening_Modus == 1) && (state.s_Tunnel_MiddenPompkelder_KnopLeeghouden_Waarnemer.lp_Tunnel_MiddenPompkelder_KnopLeeghouden_Waarnemer == 1)))) return false;
        } catch (CifSimulatorException e) {
            throw new CifSimulatorException("Evaluation of invariant \"Tunnel_MiddenPompkelder_Regime_c_leeghouden needs Tunnel_MiddenPompkelder_Bediening_Modus.auto and (Tunnel_Verkeersbuis1_StandVerkeersbuis.operationeel and Tunnel_Verkeersbuis2_StandVerkeersbuis.operationeel) or Tunnel_MiddenPompkelder_Bediening_Modus.hand and Tunnel_MiddenPompkelder_KnopLeeghouden_Waarnemer.ingedrukt\" of the top level scope of the specification failed.", e, state);
        }
