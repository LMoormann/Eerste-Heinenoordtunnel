...
/** Event "Tunnel_MiddenTunnelKanaal_Luidspreker_c_uit". */
public final class Event_Tunnel_MiddenTunnelKanaal_Luidspreker_c_uit extends RuntimeEvent<State> {
    public Event_Tunnel_MiddenTunnelKanaal_Luidspreker_c_uit() {
        super("Tunnel_MiddenTunnelKanaal_Luidspreker_c_uit", 345, true);
    }
    ...
    public boolean fillData(State state) {
        ...
        proceed = Aut_sup6.fillSyncData_evt_Tunnel_MiddenTunnelKanaal_Luidspreker_c_uit(state);
        if (!proceed) return false;

        // Check automaton "sup82".
        proceed = Aut_sup82.fillSyncData_evt_Tunnel_MiddenTunnelKanaal_Luidspreker_c_uit(state);
        if (!proceed) return false;

        // Check automaton "sup85".
        proceed = Aut_sup85.fillSyncData_evt_Tunnel_MiddenTunnelKanaal_Luidspreker_c_uit(state);
        if (!proceed) return false;

        // Check automaton "Tunnel_MiddenTunnelKanaal_Luidspreker".
        proceed = Aut_Tunnel_MiddenTunnelKanaal_Luidspreker.fillSyncData_evt_Tunnel_MiddenTunnelKanaal_Luidspreker_c_uit(state);
        if (!proceed) return false;
    ...
    public boolean allowedByInvs(State state) {
        // Invariants for "".
        try {
            if (!((state.s_Tunnel_Verkeersbuis1_Evacuatie.lp_Tunnel_Verkeersbuis1_Evacuatie == 0) && (state.s_Tunnel_Verkeersbuis2_Evacuatie.lp_Tunnel_Verkeersbuis2_Evacuatie == 0))) return false;
        } catch (CifSimulatorException e) {
            throw new CifSimulatorException("Evaluation of invariant \"Tunnel_MiddenTunnelKanaal_Luidspreker_c_uit needs Tunnel_Verkeersbuis1_Evacuatie.uit and Tunnel_Verkeersbuis2_Evacuatie.uit\" of the top level scope of the specification failed.", e, state);
        }
