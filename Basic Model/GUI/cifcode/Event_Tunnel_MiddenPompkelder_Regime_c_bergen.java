...
/** Event "Tunnel_MiddenPompkelder_Regime_c_bergen". */
public final class Event_Tunnel_MiddenPompkelder_Regime_c_bergen extends RuntimeEvent<State> {
    public Event_Tunnel_MiddenPompkelder_Regime_c_bergen() {
        super("Tunnel_MiddenPompkelder_Regime_c_bergen", 271, true);
    }
    ...
    public boolean fillData(State state) {
        ...
        proceed = Aut_sup6.fillSyncData_evt_Tunnel_MiddenPompkelder_Regime_c_bergen(state);
        if (!proceed) return false;

        // Check automaton "sup86".
        proceed = Aut_sup86.fillSyncData_evt_Tunnel_MiddenPompkelder_Regime_c_bergen(state);
        if (!proceed) return false;

        // Check automaton "sup88".
        proceed = Aut_sup88.fillSyncData_evt_Tunnel_MiddenPompkelder_Regime_c_bergen(state);
        if (!proceed) return false;

        // Check automaton "Tunnel_MiddenPompkelder_Regime".
        proceed = Aut_Tunnel_MiddenPompkelder_Regime.fillSyncData_evt_Tunnel_MiddenPompkelder_Regime_c_bergen(state);
        if (!proceed) return false;
    ...
    public boolean allowedByInvs(State state) {
        // Invariants for "".
        try {
            if (!(((state.s_Tunnel_MiddenPompkelder_Bediening_Modus.lp_Tunnel_MiddenPompkelder_Bediening_Modus == 0) && ((((state.s_Tunnel_Verkeersbuis1_StandVerkeersbuis.lp_Tunnel_Verkeersbuis1_StandVerkeersbuis == 1) || (state.s_Tunnel_Verkeersbuis1_StandVerkeersbuis.lp_Tunnel_Verkeersbuis1_StandVerkeersbuis == 2)) || (state.s_Tunnel_Verkeersbuis1_StandVerkeersbuis.lp_Tunnel_Verkeersbuis1_StandVerkeersbuis == 1)) || (state.s_Tunnel_Verkeersbuis2_StandVerkeersbuis.lp_Tunnel_Verkeersbuis2_StandVerkeersbuis == 2))) || ((state.s_Tunnel_MiddenPompkelder_Bediening_Modus.lp_Tunnel_MiddenPompkelder_Bediening_Modus == 1) && (state.s_Tunnel_MiddenPompkelder_KnopBergen_Waarnemer.lp_Tunnel_MiddenPompkelder_KnopBergen_Waarnemer == 1)))) return false;
        } catch (CifSimulatorException e) {
            throw new CifSimulatorException("Evaluation of invariant \"Tunnel_MiddenPompkelder_Regime_c_bergen needs Tunnel_MiddenPompkelder_Bediening_Modus.auto and (Tunnel_Verkeersbuis1_StandVerkeersbuis.calamiteit or Tunnel_Verkeersbuis1_StandVerkeersbuis.herstel or Tunnel_Verkeersbuis1_StandVerkeersbuis.calamiteit or Tunnel_Verkeersbuis2_StandVerkeersbuis.herstel) or Tunnel_MiddenPompkelder_Bediening_Modus.hand and Tunnel_MiddenPompkelder_KnopBergen_Waarnemer.ingedrukt\" of the top level scope of the specification failed.", e, state);
        }
