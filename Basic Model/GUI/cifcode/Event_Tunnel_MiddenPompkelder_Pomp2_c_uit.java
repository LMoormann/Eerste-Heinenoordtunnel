...
/** Event "Tunnel_MiddenPompkelder_Pomp2_c_uit". */
public final class Event_Tunnel_MiddenPompkelder_Pomp2_c_uit extends RuntimeEvent<State> {
    public Event_Tunnel_MiddenPompkelder_Pomp2_c_uit() {
        super("Tunnel_MiddenPompkelder_Pomp2_c_uit", 268, true);
    }
    ...
    public boolean fillData(State state) {
        ...
        proceed = Aut_sup86.fillSyncData_evt_Tunnel_MiddenPompkelder_Pomp2_c_uit(state);
        if (!proceed) return false;

        // Check automaton "Tunnel_MiddenPompkelder_Pomp2".
        proceed = Aut_Tunnel_MiddenPompkelder_Pomp2.fillSyncData_evt_Tunnel_MiddenPompkelder_Pomp2_c_uit(state);
        if (!proceed) return false;
    ...
    public boolean allowedByInvs(State state) {
        // Invariants for "".
        try {
            if (!((((state.s_Tunnel_MiddenPompkelder_Regime.lp_Tunnel_MiddenPompkelder_Regime == 1) && (state.s_Tunnel_MiddenPompkelder_SensorLaaghoogwater.lp_Tunnel_MiddenPompkelder_SensorLaaghoogwater == 0)) || ((state.s_Tunnel_MiddenPompkelder_Regime.lp_Tunnel_MiddenPompkelder_Regime == 0) && (state.s_Tunnel_MiddenPompkelder_SensorLaagwater.lp_Tunnel_MiddenPompkelder_SensorLaagwater == 0))) || (state.s_Tunnel_MiddenPompkelder_Regime.lp_Tunnel_MiddenPompkelder_Regime == 2))) return false;
        } catch (CifSimulatorException e) {
            throw new CifSimulatorException("Evaluation of invariant \"Tunnel_MiddenPompkelder_Pomp2_c_uit needs Tunnel_MiddenPompkelder_Regime.bergen and Tunnel_MiddenPompkelder_SensorLaaghoogwater.uit or Tunnel_MiddenPompkelder_Regime.leeghouden and Tunnel_MiddenPompkelder_SensorLaagwater.uit or Tunnel_MiddenPompkelder_Regime.uit\" of the top level scope of the specification failed.", e, state);
        }
