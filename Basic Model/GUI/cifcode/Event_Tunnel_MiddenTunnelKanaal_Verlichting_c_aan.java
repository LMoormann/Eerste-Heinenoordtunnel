...
/** Event "Tunnel_MiddenTunnelKanaal_Verlichting_c_aan". */
public final class Event_Tunnel_MiddenTunnelKanaal_Verlichting_c_aan extends RuntimeEvent<State> {
    public Event_Tunnel_MiddenTunnelKanaal_Verlichting_c_aan() {
        super("Tunnel_MiddenTunnelKanaal_Verlichting_c_aan", 349, true);
    }
    ...
    public boolean fillData(State state) {
        ...
        proceed = Aut_sup42.fillSyncData_evt_Tunnel_MiddenTunnelKanaal_Verlichting_c_aan(state);
        if (!proceed) return false;

        // Check automaton "sup6".
        proceed = Aut_sup6.fillSyncData_evt_Tunnel_MiddenTunnelKanaal_Verlichting_c_aan(state);
        if (!proceed) return false;

        // Check automaton "Tunnel_MiddenTunnelKanaal_Verlichting".
        proceed = Aut_Tunnel_MiddenTunnelKanaal_Verlichting.fillSyncData_evt_Tunnel_MiddenTunnelKanaal_Verlichting_c_aan(state);
        if (!proceed) return false;
    ...
    public boolean allowedByInvs(State state) {
        // Invariants for "".
        try {
            if (!(((state.s_Tunnel_MiddenTunnelKanaal_BedieningVerlichting_Modus.lp_Tunnel_MiddenTunnelKanaal_BedieningVerlichting_Modus == 0) && ((((state.s_Tunnel_Verkeersbuis1_Standby.lp_Tunnel_Verkeersbuis1_Standby == 1) || (state.s_Tunnel_Verkeersbuis1_StandVerkeersbuis.lp_Tunnel_Verkeersbuis1_StandVerkeersbuis == 1)) || (state.s_Tunnel_Verkeersbuis2_Standby.lp_Tunnel_Verkeersbuis2_Standby == 1)) || (state.s_Tunnel_Verkeersbuis2_StandVerkeersbuis.lp_Tunnel_Verkeersbuis2_StandVerkeersbuis == 1))) || ((state.s_Tunnel_MiddenTunnelKanaal_BedieningVerlichting_Modus.lp_Tunnel_MiddenTunnelKanaal_BedieningVerlichting_Modus == 1) && (state.s_Tunnel_MiddenTunnelKanaal_KnopVerlichtingAan_Waarnemer.lp_Tunnel_MiddenTunnelKanaal_KnopVerlichtingAan_Waarnemer == 1)))) return false;
        } catch (CifSimulatorException e) {
            throw new CifSimulatorException("Evaluation of invariant \"Tunnel_MiddenTunnelKanaal_Verlichting_c_aan needs Tunnel_MiddenTunnelKanaal_BedieningVerlichting_Modus.auto and (Tunnel_Verkeersbuis1_Standby.aan or Tunnel_Verkeersbuis1_StandVerkeersbuis.calamiteit or Tunnel_Verkeersbuis2_Standby.aan or Tunnel_Verkeersbuis2_StandVerkeersbuis.calamiteit) or Tunnel_MiddenTunnelKanaal_BedieningVerlichting_Modus.hand and Tunnel_MiddenTunnelKanaal_KnopVerlichtingAan_Waarnemer.ingedrukt\" of the top level scope of the specification failed.", e, state);
        }
