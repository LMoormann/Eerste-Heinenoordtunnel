...
/** Automaton "Tunnel_HoofdPompkelder2_Regime". */
public final class Aut_Tunnel_HoofdPompkelder2_Regime extends RuntimeSimpleAutomaton<State> {
    public Aut_Tunnel_HoofdPompkelder2_Regime() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_Tunnel_HoofdPompkelder2_Regime", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "Tunnel_HoofdPompkelder2_Regime";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_Tunnel_HoofdPompkelder2_Regime.lp_Tunnel_HoofdPompkelder2_Regime);
    }
    ...
    public static boolean fillSyncData_evt_Tunnel_HoofdPompkelder2_Regime_c_bergen(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(235).get(3);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(164);
        fillSyncData(aut, 235, rslt, state.s_Tunnel_HoofdPompkelder2_Regime.lp_Tunnel_HoofdPompkelder2_Regime);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_HoofdPompkelder2_Regime_c_leeghouden(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(236).get(3);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(164);
        fillSyncData(aut, 236, rslt, state.s_Tunnel_HoofdPompkelder2_Regime.lp_Tunnel_HoofdPompkelder2_Regime);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_HoofdPompkelder2_Regime_c_uit(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(237).get(3);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(164);
        fillSyncData(aut, 237, rslt, state.s_Tunnel_HoofdPompkelder2_Regime.lp_Tunnel_HoofdPompkelder2_Regime);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_Tunnel_HoofdPompkelder2_Regime = target.s_Tunnel_HoofdPompkelder2_Regime.copy();
        target.s_Tunnel_HoofdPompkelder2_Regime.lp_Tunnel_HoofdPompkelder2_Regime = value;
    }
}
