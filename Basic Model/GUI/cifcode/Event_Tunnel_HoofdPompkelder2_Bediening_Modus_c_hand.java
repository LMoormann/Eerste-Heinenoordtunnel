...
/** Event "Tunnel_HoofdPompkelder2_Bediening_Modus_c_hand". */
public final class Event_Tunnel_HoofdPompkelder2_Bediening_Modus_c_hand extends RuntimeEvent<State> {
    public Event_Tunnel_HoofdPompkelder2_Bediening_Modus_c_hand() {
        super("Tunnel_HoofdPompkelder2_Bediening_Modus_c_hand", 221, true);
    }
    ...
    public boolean fillData(State state) {
        ...
        proceed = Aut_sup6.fillSyncData_evt_Tunnel_HoofdPompkelder2_Bediening_Modus_c_hand(state);
        if (!proceed) return false;

        // Check automaton "sup93".
        proceed = Aut_sup93.fillSyncData_evt_Tunnel_HoofdPompkelder2_Bediening_Modus_c_hand(state);
        if (!proceed) return false;

        // Check automaton "sup95".
        proceed = Aut_sup95.fillSyncData_evt_Tunnel_HoofdPompkelder2_Bediening_Modus_c_hand(state);
        if (!proceed) return false;

        // Check automaton "Tunnel_HoofdPompkelder2_Bediening_Modus".
        proceed = Aut_Tunnel_HoofdPompkelder2_Bediening_Modus.fillSyncData_evt_Tunnel_HoofdPompkelder2_Bediening_Modus_c_hand(state);
        if (!proceed) return false;
    ...
    public boolean allowedByInvs(State state) {
        // Invariants for "".
        try {
            if (!(state.s_Tunnel_HoofdPompkelder2_Bediening_KnopHand_Waarnemer.lp_Tunnel_HoofdPompkelder2_Bediening_KnopHand_Waarnemer == 1)) return false;
        } catch (CifSimulatorException e) {
            throw new CifSimulatorException("Evaluation of invariant \"Tunnel_HoofdPompkelder2_Bediening_Modus_c_hand needs Tunnel_HoofdPompkelder2_Bediening_KnopHand_Waarnemer.ingedrukt\" of the top level scope of the specification failed.", e, state);
        }
