...
/** Automaton "rb_overzicht_systemen_en_detecties_verlichting_dropdown_percentage_stand". */
public final class Aut_rb_overzicht_systemen_en_detecties_verlichting_dropdown_percentage_stand extends RuntimeSimpleAutomaton<State> {
    public Aut_rb_overzicht_systemen_en_detecties_verlichting_dropdown_percentage_stand() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_rb_overzicht_systemen_en_detecties_verlichting_dropdown_percentage_stand", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "rb_overzicht_systemen_en_detecties_verlichting_dropdown_percentage_stand";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_rb_overzicht_systemen_en_detecties_verlichting_dropdown_percentage_stand.lp_rb_overzicht_systemen_en_detecties_verlichting_dropdown_percentage_stand);
    }
    ...
    public static boolean fillSyncData_evt_rb_overzicht_systemen_en_detecties_verlichting_dropdown_percentage_stand_open_drop_down_menu(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(137).get(0);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(66);
        fillSyncData(aut, 137, rslt, state.s_rb_overzicht_systemen_en_detecties_verlichting_dropdown_percentage_stand.lp_rb_overzicht_systemen_en_detecties_verlichting_dropdown_percentage_stand);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_rb_overzicht_systemen_en_detecties_verlichting_dropdown_percentage_stand_sluit_drop_down_menu(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(138).get(0);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(66);
        fillSyncData(aut, 138, rslt, state.s_rb_overzicht_systemen_en_detecties_verlichting_dropdown_percentage_stand.lp_rb_overzicht_systemen_en_detecties_verlichting_dropdown_percentage_stand);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_sluit_alle_tabs(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(157).get(62);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(66);
        fillSyncData(aut, 157, rslt, state.s_rb_overzicht_systemen_en_detecties_verlichting_dropdown_percentage_stand.lp_rb_overzicht_systemen_en_detecties_verlichting_dropdown_percentage_stand);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_rb_overzicht_systemen_en_detecties_verlichting_dropdown_percentage_stand = target.s_rb_overzicht_systemen_en_detecties_verlichting_dropdown_percentage_stand.copy();
        target.s_rb_overzicht_systemen_en_detecties_verlichting_dropdown_percentage_stand.lp_rb_overzicht_systemen_en_detecties_verlichting_dropdown_percentage_stand = value;
    }
}
