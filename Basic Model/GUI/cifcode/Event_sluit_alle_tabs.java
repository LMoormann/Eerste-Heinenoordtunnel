...
/** Event "sluit_alle_tabs". */
public final class Event_sluit_alle_tabs extends RuntimeEvent<State> {
    public Event_sluit_alle_tabs() {
        super("sluit_alle_tabs", 157, true);
    }
    ...
    public boolean fillData(State state) {
        ...
        // Check automaton "bevestigde_melding_sos_rechterbuis".
        proceed = Aut_bevestigde_melding_sos_rechterbuis.fillSyncData_evt_sluit_alle_tabs(state);
        if (!proceed) return false;

        // Check automaton "lb_bedienmenu_voor_verkeerslichtenstanden".
        proceed = Aut_lb_bedienmenu_voor_verkeerslichtenstanden.fillSyncData_evt_sluit_alle_tabs(state);
        if (!proceed) return false;

        // Check automaton "lb_drop_down_menu_buis_states".
        proceed = Aut_lb_drop_down_menu_buis_states.fillSyncData_evt_sluit_alle_tabs(state);
        if (!proceed) return false;

        // Check automaton "lb_overzicht_systemen_en_detecties_blusser".
        proceed = Aut_lb_overzicht_systemen_en_detecties_blusser.fillSyncData_evt_sluit_alle_tabs(state);
        if (!proceed) return false;

        // Check automaton "lb_overzicht_systemen_en_detecties_blusvoorziening".
        proceed = Aut_lb_overzicht_systemen_en_detecties_blusvoorziening.fillSyncData_evt_sluit_alle_tabs(state);
        if (!proceed) return false;

        // Check automaton "lb_overzicht_systemen_en_detecties_bluswatervoorziening_drop_down_stand".
        proceed = Aut_lb_overzicht_systemen_en_detecties_bluswatervoorziening_drop_down_stand.fillSyncData_evt_sluit_alle_tabs(state);
        if (!proceed) return false;

        // Check automaton "lb_overzicht_systemen_en_detecties_buis".
        proceed = Aut_lb_overzicht_systemen_en_detecties_buis.fillSyncData_evt_sluit_alle_tabs(state);
        if (!proceed) return false;

        // Check automaton "lb_overzicht_systemen_en_detecties_detecties".
        proceed = Aut_lb_overzicht_systemen_en_detecties_detecties.fillSyncData_evt_sluit_alle_tabs(state);
        if (!proceed) return false;

        // Check automaton "lb_overzicht_systemen_en_detecties_deur".
        proceed = Aut_lb_overzicht_systemen_en_detecties_deur.fillSyncData_evt_sluit_alle_tabs(state);
        if (!proceed) return false;

        // Check automaton "lb_overzicht_systemen_en_detecties_drop_down_richting_ventilatie".
        proceed = Aut_lb_overzicht_systemen_en_detecties_drop_down_richting_ventilatie.fillSyncData_evt_sluit_alle_tabs(state);
        if (!proceed) return false;

        // Check automaton "lb_overzicht_systemen_en_detecties_dynamische_vluchtroute".
        proceed = Aut_lb_overzicht_systemen_en_detecties_dynamische_vluchtroute.fillSyncData_evt_sluit_alle_tabs(state);
        if (!proceed) return false;

        // Check automaton "lb_overzicht_systemen_en_detecties_dynamische_vluchtroute_dropdown_stand".
        proceed = Aut_lb_overzicht_systemen_en_detecties_dynamische_vluchtroute_dropdown_stand.fillSyncData_evt_sluit_alle_tabs(state);
        if (!proceed) return false;

        // Check automaton "lb_overzicht_systemen_en_detecties_haspel".
        proceed = Aut_lb_overzicht_systemen_en_detecties_haspel.fillSyncData_evt_sluit_alle_tabs(state);
        if (!proceed) return false;

        // Check automaton "lb_overzicht_systemen_en_detecties_hulppost".
        proceed = Aut_lb_overzicht_systemen_en_detecties_hulppost.fillSyncData_evt_sluit_alle_tabs(state);
        if (!proceed) return false;

        // Check automaton "lb_overzicht_systemen_en_detecties_noodtelefoon".
        proceed = Aut_lb_overzicht_systemen_en_detecties_noodtelefoon.fillSyncData_evt_sluit_alle_tabs(state);
        if (!proceed) return false;

        // Check automaton "lb_overzicht_systemen_en_detecties_overdruk_veilige_ruimte".
        proceed = Aut_lb_overzicht_systemen_en_detecties_overdruk_veilige_ruimte.fillSyncData_evt_sluit_alle_tabs(state);
        if (!proceed) return false;

        // Check automaton "lb_overzicht_systemen_en_detecties_overdruk_veilige_ruimte_drop_down_stand".
        proceed = Aut_lb_overzicht_systemen_en_detecties_overdruk_veilige_ruimte_drop_down_stand.fillSyncData_evt_sluit_alle_tabs(state);
        if (!proceed) return false;

        // Check automaton "lb_overzicht_systemen_en_detecties_overig".
        proceed = Aut_lb_overzicht_systemen_en_detecties_overig.fillSyncData_evt_sluit_alle_tabs(state);
        if (!proceed) return false;

        // Check automaton "lb_overzicht_systemen_en_detecties_SOS".
        proceed = Aut_lb_overzicht_systemen_en_detecties_SOS.fillSyncData_evt_sluit_alle_tabs(state);
        if (!proceed) return false;

        // Check automaton "lb_overzicht_systemen_en_detecties_tunnel".
        proceed = Aut_lb_overzicht_systemen_en_detecties_tunnel.fillSyncData_evt_sluit_alle_tabs(state);
        if (!proceed) return false;

        // Check automaton "lb_overzicht_systemen_en_detecties_ventilatie".
        proceed = Aut_lb_overzicht_systemen_en_detecties_ventilatie.fillSyncData_evt_sluit_alle_tabs(state);
        if (!proceed) return false;

        // Check automaton "lb_overzicht_systemen_en_detecties_ventilatie_dropdown_percentage_stand".
        proceed = Aut_lb_overzicht_systemen_en_detecties_ventilatie_dropdown_percentage_stand.fillSyncData_evt_sluit_alle_tabs(state);
        if (!proceed) return false;

        // Check automaton "lb_overzicht_systemen_en_detecties_verkeer".
        proceed = Aut_lb_overzicht_systemen_en_detecties_verkeer.fillSyncData_evt_sluit_alle_tabs(state);
        if (!proceed) return false;

        // Check automaton "lb_overzicht_systemen_en_detecties_verlichting".
        proceed = Aut_lb_overzicht_systemen_en_detecties_verlichting.fillSyncData_evt_sluit_alle_tabs(state);
        if (!proceed) return false;

        // Check automaton "lb_overzicht_systemen_en_detecties_verlichting_dropdown_percentage_stand".
        proceed = Aut_lb_overzicht_systemen_en_detecties_verlichting_dropdown_percentage_stand.fillSyncData_evt_sluit_alle_tabs(state);
        if (!proceed) return false;

        // Check automaton "lb_overzicht_systemen_en_detecties_verlichting_veilige_ruimte".
        proceed = Aut_lb_overzicht_systemen_en_detecties_verlichting_veilige_ruimte.fillSyncData_evt_sluit_alle_tabs(state);
        if (!proceed) return false;

        // Check automaton "lb_overzicht_systemen_en_detecties_verlichting_veilige_ruimte_drop_down_stand".
        proceed = Aut_lb_overzicht_systemen_en_detecties_verlichting_veilige_ruimte_drop_down_stand.fillSyncData_evt_sluit_alle_tabs(state);
        if (!proceed) return false;

        // Check automaton "lb_overzicht_systemen_en_detecties_vloeistofafvoer".
        proceed = Aut_lb_overzicht_systemen_en_detecties_vloeistofafvoer.fillSyncData_evt_sluit_alle_tabs(state);
        if (!proceed) return false;

        // Check automaton "lb_overzicht_systemen_en_detecties_vluchtdeuren".
        proceed = Aut_lb_overzicht_systemen_en_detecties_vluchtdeuren.fillSyncData_evt_sluit_alle_tabs(state);
        if (!proceed) return false;

        // Check automaton "lb_overzicht_systemen_en_detecties_vluchtdeurindicatie".
        proceed = Aut_lb_overzicht_systemen_en_detecties_vluchtdeurindicatie.fillSyncData_evt_sluit_alle_tabs(state);
        if (!proceed) return false;

        // Check automaton "lb_overzicht_systemen_en_detecties_vluchtdeurindicatie_dropdown_contourverlichting_stand".
        proceed = Aut_lb_overzicht_systemen_en_detecties_vluchtdeurindicatie_dropdown_contourverlichting_stand.fillSyncData_evt_sluit_alle_tabs(state);
        if (!proceed) return false;

        // Check automaton "lb_overzicht_systemen_en_detecties_vluchtdeurindicatie_dropdown_geluidsbakens_stand".
        proceed = Aut_lb_overzicht_systemen_en_detecties_vluchtdeurindicatie_dropdown_geluidsbakens_stand.fillSyncData_evt_sluit_alle_tabs(state);
        if (!proceed) return false;

        // Check automaton "lb_overzicht_systemen_en_detecties_vluchtroute".
        proceed = Aut_lb_overzicht_systemen_en_detecties_vluchtroute.fillSyncData_evt_sluit_alle_tabs(state);
        if (!proceed) return false;

        // Check automaton "lb_overzicht_systemen_en_detecties_VRI_en_afsluitboom".
        proceed = Aut_lb_overzicht_systemen_en_detecties_VRI_en_afsluitboom.fillSyncData_evt_sluit_alle_tabs(state);
        if (!proceed) return false;

        // Check automaton "melding_sos_rechterbuis_geselecteerd".
        proceed = Aut_melding_sos_rechterbuis_geselecteerd.fillSyncData_evt_sluit_alle_tabs(state);
        if (!proceed) return false;

        // Check automaton "openen_en_sluiten_bevestigde_meldingen".
        proceed = Aut_openen_en_sluiten_bevestigde_meldingen.fillSyncData_evt_sluit_alle_tabs(state);
        if (!proceed) return false;

        // Check automaton "overzicht_systemen_en_detecties_drop_down_hoofd_pomp_kelder_1_stand_automaton".
        proceed = Aut_overzicht_systemen_en_detecties_drop_down_hoofd_pomp_kelder_1_stand_automaton.fillSyncData_evt_sluit_alle_tabs(state);
        if (!proceed) return false;

        // Check automaton "overzicht_systemen_en_detecties_drop_down_hoofd_pomp_kelder_2_stand_automaton".
        proceed = Aut_overzicht_systemen_en_detecties_drop_down_hoofd_pomp_kelder_2_stand_automaton.fillSyncData_evt_sluit_alle_tabs(state);
        if (!proceed) return false;

        // Check automaton "overzicht_systemen_en_detecties_drop_down_midden_pomp_kelder_stand_automaton".
        proceed = Aut_overzicht_systemen_en_detecties_drop_down_midden_pomp_kelder_stand_automaton.fillSyncData_evt_sluit_alle_tabs(state);
        if (!proceed) return false;

        // Check automaton "rb_bedienmenu_voor_verkeerslichtenstanden".
        proceed = Aut_rb_bedienmenu_voor_verkeerslichtenstanden.fillSyncData_evt_sluit_alle_tabs(state);
        if (!proceed) return false;

        // Check automaton "rb_drop_down_menu_buis_states".
        proceed = Aut_rb_drop_down_menu_buis_states.fillSyncData_evt_sluit_alle_tabs(state);
        if (!proceed) return false;

        // Check automaton "rb_overzicht_systemen_en_detecties_blusser".
        proceed = Aut_rb_overzicht_systemen_en_detecties_blusser.fillSyncData_evt_sluit_alle_tabs(state);
        if (!proceed) return false;

        // Check automaton "rb_overzicht_systemen_en_detecties_blusvoorziening".
        proceed = Aut_rb_overzicht_systemen_en_detecties_blusvoorziening.fillSyncData_evt_sluit_alle_tabs(state);
        if (!proceed) return false;

        // Check automaton "rb_overzicht_systemen_en_detecties_bluswatervoorziening_drop_down_stand".
        proceed = Aut_rb_overzicht_systemen_en_detecties_bluswatervoorziening_drop_down_stand.fillSyncData_evt_sluit_alle_tabs(state);
        if (!proceed) return false;

        // Check automaton "rb_overzicht_systemen_en_detecties_buis".
        proceed = Aut_rb_overzicht_systemen_en_detecties_buis.fillSyncData_evt_sluit_alle_tabs(state);
        if (!proceed) return false;

        // Check automaton "rb_overzicht_systemen_en_detecties_detecties".
        proceed = Aut_rb_overzicht_systemen_en_detecties_detecties.fillSyncData_evt_sluit_alle_tabs(state);
        if (!proceed) return false;

        // Check automaton "rb_overzicht_systemen_en_detecties_deur".
        proceed = Aut_rb_overzicht_systemen_en_detecties_deur.fillSyncData_evt_sluit_alle_tabs(state);
        if (!proceed) return false;

        // Check automaton "rb_overzicht_systemen_en_detecties_drop_down_richting_ventilatie".
        proceed = Aut_rb_overzicht_systemen_en_detecties_drop_down_richting_ventilatie.fillSyncData_evt_sluit_alle_tabs(state);
        if (!proceed) return false;

        // Check automaton "rb_overzicht_systemen_en_detecties_dynamische_vluchtroute".
        proceed = Aut_rb_overzicht_systemen_en_detecties_dynamische_vluchtroute.fillSyncData_evt_sluit_alle_tabs(state);
        if (!proceed) return false;

        // Check automaton "rb_overzicht_systemen_en_detecties_dynamische_vluchtroute_dropdown_stand".
        proceed = Aut_rb_overzicht_systemen_en_detecties_dynamische_vluchtroute_dropdown_stand.fillSyncData_evt_sluit_alle_tabs(state);
        if (!proceed) return false;

        // Check automaton "rb_overzicht_systemen_en_detecties_haspel".
        proceed = Aut_rb_overzicht_systemen_en_detecties_haspel.fillSyncData_evt_sluit_alle_tabs(state);
        if (!proceed) return false;

        // Check automaton "rb_overzicht_systemen_en_detecties_hulppost".
        proceed = Aut_rb_overzicht_systemen_en_detecties_hulppost.fillSyncData_evt_sluit_alle_tabs(state);
        if (!proceed) return false;

        // Check automaton "rb_overzicht_systemen_en_detecties_noodtelefoon".
        proceed = Aut_rb_overzicht_systemen_en_detecties_noodtelefoon.fillSyncData_evt_sluit_alle_tabs(state);
        if (!proceed) return false;

        // Check automaton "rb_overzicht_systemen_en_detecties_overdruk_veilige_ruimte".
        proceed = Aut_rb_overzicht_systemen_en_detecties_overdruk_veilige_ruimte.fillSyncData_evt_sluit_alle_tabs(state);
        if (!proceed) return false;

        // Check automaton "rb_overzicht_systemen_en_detecties_overdruk_veilige_ruimte_drop_down_stand".
        proceed = Aut_rb_overzicht_systemen_en_detecties_overdruk_veilige_ruimte_drop_down_stand.fillSyncData_evt_sluit_alle_tabs(state);
        if (!proceed) return false;

        // Check automaton "rb_overzicht_systemen_en_detecties_overig".
        proceed = Aut_rb_overzicht_systemen_en_detecties_overig.fillSyncData_evt_sluit_alle_tabs(state);
        if (!proceed) return false;

        // Check automaton "rb_overzicht_systemen_en_detecties_SOS".
        proceed = Aut_rb_overzicht_systemen_en_detecties_SOS.fillSyncData_evt_sluit_alle_tabs(state);
        if (!proceed) return false;

        // Check automaton "rb_overzicht_systemen_en_detecties_tunnel".
        proceed = Aut_rb_overzicht_systemen_en_detecties_tunnel.fillSyncData_evt_sluit_alle_tabs(state);
        if (!proceed) return false;

        // Check automaton "rb_overzicht_systemen_en_detecties_ventilatie".
        proceed = Aut_rb_overzicht_systemen_en_detecties_ventilatie.fillSyncData_evt_sluit_alle_tabs(state);
        if (!proceed) return false;

        // Check automaton "rb_overzicht_systemen_en_detecties_ventilatie_dropdown_percentage_stand".
        proceed = Aut_rb_overzicht_systemen_en_detecties_ventilatie_dropdown_percentage_stand.fillSyncData_evt_sluit_alle_tabs(state);
        if (!proceed) return false;

        // Check automaton "rb_overzicht_systemen_en_detecties_verkeer".
        proceed = Aut_rb_overzicht_systemen_en_detecties_verkeer.fillSyncData_evt_sluit_alle_tabs(state);
        if (!proceed) return false;

        // Check automaton "rb_overzicht_systemen_en_detecties_verlichting".
        proceed = Aut_rb_overzicht_systemen_en_detecties_verlichting.fillSyncData_evt_sluit_alle_tabs(state);
        if (!proceed) return false;

        // Check automaton "rb_overzicht_systemen_en_detecties_verlichting_dropdown_percentage_stand".
        proceed = Aut_rb_overzicht_systemen_en_detecties_verlichting_dropdown_percentage_stand.fillSyncData_evt_sluit_alle_tabs(state);
        if (!proceed) return false;

        // Check automaton "rb_overzicht_systemen_en_detecties_verlichting_veilige_ruimte".
        proceed = Aut_rb_overzicht_systemen_en_detecties_verlichting_veilige_ruimte.fillSyncData_evt_sluit_alle_tabs(state);
        if (!proceed) return false;

        // Check automaton "rb_overzicht_systemen_en_detecties_verlichting_veilige_ruimte_drop_down_stand".
        proceed = Aut_rb_overzicht_systemen_en_detecties_verlichting_veilige_ruimte_drop_down_stand.fillSyncData_evt_sluit_alle_tabs(state);
        if (!proceed) return false;

        // Check automaton "rb_overzicht_systemen_en_detecties_vloeistofafvoer".
        proceed = Aut_rb_overzicht_systemen_en_detecties_vloeistofafvoer.fillSyncData_evt_sluit_alle_tabs(state);
        if (!proceed) return false;

        // Check automaton "rb_overzicht_systemen_en_detecties_vluchtdeuren".
        proceed = Aut_rb_overzicht_systemen_en_detecties_vluchtdeuren.fillSyncData_evt_sluit_alle_tabs(state);
        if (!proceed) return false;

        // Check automaton "rb_overzicht_systemen_en_detecties_vluchtdeurindicatie".
        proceed = Aut_rb_overzicht_systemen_en_detecties_vluchtdeurindicatie.fillSyncData_evt_sluit_alle_tabs(state);
        if (!proceed) return false;

        // Check automaton "rb_overzicht_systemen_en_detecties_vluchtdeurindicatie_dropdown_contourverlichting_stand".
        proceed = Aut_rb_overzicht_systemen_en_detecties_vluchtdeurindicatie_dropdown_contourverlichting_stand.fillSyncData_evt_sluit_alle_tabs(state);
        if (!proceed) return false;

        // Check automaton "rb_overzicht_systemen_en_detecties_vluchtdeurindicatie_dropdown_geluidsbakens_stand".
        proceed = Aut_rb_overzicht_systemen_en_detecties_vluchtdeurindicatie_dropdown_geluidsbakens_stand.fillSyncData_evt_sluit_alle_tabs(state);
        if (!proceed) return false;

        // Check automaton "rb_overzicht_systemen_en_detecties_vluchtroute".
        proceed = Aut_rb_overzicht_systemen_en_detecties_vluchtroute.fillSyncData_evt_sluit_alle_tabs(state);
        if (!proceed) return false;

        // Check automaton "rb_overzicht_systemen_en_detecties_VRI_en_afsluitboom".
        proceed = Aut_rb_overzicht_systemen_en_detecties_VRI_en_afsluitboom.fillSyncData_evt_sluit_alle_tabs(state);
        if (!proceed) return false;
