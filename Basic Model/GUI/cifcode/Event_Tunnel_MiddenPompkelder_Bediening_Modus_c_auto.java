...
/** Event "Tunnel_MiddenPompkelder_Bediening_Modus_c_auto". */
public final class Event_Tunnel_MiddenPompkelder_Bediening_Modus_c_auto extends RuntimeEvent<State> {
    public Event_Tunnel_MiddenPompkelder_Bediening_Modus_c_auto() {
        super("Tunnel_MiddenPompkelder_Bediening_Modus_c_auto", 254, true);
    }
    ...
    public boolean fillData(State state) {
        ...
        proceed = Aut_sup6.fillSyncData_evt_Tunnel_MiddenPompkelder_Bediening_Modus_c_auto(state);
        if (!proceed) return false;

        // Check automaton "sup86".
        proceed = Aut_sup86.fillSyncData_evt_Tunnel_MiddenPompkelder_Bediening_Modus_c_auto(state);
        if (!proceed) return false;

        // Check automaton "sup88".
        proceed = Aut_sup88.fillSyncData_evt_Tunnel_MiddenPompkelder_Bediening_Modus_c_auto(state);
        if (!proceed) return false;

        // Check automaton "Tunnel_MiddenPompkelder_Bediening_Modus".
        proceed = Aut_Tunnel_MiddenPompkelder_Bediening_Modus.fillSyncData_evt_Tunnel_MiddenPompkelder_Bediening_Modus_c_auto(state);
        if (!proceed) return false;
    ...
    public boolean allowedByInvs(State state) {
        // Invariants for "".
        try {
            if (!(state.s_Tunnel_MiddenPompkelder_Bediening_KnopAuto_Waarnemer.lp_Tunnel_MiddenPompkelder_Bediening_KnopAuto_Waarnemer == 1)) return false;
        } catch (CifSimulatorException e) {
            throw new CifSimulatorException("Evaluation of invariant \"Tunnel_MiddenPompkelder_Bediening_Modus_c_auto needs Tunnel_MiddenPompkelder_Bediening_KnopAuto_Waarnemer.ingedrukt\" of the top level scope of the specification failed.", e, state);
        }
