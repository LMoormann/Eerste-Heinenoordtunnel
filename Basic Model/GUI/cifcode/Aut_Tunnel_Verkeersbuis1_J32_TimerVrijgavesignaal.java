...
/** Automaton "Tunnel_Verkeersbuis1_J32_TimerVrijgavesignaal". */
public final class Aut_Tunnel_Verkeersbuis1_J32_TimerVrijgavesignaal extends RuntimeSimpleAutomaton<State> {
    public Aut_Tunnel_Verkeersbuis1_J32_TimerVrijgavesignaal() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_Tunnel_Verkeersbuis1_J32_TimerVrijgavesignaal", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "Tunnel_Verkeersbuis1_J32_TimerVrijgavesignaal";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_Tunnel_Verkeersbuis1_J32_TimerVrijgavesignaal.lp_Tunnel_Verkeersbuis1_J32_TimerVrijgavesignaal);
    }
    ...
    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis1_J32_TimerVrijgavesignaal_c_aan(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(425).get(2);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(271);
        fillSyncData(aut, 425, rslt, state.s_Tunnel_Verkeersbuis1_J32_TimerVrijgavesignaal.lp_Tunnel_Verkeersbuis1_J32_TimerVrijgavesignaal);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis1_J32_TimerVrijgavesignaal_c_reset(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(426).get(2);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(271);
        fillSyncData(aut, 426, rslt, state.s_Tunnel_Verkeersbuis1_J32_TimerVrijgavesignaal.lp_Tunnel_Verkeersbuis1_J32_TimerVrijgavesignaal);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis1_J32_TimerVrijgavesignaal_u_timeout(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(427).get(1);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(271);
        fillSyncData(aut, 427, rslt, state.s_Tunnel_Verkeersbuis1_J32_TimerVrijgavesignaal.lp_Tunnel_Verkeersbuis1_J32_TimerVrijgavesignaal);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_Tunnel_Verkeersbuis1_J32_TimerVrijgavesignaal = target.s_Tunnel_Verkeersbuis1_J32_TimerVrijgavesignaal.copy();
        target.s_Tunnel_Verkeersbuis1_J32_TimerVrijgavesignaal.lp_Tunnel_Verkeersbuis1_J32_TimerVrijgavesignaal = value;
    }
}
