...
/** Event "Tunnel_MiddenTunnelKanaal_Overdruk_c_rechts". */
public final class Event_Tunnel_MiddenTunnelKanaal_Overdruk_c_rechts extends RuntimeEvent<State> {
    public Event_Tunnel_MiddenTunnelKanaal_Overdruk_c_rechts() {
        super("Tunnel_MiddenTunnelKanaal_Overdruk_c_rechts", 347, true);
    }
    ...
    public boolean fillData(State state) {
        ...
        proceed = Aut_sup35.fillSyncData_evt_Tunnel_MiddenTunnelKanaal_Overdruk_c_rechts(state);
        if (!proceed) return false;

        // Check automaton "sup41".
        proceed = Aut_sup41.fillSyncData_evt_Tunnel_MiddenTunnelKanaal_Overdruk_c_rechts(state);
        if (!proceed) return false;

        // Check automaton "sup6".
        proceed = Aut_sup6.fillSyncData_evt_Tunnel_MiddenTunnelKanaal_Overdruk_c_rechts(state);
        if (!proceed) return false;

        // Check automaton "Tunnel_MiddenTunnelKanaal_Overdruk".
        proceed = Aut_Tunnel_MiddenTunnelKanaal_Overdruk.fillSyncData_evt_Tunnel_MiddenTunnelKanaal_Overdruk_c_rechts(state);
        if (!proceed) return false;
    ...
    public boolean allowedByInvs(State state) {
        // Invariants for "".
        try {
            if (!(state.s_Tunnel_MiddenTunnelKanaal_Overdruk.lp_Tunnel_MiddenTunnelKanaal_Overdruk == 0)) return false;
        } catch (CifSimulatorException e) {
            throw new CifSimulatorException("Evaluation of invariant \"Tunnel_MiddenTunnelKanaal_Overdruk_c_rechts needs Tunnel_MiddenTunnelKanaal_Overdruk.uit\" of the top level scope of the specification failed.", e, state);
        }

        try {
            if (!(((state.s_Tunnel_MiddenTunnelKanaal_BedieningOverdruk_Modus.lp_Tunnel_MiddenTunnelKanaal_BedieningOverdruk_Modus == 0) && ((state.s_Tunnel_Verkeersbuis2_Standby.lp_Tunnel_Verkeersbuis2_Standby == 1) || ((!(state.s_Tunnel_Verkeersbuis2_StandVerkeersbuis.lp_Tunnel_Verkeersbuis2_StandVerkeersbuis == 0)) && (!(state.s_Tunnel_Verkeersbuis2_StandVerkeersbuis.lp_Tunnel_Verkeersbuis2_StandVerkeersbuis == 3))))) || ((state.s_Tunnel_MiddenTunnelKanaal_BedieningOverdruk_Modus.lp_Tunnel_MiddenTunnelKanaal_BedieningOverdruk_Modus == 1) && (state.s_Tunnel_MiddenTunnelKanaal_KnopOverdrukRechts_Waarnemer.lp_Tunnel_MiddenTunnelKanaal_KnopOverdrukRechts_Waarnemer == 1)))) return false;
        } catch (CifSimulatorException e) {
            throw new CifSimulatorException("Evaluation of invariant \"Tunnel_MiddenTunnelKanaal_Overdruk_c_rechts needs Tunnel_MiddenTunnelKanaal_BedieningOverdruk_Modus.auto and (Tunnel_Verkeersbuis2_Standby.aan or not Tunnel_Verkeersbuis2_StandVerkeersbuis.operationeel and not Tunnel_Verkeersbuis2_StandVerkeersbuis.ondersteunend) or Tunnel_MiddenTunnelKanaal_BedieningOverdruk_Modus.hand and Tunnel_MiddenTunnelKanaal_KnopOverdrukRechts_Waarnemer.ingedrukt\" of the top level scope of the specification failed.", e, state);
        }
