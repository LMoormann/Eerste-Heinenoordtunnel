...
/** Automaton "rb_overzicht_systemen_en_detecties_blusvoorziening". */
public final class Aut_rb_overzicht_systemen_en_detecties_blusvoorziening extends RuntimeSimpleAutomaton<State> {
    public Aut_rb_overzicht_systemen_en_detecties_blusvoorziening() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_rb_overzicht_systemen_en_detecties_blusvoorziening", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "rb_overzicht_systemen_en_detecties_blusvoorziening";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_rb_overzicht_systemen_en_detecties_blusvoorziening.lp_rb_overzicht_systemen_en_detecties_blusvoorziening);
    }
    ...
    public static boolean fillSyncData_evt_rb_overzicht_systemen_en_detecties_blusvoorziening_open_drop_down_menu(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(97).get(0);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(46);
        fillSyncData(aut, 97, rslt, state.s_rb_overzicht_systemen_en_detecties_blusvoorziening.lp_rb_overzicht_systemen_en_detecties_blusvoorziening);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_rb_overzicht_systemen_en_detecties_blusvoorziening_sluit_drop_down_menu(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(98).get(0);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(46);
        fillSyncData(aut, 98, rslt, state.s_rb_overzicht_systemen_en_detecties_blusvoorziening.lp_rb_overzicht_systemen_en_detecties_blusvoorziening);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_sluit_alle_tabs(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(157).get(42);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(46);
        fillSyncData(aut, 157, rslt, state.s_rb_overzicht_systemen_en_detecties_blusvoorziening.lp_rb_overzicht_systemen_en_detecties_blusvoorziening);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_rb_overzicht_systemen_en_detecties_blusvoorziening = target.s_rb_overzicht_systemen_en_detecties_blusvoorziening.copy();
        target.s_rb_overzicht_systemen_en_detecties_blusvoorziening.lp_rb_overzicht_systemen_en_detecties_blusvoorziening = value;
    }
}
