...
/** Automaton "rb_overzicht_systemen_en_detecties_blusser". */
public final class Aut_rb_overzicht_systemen_en_detecties_blusser extends RuntimeSimpleAutomaton<State> {
    public Aut_rb_overzicht_systemen_en_detecties_blusser() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_rb_overzicht_systemen_en_detecties_blusser", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "rb_overzicht_systemen_en_detecties_blusser";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_rb_overzicht_systemen_en_detecties_blusser.lp_rb_overzicht_systemen_en_detecties_blusser);
    }
    ...
    public static boolean fillSyncData_evt_rb_overzicht_systemen_en_detecties_blusser_open_drop_down_menu(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(95).get(0);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(45);
        fillSyncData(aut, 95, rslt, state.s_rb_overzicht_systemen_en_detecties_blusser.lp_rb_overzicht_systemen_en_detecties_blusser);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_rb_overzicht_systemen_en_detecties_blusser_sluit_drop_down_menu(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(96).get(0);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(45);
        fillSyncData(aut, 96, rslt, state.s_rb_overzicht_systemen_en_detecties_blusser.lp_rb_overzicht_systemen_en_detecties_blusser);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_sluit_alle_tabs(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(157).get(41);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(45);
        fillSyncData(aut, 157, rslt, state.s_rb_overzicht_systemen_en_detecties_blusser.lp_rb_overzicht_systemen_en_detecties_blusser);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_rb_overzicht_systemen_en_detecties_blusser = target.s_rb_overzicht_systemen_en_detecties_blusser.copy();
        target.s_rb_overzicht_systemen_en_detecties_blusser.lp_rb_overzicht_systemen_en_detecties_blusser = value;
    }
}
