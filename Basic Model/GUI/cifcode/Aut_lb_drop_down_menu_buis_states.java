...
/** Automaton "lb_drop_down_menu_buis_states". */
public final class Aut_lb_drop_down_menu_buis_states extends RuntimeSimpleAutomaton<State> {
    public Aut_lb_drop_down_menu_buis_states() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_lb_drop_down_menu_buis_states", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "lb_drop_down_menu_buis_states";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_lb_drop_down_menu_buis_states.lp_lb_drop_down_menu_buis_states);
    }
    ...
    public static boolean fillSyncData_evt_lb_drop_down_menu_buis_states_open_drop_down_menu(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(17).get(0);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(6);
        fillSyncData(aut, 17, rslt, state.s_lb_drop_down_menu_buis_states.lp_lb_drop_down_menu_buis_states);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_lb_drop_down_menu_buis_states_sluit_drop_down_menu(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(18).get(0);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(6);
        fillSyncData(aut, 18, rslt, state.s_lb_drop_down_menu_buis_states.lp_lb_drop_down_menu_buis_states);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_sluit_alle_tabs(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(157).get(2);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(6);
        fillSyncData(aut, 157, rslt, state.s_lb_drop_down_menu_buis_states.lp_lb_drop_down_menu_buis_states);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_lb_drop_down_menu_buis_states = target.s_lb_drop_down_menu_buis_states.copy();
        target.s_lb_drop_down_menu_buis_states.lp_lb_drop_down_menu_buis_states = value;
    }
}
