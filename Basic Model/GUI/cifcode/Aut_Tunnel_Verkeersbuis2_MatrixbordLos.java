...
/** Automaton "Tunnel_Verkeersbuis2_MatrixbordLos". */
public final class Aut_Tunnel_Verkeersbuis2_MatrixbordLos extends RuntimeSimpleAutomaton<State> {
    public Aut_Tunnel_Verkeersbuis2_MatrixbordLos() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_Tunnel_Verkeersbuis2_MatrixbordLos", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "Tunnel_Verkeersbuis2_MatrixbordLos";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_Tunnel_Verkeersbuis2_MatrixbordLos.lp_Tunnel_Verkeersbuis2_MatrixbordLos);
    }
    ...
    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis2_MatrixbordLos_c_aan(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(937).get(1);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(541);
        fillSyncData(aut, 937, rslt, state.s_Tunnel_Verkeersbuis2_MatrixbordLos.lp_Tunnel_Verkeersbuis2_MatrixbordLos);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis2_MatrixbordLos_c_uit(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(938).get(1);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(541);
        fillSyncData(aut, 938, rslt, state.s_Tunnel_Verkeersbuis2_MatrixbordLos.lp_Tunnel_Verkeersbuis2_MatrixbordLos);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_Tunnel_Verkeersbuis2_MatrixbordLos = target.s_Tunnel_Verkeersbuis2_MatrixbordLos.copy();
        target.s_Tunnel_Verkeersbuis2_MatrixbordLos.lp_Tunnel_Verkeersbuis2_MatrixbordLos = value;
    }
}
