...
/** Event "Tunnel_Verkeersbuis2_StandVerkeersbuis_c_operationeel". */
public final class Event_Tunnel_Verkeersbuis2_StandVerkeersbuis_c_operationeel extends RuntimeEvent<State> {
    public Event_Tunnel_Verkeersbuis2_StandVerkeersbuis_c_operationeel() {
        super("Tunnel_Verkeersbuis2_StandVerkeersbuis_c_operationeel", 972, true);
    }
    ...
    public boolean fillData(State state) {
        ...
        proceed = Aut_sup48.fillSyncData_evt_Tunnel_Verkeersbuis2_StandVerkeersbuis_c_operationeel(state);
        if (!proceed) return false;

        // Check automaton "sup6".
        proceed = Aut_sup6.fillSyncData_evt_Tunnel_Verkeersbuis2_StandVerkeersbuis_c_operationeel(state);
        if (!proceed) return false;

        // Check automaton "sup72".
        proceed = Aut_sup72.fillSyncData_evt_Tunnel_Verkeersbuis2_StandVerkeersbuis_c_operationeel(state);
        if (!proceed) return false;

        // Check automaton "sup76".
        proceed = Aut_sup76.fillSyncData_evt_Tunnel_Verkeersbuis2_StandVerkeersbuis_c_operationeel(state);
        if (!proceed) return false;

        // Check automaton "sup80".
        proceed = Aut_sup80.fillSyncData_evt_Tunnel_Verkeersbuis2_StandVerkeersbuis_c_operationeel(state);
        if (!proceed) return false;

        // Check automaton "Tunnel_Verkeersbuis2_StandVerkeersbuis".
        proceed = Aut_Tunnel_Verkeersbuis2_StandVerkeersbuis.fillSyncData_evt_Tunnel_Verkeersbuis2_StandVerkeersbuis_c_operationeel(state);
        if (!proceed) return false;
    ...
    public boolean allowedByInvs(State state) {
        // Invariants for "".
        try {
            if (!(state.s_Tunnel_Verkeersbuis2_StandVerkeersbuis.lp_Tunnel_Verkeersbuis2_StandVerkeersbuis == 2)) return false;
        } catch (CifSimulatorException e) {
            throw new CifSimulatorException("Evaluation of invariant \"Tunnel_Verkeersbuis2_StandVerkeersbuis_c_operationeel needs Tunnel_Verkeersbuis2_StandVerkeersbuis.herstel\" of the top level scope of the specification failed.", e, state);
        }

        try {
            if (!(state.s_Tunnel_Verkeersbuis2_KnopOperationeel_Waarnemer.lp_Tunnel_Verkeersbuis2_KnopOperationeel_Waarnemer == 1)) return false;
        } catch (CifSimulatorException e) {
            throw new CifSimulatorException("Evaluation of invariant \"Tunnel_Verkeersbuis2_StandVerkeersbuis_c_operationeel needs Tunnel_Verkeersbuis2_KnopOperationeel_Waarnemer.ingedrukt\" of the top level scope of the specification failed.", e, state);
        }
