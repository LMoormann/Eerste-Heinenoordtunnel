...
/** Automaton "Tunnel_MiddenPompkelder_Pomp2". */
public final class Aut_Tunnel_MiddenPompkelder_Pomp2 extends RuntimeSimpleAutomaton<State> {
    public Aut_Tunnel_MiddenPompkelder_Pomp2() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_Tunnel_MiddenPompkelder_Pomp2", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "Tunnel_MiddenPompkelder_Pomp2";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_Tunnel_MiddenPompkelder_Pomp2.lp_Tunnel_MiddenPompkelder_Pomp2);
    }
    ...
    public static boolean fillSyncData_evt_Tunnel_MiddenPompkelder_Pomp2_c_aan(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(267).get(1);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(183);
        fillSyncData(aut, 267, rslt, state.s_Tunnel_MiddenPompkelder_Pomp2.lp_Tunnel_MiddenPompkelder_Pomp2);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_MiddenPompkelder_Pomp2_c_uit(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(268).get(1);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(183);
        fillSyncData(aut, 268, rslt, state.s_Tunnel_MiddenPompkelder_Pomp2.lp_Tunnel_MiddenPompkelder_Pomp2);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_Tunnel_MiddenPompkelder_Pomp2 = target.s_Tunnel_MiddenPompkelder_Pomp2.copy();
        target.s_Tunnel_MiddenPompkelder_Pomp2.lp_Tunnel_MiddenPompkelder_Pomp2 = value;
    }
}
