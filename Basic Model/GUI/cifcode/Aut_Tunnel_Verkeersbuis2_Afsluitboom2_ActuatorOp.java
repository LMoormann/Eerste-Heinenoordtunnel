...
/** Automaton "Tunnel_Verkeersbuis2_Afsluitboom2_ActuatorOp". */
public final class Aut_Tunnel_Verkeersbuis2_Afsluitboom2_ActuatorOp extends RuntimeSimpleAutomaton<State> {
    public Aut_Tunnel_Verkeersbuis2_Afsluitboom2_ActuatorOp() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_Tunnel_Verkeersbuis2_Afsluitboom2_ActuatorOp", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "Tunnel_Verkeersbuis2_Afsluitboom2_ActuatorOp";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_Tunnel_Verkeersbuis2_Afsluitboom2_ActuatorOp.lp_Tunnel_Verkeersbuis2_Afsluitboom2_ActuatorOp);
    }
    ...
    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis2_Afsluitboom2_ActuatorOp_c_aan(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(724).get(4);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(419);
        fillSyncData(aut, 724, rslt, state.s_Tunnel_Verkeersbuis2_Afsluitboom2_ActuatorOp.lp_Tunnel_Verkeersbuis2_Afsluitboom2_ActuatorOp);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis2_Afsluitboom2_ActuatorOp_c_uit(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(725).get(4);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(419);
        fillSyncData(aut, 725, rslt, state.s_Tunnel_Verkeersbuis2_Afsluitboom2_ActuatorOp.lp_Tunnel_Verkeersbuis2_Afsluitboom2_ActuatorOp);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis2_Afsluitboom2_ActuatorOp_c_uitNood(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(726).get(4);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(419);
        fillSyncData(aut, 726, rslt, state.s_Tunnel_Verkeersbuis2_Afsluitboom2_ActuatorOp.lp_Tunnel_Verkeersbuis2_Afsluitboom2_ActuatorOp);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_Tunnel_Verkeersbuis2_Afsluitboom2_ActuatorOp = target.s_Tunnel_Verkeersbuis2_Afsluitboom2_ActuatorOp.copy();
        target.s_Tunnel_Verkeersbuis2_Afsluitboom2_ActuatorOp.lp_Tunnel_Verkeersbuis2_Afsluitboom2_ActuatorOp = value;
    }
}
