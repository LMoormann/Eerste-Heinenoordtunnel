...
/** Event "Tunnel_Verkeersbuis1_Luidspreker_c_boodschap". */
public final class Event_Tunnel_Verkeersbuis1_Luidspreker_c_boodschap extends RuntimeEvent<State> {
    public Event_Tunnel_Verkeersbuis1_Luidspreker_c_boodschap() {
        super("Tunnel_Verkeersbuis1_Luidspreker_c_boodschap", 575, true);
    }
    ...
    public boolean fillData(State state) {
        ...
        proceed = Aut_sup6.fillSyncData_evt_Tunnel_Verkeersbuis1_Luidspreker_c_boodschap(state);
        if (!proceed) return false;

        // Check automaton "sup82".
        proceed = Aut_sup82.fillSyncData_evt_Tunnel_Verkeersbuis1_Luidspreker_c_boodschap(state);
        if (!proceed) return false;

        // Check automaton "sup83".
        proceed = Aut_sup83.fillSyncData_evt_Tunnel_Verkeersbuis1_Luidspreker_c_boodschap(state);
        if (!proceed) return false;

        // Check automaton "Tunnel_Verkeersbuis1_Luidspreker".
        proceed = Aut_Tunnel_Verkeersbuis1_Luidspreker.fillSyncData_evt_Tunnel_Verkeersbuis1_Luidspreker_c_boodschap(state);
        if (!proceed) return false;
    ...
    public boolean allowedByInvs(State state) {
        // Invariants for "".
        try {
            if (!(state.s_Tunnel_Verkeersbuis1_Vluchtdeur_Geluidsbaken.lp_Tunnel_Verkeersbuis1_Vluchtdeur_Geluidsbaken == 0)) return false;
        } catch (CifSimulatorException e) {
            throw new CifSimulatorException("Evaluation of invariant \"Tunnel_Verkeersbuis1_Luidspreker_c_boodschap needs Tunnel_Verkeersbuis1_Vluchtdeur_Geluidsbaken.uit\" of the top level scope of the specification failed.", e, state);
        }

        try {
            if (!((state.s_Tunnel_Verkeersbuis1_WisselTimerAudio.lp_Tunnel_Verkeersbuis1_WisselTimerAudio == 1) || (state.s_Tunnel_Verkeersbuis1_KnopStartBoodschap_Waarnemer.lp_Tunnel_Verkeersbuis1_KnopStartBoodschap_Waarnemer == 1))) return false;
        } catch (CifSimulatorException e) {
            throw new CifSimulatorException("Evaluation of invariant \"Tunnel_Verkeersbuis1_Luidspreker_c_boodschap needs Tunnel_Verkeersbuis1_WisselTimerAudio.aan1 or Tunnel_Verkeersbuis1_KnopStartBoodschap_Waarnemer.ingedrukt\" of the top level scope of the specification failed.", e, state);
        }
