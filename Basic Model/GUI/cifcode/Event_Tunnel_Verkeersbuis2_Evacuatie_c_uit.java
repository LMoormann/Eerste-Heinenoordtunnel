...
/** Event "Tunnel_Verkeersbuis2_Evacuatie_c_uit". */
public final class Event_Tunnel_Verkeersbuis2_Evacuatie_c_uit extends RuntimeEvent<State> {
    public Event_Tunnel_Verkeersbuis2_Evacuatie_c_uit() {
        super("Tunnel_Verkeersbuis2_Evacuatie_c_uit", 766, true);
    }
    ...
    public boolean fillData(State state) {
        ...
        proceed = Aut_sup48.fillSyncData_evt_Tunnel_Verkeersbuis2_Evacuatie_c_uit(state);
        if (!proceed) return false;

        // Check automaton "sup6".
        proceed = Aut_sup6.fillSyncData_evt_Tunnel_Verkeersbuis2_Evacuatie_c_uit(state);
        if (!proceed) return false;

        // Check automaton "sup72".
        proceed = Aut_sup72.fillSyncData_evt_Tunnel_Verkeersbuis2_Evacuatie_c_uit(state);
        if (!proceed) return false;

        // Check automaton "sup76".
        proceed = Aut_sup76.fillSyncData_evt_Tunnel_Verkeersbuis2_Evacuatie_c_uit(state);
        if (!proceed) return false;

        // Check automaton "sup80".
        proceed = Aut_sup80.fillSyncData_evt_Tunnel_Verkeersbuis2_Evacuatie_c_uit(state);
        if (!proceed) return false;

        // Check automaton "Tunnel_Verkeersbuis2_Evacuatie".
        proceed = Aut_Tunnel_Verkeersbuis2_Evacuatie.fillSyncData_evt_Tunnel_Verkeersbuis2_Evacuatie_c_uit(state);
        if (!proceed) return false;
    ...
    public boolean allowedByInvs(State state) {
        // Invariants for "".
        try {
            if (!((!(state.s_Tunnel_Verkeersbuis2_StandVerkeersbuis.lp_Tunnel_Verkeersbuis2_StandVerkeersbuis == 1)) || (state.s_Tunnel_Verkeersbuis2_KnopCalamiteit_Waarnemer.lp_Tunnel_Verkeersbuis2_KnopCalamiteit_Waarnemer == 1))) return false;
        } catch (CifSimulatorException e) {
            throw new CifSimulatorException("Evaluation of invariant \"Tunnel_Verkeersbuis2_Evacuatie_c_uit needs not Tunnel_Verkeersbuis2_StandVerkeersbuis.calamiteit or Tunnel_Verkeersbuis2_KnopCalamiteit_Waarnemer.ingedrukt\" of the top level scope of the specification failed.", e, state);
        }
