...
/** Automaton "Tunnel_Verkeersbuis2_Afsluitboom2_SensorOpen". */
public final class Aut_Tunnel_Verkeersbuis2_Afsluitboom2_SensorOpen extends RuntimeSimpleAutomaton<State> {
    public Aut_Tunnel_Verkeersbuis2_Afsluitboom2_SensorOpen() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_Tunnel_Verkeersbuis2_Afsluitboom2_SensorOpen", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "Tunnel_Verkeersbuis2_Afsluitboom2_SensorOpen";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_Tunnel_Verkeersbuis2_Afsluitboom2_SensorOpen.lp_Tunnel_Verkeersbuis2_Afsluitboom2_SensorOpen);
    }
    ...
    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis2_Afsluitboom2_SensorOpen_u_aan(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(731).get(2);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(423);
        fillSyncData(aut, 731, rslt, state.s_Tunnel_Verkeersbuis2_Afsluitboom2_SensorOpen.lp_Tunnel_Verkeersbuis2_Afsluitboom2_SensorOpen);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis2_Afsluitboom2_SensorOpen_u_uit(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(732).get(2);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(423);
        fillSyncData(aut, 732, rslt, state.s_Tunnel_Verkeersbuis2_Afsluitboom2_SensorOpen.lp_Tunnel_Verkeersbuis2_Afsluitboom2_SensorOpen);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_Tunnel_Verkeersbuis2_Afsluitboom2_SensorOpen = target.s_Tunnel_Verkeersbuis2_Afsluitboom2_SensorOpen.copy();
        target.s_Tunnel_Verkeersbuis2_Afsluitboom2_SensorOpen.lp_Tunnel_Verkeersbuis2_Afsluitboom2_SensorOpen = value;
    }
}
