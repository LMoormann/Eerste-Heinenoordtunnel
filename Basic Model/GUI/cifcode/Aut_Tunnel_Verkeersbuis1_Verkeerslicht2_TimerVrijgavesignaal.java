...
/** Automaton "Tunnel_Verkeersbuis1_Verkeerslicht2_TimerVrijgavesignaal". */
public final class Aut_Tunnel_Verkeersbuis1_Verkeerslicht2_TimerVrijgavesignaal extends RuntimeSimpleAutomaton<State> {
    public Aut_Tunnel_Verkeersbuis1_Verkeerslicht2_TimerVrijgavesignaal() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_Tunnel_Verkeersbuis1_Verkeerslicht2_TimerVrijgavesignaal", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "Tunnel_Verkeersbuis1_Verkeerslicht2_TimerVrijgavesignaal";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_Tunnel_Verkeersbuis1_Verkeerslicht2_TimerVrijgavesignaal.lp_Tunnel_Verkeersbuis1_Verkeerslicht2_TimerVrijgavesignaal);
    }
    ...
    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis1_Verkeerslicht2_TimerVrijgavesignaal_c_aan(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(648).get(4);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(383);
        fillSyncData(aut, 648, rslt, state.s_Tunnel_Verkeersbuis1_Verkeerslicht2_TimerVrijgavesignaal.lp_Tunnel_Verkeersbuis1_Verkeerslicht2_TimerVrijgavesignaal);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis1_Verkeerslicht2_TimerVrijgavesignaal_c_reset(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(649).get(4);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(383);
        fillSyncData(aut, 649, rslt, state.s_Tunnel_Verkeersbuis1_Verkeerslicht2_TimerVrijgavesignaal.lp_Tunnel_Verkeersbuis1_Verkeerslicht2_TimerVrijgavesignaal);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis1_Verkeerslicht2_TimerVrijgavesignaal_u_timeout(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(650).get(1);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(383);
        fillSyncData(aut, 650, rslt, state.s_Tunnel_Verkeersbuis1_Verkeerslicht2_TimerVrijgavesignaal.lp_Tunnel_Verkeersbuis1_Verkeerslicht2_TimerVrijgavesignaal);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_Tunnel_Verkeersbuis1_Verkeerslicht2_TimerVrijgavesignaal = target.s_Tunnel_Verkeersbuis1_Verkeerslicht2_TimerVrijgavesignaal.copy();
        target.s_Tunnel_Verkeersbuis1_Verkeerslicht2_TimerVrijgavesignaal.lp_Tunnel_Verkeersbuis1_Verkeerslicht2_TimerVrijgavesignaal = value;
    }
}
