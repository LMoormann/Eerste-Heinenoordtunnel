...
/** Automaton "Tunnel_MiddenPompkelder_PompRichting". */
public final class Aut_Tunnel_MiddenPompkelder_PompRichting extends RuntimeSimpleAutomaton<State> {
    public Aut_Tunnel_MiddenPompkelder_PompRichting() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_Tunnel_MiddenPompkelder_PompRichting", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "Tunnel_MiddenPompkelder_PompRichting";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_Tunnel_MiddenPompkelder_PompRichting.lp_Tunnel_MiddenPompkelder_PompRichting);
    }
    ...
    public static boolean fillSyncData_evt_Tunnel_MiddenPompkelder_PompRichting_c_hoofdPompkelder1(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(269).get(1);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(184);
        fillSyncData(aut, 269, rslt, state.s_Tunnel_MiddenPompkelder_PompRichting.lp_Tunnel_MiddenPompkelder_PompRichting);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_MiddenPompkelder_PompRichting_c_hoofdPompkelder2(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(270).get(1);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(184);
        fillSyncData(aut, 270, rslt, state.s_Tunnel_MiddenPompkelder_PompRichting.lp_Tunnel_MiddenPompkelder_PompRichting);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_Tunnel_MiddenPompkelder_PompRichting = target.s_Tunnel_MiddenPompkelder_PompRichting.copy();
        target.s_Tunnel_MiddenPompkelder_PompRichting.lp_Tunnel_MiddenPompkelder_PompRichting = value;
    }
}
