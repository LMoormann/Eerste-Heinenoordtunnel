...
/** Event "Tunnel_MiddenTunnelKanaal_BedieningOverdruk_Modus_c_auto". */
public final class Event_Tunnel_MiddenTunnelKanaal_BedieningOverdruk_Modus_c_auto extends RuntimeEvent<State> {
    public Event_Tunnel_MiddenTunnelKanaal_BedieningOverdruk_Modus_c_auto() {
        super("Tunnel_MiddenTunnelKanaal_BedieningOverdruk_Modus_c_auto", 298, true);
    }
    ...
    public boolean fillData(State state) {
        ...
        proceed = Aut_sup35.fillSyncData_evt_Tunnel_MiddenTunnelKanaal_BedieningOverdruk_Modus_c_auto(state);
        if (!proceed) return false;

        // Check automaton "sup41".
        proceed = Aut_sup41.fillSyncData_evt_Tunnel_MiddenTunnelKanaal_BedieningOverdruk_Modus_c_auto(state);
        if (!proceed) return false;

        // Check automaton "sup6".
        proceed = Aut_sup6.fillSyncData_evt_Tunnel_MiddenTunnelKanaal_BedieningOverdruk_Modus_c_auto(state);
        if (!proceed) return false;

        // Check automaton "Tunnel_MiddenTunnelKanaal_BedieningOverdruk_Modus".
        proceed = Aut_Tunnel_MiddenTunnelKanaal_BedieningOverdruk_Modus.fillSyncData_evt_Tunnel_MiddenTunnelKanaal_BedieningOverdruk_Modus_c_auto(state);
        if (!proceed) return false;
    ...
    public boolean allowedByInvs(State state) {
        // Invariants for "".
        try {
            if (!(state.s_Tunnel_MiddenTunnelKanaal_BedieningOverdruk_KnopAuto_Waarnemer.lp_Tunnel_MiddenTunnelKanaal_BedieningOverdruk_KnopAuto_Waarnemer == 1)) return false;
        } catch (CifSimulatorException e) {
            throw new CifSimulatorException("Evaluation of invariant \"Tunnel_MiddenTunnelKanaal_BedieningOverdruk_Modus_c_auto needs Tunnel_MiddenTunnelKanaal_BedieningOverdruk_KnopAuto_Waarnemer.ingedrukt\" of the top level scope of the specification failed.", e, state);
        }
