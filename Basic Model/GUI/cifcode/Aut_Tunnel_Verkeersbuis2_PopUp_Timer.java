...
/** Automaton "Tunnel_Verkeersbuis2_PopUp_Timer". */
public final class Aut_Tunnel_Verkeersbuis2_PopUp_Timer extends RuntimeSimpleAutomaton<State> {
    public Aut_Tunnel_Verkeersbuis2_PopUp_Timer() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_Tunnel_Verkeersbuis2_PopUp_Timer", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "Tunnel_Verkeersbuis2_PopUp_Timer";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_Tunnel_Verkeersbuis2_PopUp_Timer.lp_Tunnel_Verkeersbuis2_PopUp_Timer);
    }
    ...
    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis2_PopUp_Timer_c_aan(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(949).get(1);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(549);
        fillSyncData(aut, 949, rslt, state.s_Tunnel_Verkeersbuis2_PopUp_Timer.lp_Tunnel_Verkeersbuis2_PopUp_Timer);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis2_PopUp_Timer_c_reset(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(950).get(1);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(549);
        fillSyncData(aut, 950, rslt, state.s_Tunnel_Verkeersbuis2_PopUp_Timer.lp_Tunnel_Verkeersbuis2_PopUp_Timer);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis2_PopUp_Timer_u_timeout(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(951).get(1);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(549);
        fillSyncData(aut, 951, rslt, state.s_Tunnel_Verkeersbuis2_PopUp_Timer.lp_Tunnel_Verkeersbuis2_PopUp_Timer);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_Tunnel_Verkeersbuis2_PopUp_Timer = target.s_Tunnel_Verkeersbuis2_PopUp_Timer.copy();
        target.s_Tunnel_Verkeersbuis2_PopUp_Timer.lp_Tunnel_Verkeersbuis2_PopUp_Timer = value;
    }
}
