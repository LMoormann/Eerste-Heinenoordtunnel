...
/** Automaton "rb_drop_down_menu_buis_states". */
public final class Aut_rb_drop_down_menu_buis_states extends RuntimeSimpleAutomaton<State> {
    public Aut_rb_drop_down_menu_buis_states() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_rb_drop_down_menu_buis_states", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "rb_drop_down_menu_buis_states";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_rb_drop_down_menu_buis_states.lp_rb_drop_down_menu_buis_states);
    }
    ...
    public static boolean fillSyncData_evt_rb_drop_down_menu_buis_states_open_drop_down_menu(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(93).get(0);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(44);
        fillSyncData(aut, 93, rslt, state.s_rb_drop_down_menu_buis_states.lp_rb_drop_down_menu_buis_states);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_rb_drop_down_menu_buis_states_sluit_drop_down_menu(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(94).get(0);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(44);
        fillSyncData(aut, 94, rslt, state.s_rb_drop_down_menu_buis_states.lp_rb_drop_down_menu_buis_states);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_sluit_alle_tabs(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(157).get(40);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(44);
        fillSyncData(aut, 157, rslt, state.s_rb_drop_down_menu_buis_states.lp_rb_drop_down_menu_buis_states);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_rb_drop_down_menu_buis_states = target.s_rb_drop_down_menu_buis_states.copy();
        target.s_rb_drop_down_menu_buis_states.lp_rb_drop_down_menu_buis_states = value;
    }
}
