...
/** Automaton "Tunnel_Verkeersbuis1_Verkeerslicht1_Stand". */
public final class Aut_Tunnel_Verkeersbuis1_Verkeerslicht1_Stand extends RuntimeSimpleAutomaton<State> {
    public Aut_Tunnel_Verkeersbuis1_Verkeerslicht1_Stand() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_Tunnel_Verkeersbuis1_Verkeerslicht1_Stand", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "Tunnel_Verkeersbuis1_Verkeerslicht1_Stand";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_Tunnel_Verkeersbuis1_Verkeerslicht1_Stand.lp_Tunnel_Verkeersbuis1_Verkeerslicht1_Stand);
    }
    ...
    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis1_Verkeerslicht1_Stand_c_geel(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(635).get(4);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(378);
        fillSyncData(aut, 635, rslt, state.s_Tunnel_Verkeersbuis1_Verkeerslicht1_Stand.lp_Tunnel_Verkeersbuis1_Verkeerslicht1_Stand);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis1_Verkeerslicht1_Stand_c_knipper(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(636).get(4);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(378);
        fillSyncData(aut, 636, rslt, state.s_Tunnel_Verkeersbuis1_Verkeerslicht1_Stand.lp_Tunnel_Verkeersbuis1_Verkeerslicht1_Stand);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis1_Verkeerslicht1_Stand_c_rood(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(637).get(4);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(378);
        fillSyncData(aut, 637, rslt, state.s_Tunnel_Verkeersbuis1_Verkeerslicht1_Stand.lp_Tunnel_Verkeersbuis1_Verkeerslicht1_Stand);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis1_Verkeerslicht1_Stand_c_uit(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(638).get(4);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(378);
        fillSyncData(aut, 638, rslt, state.s_Tunnel_Verkeersbuis1_Verkeerslicht1_Stand.lp_Tunnel_Verkeersbuis1_Verkeerslicht1_Stand);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_Tunnel_Verkeersbuis1_Verkeerslicht1_Stand = target.s_Tunnel_Verkeersbuis1_Verkeerslicht1_Stand.copy();
        target.s_Tunnel_Verkeersbuis1_Verkeerslicht1_Stand.lp_Tunnel_Verkeersbuis1_Verkeerslicht1_Stand = value;
    }
}
