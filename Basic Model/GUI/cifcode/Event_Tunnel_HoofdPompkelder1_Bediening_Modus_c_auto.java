...
/** Event "Tunnel_HoofdPompkelder1_Bediening_Modus_c_auto". */
public final class Event_Tunnel_HoofdPompkelder1_Bediening_Modus_c_auto extends RuntimeEvent<State> {
    public Event_Tunnel_HoofdPompkelder1_Bediening_Modus_c_auto() {
        super("Tunnel_HoofdPompkelder1_Bediening_Modus_c_auto", 186, true);
    }
    ...
    public boolean fillData(State state) {
        ...
        proceed = Aut_sup6.fillSyncData_evt_Tunnel_HoofdPompkelder1_Bediening_Modus_c_auto(state);
        if (!proceed) return false;

        // Check automaton "sup89".
        proceed = Aut_sup89.fillSyncData_evt_Tunnel_HoofdPompkelder1_Bediening_Modus_c_auto(state);
        if (!proceed) return false;

        // Check automaton "sup92".
        proceed = Aut_sup92.fillSyncData_evt_Tunnel_HoofdPompkelder1_Bediening_Modus_c_auto(state);
        if (!proceed) return false;

        // Check automaton "Tunnel_HoofdPompkelder1_Bediening_Modus".
        proceed = Aut_Tunnel_HoofdPompkelder1_Bediening_Modus.fillSyncData_evt_Tunnel_HoofdPompkelder1_Bediening_Modus_c_auto(state);
        if (!proceed) return false;
    ...
    public boolean allowedByInvs(State state) {
        // Invariants for "".
        try {
            if (!(state.s_Tunnel_HoofdPompkelder1_Bediening_KnopAuto_Waarnemer.lp_Tunnel_HoofdPompkelder1_Bediening_KnopAuto_Waarnemer == 1)) return false;
        } catch (CifSimulatorException e) {
            throw new CifSimulatorException("Evaluation of invariant \"Tunnel_HoofdPompkelder1_Bediening_Modus_c_auto needs Tunnel_HoofdPompkelder1_Bediening_KnopAuto_Waarnemer.ingedrukt\" of the top level scope of the specification failed.", e, state);
        }
