...
/** Event "Tunnel_Verkeersbuis1_KnopHerstel_Waarnemer_c_klaar". */
public final class Event_Tunnel_Verkeersbuis1_KnopHerstel_Waarnemer_c_klaar extends RuntimeEvent<State> {
    public Event_Tunnel_Verkeersbuis1_KnopHerstel_Waarnemer_c_klaar() {
        super("Tunnel_Verkeersbuis1_KnopHerstel_Waarnemer_c_klaar", 465, true);
    }
    ...
    public boolean fillData(State state) {
        ...
        proceed = Aut_sup31.fillSyncData_evt_Tunnel_Verkeersbuis1_KnopHerstel_Waarnemer_c_klaar(state);
        if (!proceed) return false;

        // Check automaton "sup35".
        proceed = Aut_sup35.fillSyncData_evt_Tunnel_Verkeersbuis1_KnopHerstel_Waarnemer_c_klaar(state);
        if (!proceed) return false;

        // Check automaton "sup39".
        proceed = Aut_sup39.fillSyncData_evt_Tunnel_Verkeersbuis1_KnopHerstel_Waarnemer_c_klaar(state);
        if (!proceed) return false;

        // Check automaton "sup6".
        proceed = Aut_sup6.fillSyncData_evt_Tunnel_Verkeersbuis1_KnopHerstel_Waarnemer_c_klaar(state);
        if (!proceed) return false;

        // Check automaton "sup7".
        proceed = Aut_sup7.fillSyncData_evt_Tunnel_Verkeersbuis1_KnopHerstel_Waarnemer_c_klaar(state);
        if (!proceed) return false;

        // Check automaton "Tunnel_Verkeersbuis1_KnopHerstel_Waarnemer".
        proceed = Aut_Tunnel_Verkeersbuis1_KnopHerstel_Waarnemer.fillSyncData_evt_Tunnel_Verkeersbuis1_KnopHerstel_Waarnemer_c_klaar(state);
        if (!proceed) return false;
