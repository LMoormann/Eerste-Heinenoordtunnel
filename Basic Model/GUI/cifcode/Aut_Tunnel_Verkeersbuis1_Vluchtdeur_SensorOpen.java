...
/** Automaton "Tunnel_Verkeersbuis1_Vluchtdeur_SensorOpen". */
public final class Aut_Tunnel_Verkeersbuis1_Vluchtdeur_SensorOpen extends RuntimeSimpleAutomaton<State> {
    public Aut_Tunnel_Verkeersbuis1_Vluchtdeur_SensorOpen() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_Tunnel_Verkeersbuis1_Vluchtdeur_SensorOpen", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "Tunnel_Verkeersbuis1_Vluchtdeur_SensorOpen";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_Tunnel_Verkeersbuis1_Vluchtdeur_SensorOpen.lp_Tunnel_Verkeersbuis1_Vluchtdeur_SensorOpen);
    }
    ...
    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis1_Vluchtdeur_SensorOpen_u_aan(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(704).get(1);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(410);
        fillSyncData(aut, 704, rslt, state.s_Tunnel_Verkeersbuis1_Vluchtdeur_SensorOpen.lp_Tunnel_Verkeersbuis1_Vluchtdeur_SensorOpen);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis1_Vluchtdeur_SensorOpen_u_uit(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(705).get(1);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(410);
        fillSyncData(aut, 705, rslt, state.s_Tunnel_Verkeersbuis1_Vluchtdeur_SensorOpen.lp_Tunnel_Verkeersbuis1_Vluchtdeur_SensorOpen);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_Tunnel_Verkeersbuis1_Vluchtdeur_SensorOpen = target.s_Tunnel_Verkeersbuis1_Vluchtdeur_SensorOpen.copy();
        target.s_Tunnel_Verkeersbuis1_Vluchtdeur_SensorOpen.lp_Tunnel_Verkeersbuis1_Vluchtdeur_SensorOpen = value;
    }
}
