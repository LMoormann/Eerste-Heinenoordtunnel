...
/** Automaton "Tunnel_Verkeersbuis1_WisselTimerAudio". */
public final class Aut_Tunnel_Verkeersbuis1_WisselTimerAudio extends RuntimeSimpleAutomaton<State> {
    public Aut_Tunnel_Verkeersbuis1_WisselTimerAudio() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_Tunnel_Verkeersbuis1_WisselTimerAudio", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "Tunnel_Verkeersbuis1_WisselTimerAudio";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_Tunnel_Verkeersbuis1_WisselTimerAudio.lp_Tunnel_Verkeersbuis1_WisselTimerAudio);
    }
    ...
    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis1_WisselTimerAudio_c_aan(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(706).get(3);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(411);
        fillSyncData(aut, 706, rslt, state.s_Tunnel_Verkeersbuis1_WisselTimerAudio.lp_Tunnel_Verkeersbuis1_WisselTimerAudio);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis1_WisselTimerAudio_c_uit(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(707).get(3);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(411);
        fillSyncData(aut, 707, rslt, state.s_Tunnel_Verkeersbuis1_WisselTimerAudio.lp_Tunnel_Verkeersbuis1_WisselTimerAudio);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis1_WisselTimerAudio_u_timeout(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(708).get(1);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(411);
        fillSyncData(aut, 708, rslt, state.s_Tunnel_Verkeersbuis1_WisselTimerAudio.lp_Tunnel_Verkeersbuis1_WisselTimerAudio);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_Tunnel_Verkeersbuis1_WisselTimerAudio = target.s_Tunnel_Verkeersbuis1_WisselTimerAudio.copy();
        target.s_Tunnel_Verkeersbuis1_WisselTimerAudio.lp_Tunnel_Verkeersbuis1_WisselTimerAudio = value;
    }
}
