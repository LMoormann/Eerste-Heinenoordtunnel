...
/** Automaton "lb_overzicht_systemen_en_detecties_hulppost". */
public final class Aut_lb_overzicht_systemen_en_detecties_hulppost extends RuntimeSimpleAutomaton<State> {
    public Aut_lb_overzicht_systemen_en_detecties_hulppost() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_lb_overzicht_systemen_en_detecties_hulppost", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "lb_overzicht_systemen_en_detecties_hulppost";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_lb_overzicht_systemen_en_detecties_hulppost.lp_lb_overzicht_systemen_en_detecties_hulppost);
    }
    ...
    public static boolean fillSyncData_evt_lb_overzicht_systemen_en_detecties_hulppost_open_drop_down_menu(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(39).get(0);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(17);
        fillSyncData(aut, 39, rslt, state.s_lb_overzicht_systemen_en_detecties_hulppost.lp_lb_overzicht_systemen_en_detecties_hulppost);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_lb_overzicht_systemen_en_detecties_hulppost_sluit_drop_down_menu(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(40).get(0);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(17);
        fillSyncData(aut, 40, rslt, state.s_lb_overzicht_systemen_en_detecties_hulppost.lp_lb_overzicht_systemen_en_detecties_hulppost);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_sluit_alle_tabs(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(157).get(13);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(17);
        fillSyncData(aut, 157, rslt, state.s_lb_overzicht_systemen_en_detecties_hulppost.lp_lb_overzicht_systemen_en_detecties_hulppost);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_lb_overzicht_systemen_en_detecties_hulppost = target.s_lb_overzicht_systemen_en_detecties_hulppost.copy();
        target.s_lb_overzicht_systemen_en_detecties_hulppost.lp_lb_overzicht_systemen_en_detecties_hulppost = value;
    }
}
