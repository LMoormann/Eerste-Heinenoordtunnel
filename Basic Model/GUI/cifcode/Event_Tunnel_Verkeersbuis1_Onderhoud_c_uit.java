...
/** Event "Tunnel_Verkeersbuis1_Onderhoud_c_uit". */
public final class Event_Tunnel_Verkeersbuis1_Onderhoud_c_uit extends RuntimeEvent<State> {
    public Event_Tunnel_Verkeersbuis1_Onderhoud_c_uit() {
        super("Tunnel_Verkeersbuis1_Onderhoud_c_uit", 584, true);
    }
    ...
    public boolean fillData(State state) {
        ...
        proceed = Aut_sup31.fillSyncData_evt_Tunnel_Verkeersbuis1_Onderhoud_c_uit(state);
        if (!proceed) return false;

        // Check automaton "sup35".
        proceed = Aut_sup35.fillSyncData_evt_Tunnel_Verkeersbuis1_Onderhoud_c_uit(state);
        if (!proceed) return false;

        // Check automaton "sup39".
        proceed = Aut_sup39.fillSyncData_evt_Tunnel_Verkeersbuis1_Onderhoud_c_uit(state);
        if (!proceed) return false;

        // Check automaton "sup6".
        proceed = Aut_sup6.fillSyncData_evt_Tunnel_Verkeersbuis1_Onderhoud_c_uit(state);
        if (!proceed) return false;

        // Check automaton "sup7".
        proceed = Aut_sup7.fillSyncData_evt_Tunnel_Verkeersbuis1_Onderhoud_c_uit(state);
        if (!proceed) return false;

        // Check automaton "Tunnel_Verkeersbuis1_Onderhoud".
        proceed = Aut_Tunnel_Verkeersbuis1_Onderhoud.fillSyncData_evt_Tunnel_Verkeersbuis1_Onderhoud_c_uit(state);
        if (!proceed) return false;
    ...
    public boolean allowedByInvs(State state) {
        // Invariants for "".
        try {
            if (!((state.s_Tunnel_Verkeersbuis1_KnopOperationeel_Waarnemer.lp_Tunnel_Verkeersbuis1_KnopOperationeel_Waarnemer == 1) || (!(state.s_Tunnel_Verkeersbuis1_StandVerkeersbuis.lp_Tunnel_Verkeersbuis1_StandVerkeersbuis == 0)))) return false;
        } catch (CifSimulatorException e) {
            throw new CifSimulatorException("Evaluation of invariant \"Tunnel_Verkeersbuis1_Onderhoud_c_uit needs Tunnel_Verkeersbuis1_KnopOperationeel_Waarnemer.ingedrukt or not Tunnel_Verkeersbuis1_StandVerkeersbuis.operationeel\" of the top level scope of the specification failed.", e, state);
        }
