...
/** Automaton "Tunnel_Verkeersbuis1_Recorder". */
public final class Aut_Tunnel_Verkeersbuis1_Recorder extends RuntimeSimpleAutomaton<State> {
    public Aut_Tunnel_Verkeersbuis1_Recorder() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_Tunnel_Verkeersbuis1_Recorder", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "Tunnel_Verkeersbuis1_Recorder";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_Tunnel_Verkeersbuis1_Recorder.lp_Tunnel_Verkeersbuis1_Recorder);
    }
    ...
    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis1_Recorder_c_aan(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(596).get(1);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(369);
        fillSyncData(aut, 596, rslt, state.s_Tunnel_Verkeersbuis1_Recorder.lp_Tunnel_Verkeersbuis1_Recorder);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis1_Recorder_c_uit(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(597).get(1);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(369);
        fillSyncData(aut, 597, rslt, state.s_Tunnel_Verkeersbuis1_Recorder.lp_Tunnel_Verkeersbuis1_Recorder);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_Tunnel_Verkeersbuis1_Recorder = target.s_Tunnel_Verkeersbuis1_Recorder.copy();
        target.s_Tunnel_Verkeersbuis1_Recorder.lp_Tunnel_Verkeersbuis1_Recorder = value;
    }
}
