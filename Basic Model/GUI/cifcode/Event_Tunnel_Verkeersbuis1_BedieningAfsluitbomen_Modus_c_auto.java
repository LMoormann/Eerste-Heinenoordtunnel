...
/** Event "Tunnel_Verkeersbuis1_BedieningAfsluitbomen_Modus_c_auto". */
public final class Event_Tunnel_Verkeersbuis1_BedieningAfsluitbomen_Modus_c_auto extends RuntimeEvent<State> {
    public Event_Tunnel_Verkeersbuis1_BedieningAfsluitbomen_Modus_c_auto() {
        super("Tunnel_Verkeersbuis1_BedieningAfsluitbomen_Modus_c_auto", 381, true);
    }
    ...
    public boolean fillData(State state) {
        ...
        proceed = Aut_sup10.fillSyncData_evt_Tunnel_Verkeersbuis1_BedieningAfsluitbomen_Modus_c_auto(state);
        if (!proceed) return false;

        // Check automaton "sup7".
        proceed = Aut_sup7.fillSyncData_evt_Tunnel_Verkeersbuis1_BedieningAfsluitbomen_Modus_c_auto(state);
        if (!proceed) return false;

        // Check automaton "sup8".
        proceed = Aut_sup8.fillSyncData_evt_Tunnel_Verkeersbuis1_BedieningAfsluitbomen_Modus_c_auto(state);
        if (!proceed) return false;

        // Check automaton "sup9".
        proceed = Aut_sup9.fillSyncData_evt_Tunnel_Verkeersbuis1_BedieningAfsluitbomen_Modus_c_auto(state);
        if (!proceed) return false;

        // Check automaton "Tunnel_Verkeersbuis1_BedieningAfsluitbomen_Modus".
        proceed = Aut_Tunnel_Verkeersbuis1_BedieningAfsluitbomen_Modus.fillSyncData_evt_Tunnel_Verkeersbuis1_BedieningAfsluitbomen_Modus_c_auto(state);
        if (!proceed) return false;
    ...
    public boolean allowedByInvs(State state) {
        // Invariants for "".
        try {
            if (!(state.s_Tunnel_Verkeersbuis1_BedieningAfsluitbomen_KnopAuto_Waarnemer.lp_Tunnel_Verkeersbuis1_BedieningAfsluitbomen_KnopAuto_Waarnemer == 1)) return false;
        } catch (CifSimulatorException e) {
            throw new CifSimulatorException("Evaluation of invariant \"Tunnel_Verkeersbuis1_BedieningAfsluitbomen_Modus_c_auto needs Tunnel_Verkeersbuis1_BedieningAfsluitbomen_KnopAuto_Waarnemer.ingedrukt\" of the top level scope of the specification failed.", e, state);
        }
