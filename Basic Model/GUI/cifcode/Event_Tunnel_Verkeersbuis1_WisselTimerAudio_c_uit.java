...
/** Event "Tunnel_Verkeersbuis1_WisselTimerAudio_c_uit". */
public final class Event_Tunnel_Verkeersbuis1_WisselTimerAudio_c_uit extends RuntimeEvent<State> {
    public Event_Tunnel_Verkeersbuis1_WisselTimerAudio_c_uit() {
        super("Tunnel_Verkeersbuis1_WisselTimerAudio_c_uit", 707, true);
    }
    ...
    public boolean fillData(State state) {
        ...
        proceed = Aut_sup6.fillSyncData_evt_Tunnel_Verkeersbuis1_WisselTimerAudio_c_uit(state);
        if (!proceed) return false;

        // Check automaton "sup82".
        proceed = Aut_sup82.fillSyncData_evt_Tunnel_Verkeersbuis1_WisselTimerAudio_c_uit(state);
        if (!proceed) return false;

        // Check automaton "sup83".
        proceed = Aut_sup83.fillSyncData_evt_Tunnel_Verkeersbuis1_WisselTimerAudio_c_uit(state);
        if (!proceed) return false;

        // Check automaton "Tunnel_Verkeersbuis1_WisselTimerAudio".
        proceed = Aut_Tunnel_Verkeersbuis1_WisselTimerAudio.fillSyncData_evt_Tunnel_Verkeersbuis1_WisselTimerAudio_c_uit(state);
        if (!proceed) return false;
    ...
    public boolean allowedByInvs(State state) {
        // Invariants for "".
        try {
            if (!(((state.s_Tunnel_Verkeersbuis1_Evacuatie.lp_Tunnel_Verkeersbuis1_Evacuatie == 0) || (((!(state.s_Tunnel_Verkeersbuis1_Luidspreker.lp_Tunnel_Verkeersbuis1_Luidspreker == 0)) && (state.s_Tunnel_Verkeersbuis1_Vluchtdeur_BedieningGeluidsbaken_Modus.lp_Tunnel_Verkeersbuis1_Vluchtdeur_BedieningGeluidsbaken_Modus == 1)) && (state.s_Tunnel_Verkeersbuis1_Vluchtdeur_KnopGeluidsbakenUit_Waarnemer.lp_Tunnel_Verkeersbuis1_Vluchtdeur_KnopGeluidsbakenUit_Waarnemer == 1))) || ((!(state.s_Tunnel_Verkeersbuis1_Vluchtdeur_Geluidsbaken.lp_Tunnel_Verkeersbuis1_Vluchtdeur_Geluidsbaken == 0)) && ((state.s_Tunnel_Verkeersbuis1_KnopStopLive_Waarnemer.lp_Tunnel_Verkeersbuis1_KnopStopLive_Waarnemer == 1) || (state.s_Tunnel_Verkeersbuis1_KnopStopBoodschap_Waarnemer.lp_Tunnel_Verkeersbuis1_KnopStopBoodschap_Waarnemer == 1))))) return false;
        } catch (CifSimulatorException e) {
            throw new CifSimulatorException("Evaluation of invariant \"Tunnel_Verkeersbuis1_WisselTimerAudio_c_uit needs Tunnel_Verkeersbuis1_Evacuatie.uit or not Tunnel_Verkeersbuis1_Luidspreker.uit and Tunnel_Verkeersbuis1_Vluchtdeur_BedieningGeluidsbaken_Modus.hand and Tunnel_Verkeersbuis1_Vluchtdeur_KnopGeluidsbakenUit_Waarnemer.ingedrukt or not Tunnel_Verkeersbuis1_Vluchtdeur_Geluidsbaken.uit and (Tunnel_Verkeersbuis1_KnopStopLive_Waarnemer.ingedrukt or Tunnel_Verkeersbuis1_KnopStopBoodschap_Waarnemer.ingedrukt)\" of the top level scope of the specification failed.", e, state);
        }
