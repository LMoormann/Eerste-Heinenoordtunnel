...
/** Event "Tunnel_MiddenPompkelder_SensorMaxstartwater_u_aan". */
public final class Event_Tunnel_MiddenPompkelder_SensorMaxstartwater_u_aan extends RuntimeEvent<State> {
    public Event_Tunnel_MiddenPompkelder_SensorMaxstartwater_u_aan() {
        super("Tunnel_MiddenPompkelder_SensorMaxstartwater_u_aan", 280, false);
    }
    ...
    public boolean fillData(State state) {
        ...
        proceed = Aut_event_disabler.fillSyncData_evt_Tunnel_MiddenPompkelder_SensorMaxstartwater_u_aan(state);
        if (!proceed) return false;

        // Check automaton "Tunnel_MiddenPompkelder_FysiekeRelatie".
        proceed = Aut_Tunnel_MiddenPompkelder_FysiekeRelatie.fillSyncData_evt_Tunnel_MiddenPompkelder_SensorMaxstartwater_u_aan(state);
        if (!proceed) return false;

        // Check automaton "Tunnel_MiddenPompkelder_SensorMaxstartwater".
        proceed = Aut_Tunnel_MiddenPompkelder_SensorMaxstartwater.fillSyncData_evt_Tunnel_MiddenPompkelder_SensorMaxstartwater_u_aan(state);
        if (!proceed) return false;
