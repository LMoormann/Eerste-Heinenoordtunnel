...
/** Event "Tunnel_MiddenPompkelder_Bediening_Modus_c_hand". */
public final class Event_Tunnel_MiddenPompkelder_Bediening_Modus_c_hand extends RuntimeEvent<State> {
    public Event_Tunnel_MiddenPompkelder_Bediening_Modus_c_hand() {
        super("Tunnel_MiddenPompkelder_Bediening_Modus_c_hand", 255, true);
    }
    ...
    public boolean fillData(State state) {
        ...
        proceed = Aut_sup6.fillSyncData_evt_Tunnel_MiddenPompkelder_Bediening_Modus_c_hand(state);
        if (!proceed) return false;

        // Check automaton "sup86".
        proceed = Aut_sup86.fillSyncData_evt_Tunnel_MiddenPompkelder_Bediening_Modus_c_hand(state);
        if (!proceed) return false;

        // Check automaton "sup88".
        proceed = Aut_sup88.fillSyncData_evt_Tunnel_MiddenPompkelder_Bediening_Modus_c_hand(state);
        if (!proceed) return false;

        // Check automaton "Tunnel_MiddenPompkelder_Bediening_Modus".
        proceed = Aut_Tunnel_MiddenPompkelder_Bediening_Modus.fillSyncData_evt_Tunnel_MiddenPompkelder_Bediening_Modus_c_hand(state);
        if (!proceed) return false;
    ...
    public boolean allowedByInvs(State state) {
        // Invariants for "".
        try {
            if (!(state.s_Tunnel_MiddenPompkelder_Bediening_KnopHand_Waarnemer.lp_Tunnel_MiddenPompkelder_Bediening_KnopHand_Waarnemer == 1)) return false;
        } catch (CifSimulatorException e) {
            throw new CifSimulatorException("Evaluation of invariant \"Tunnel_MiddenPompkelder_Bediening_Modus_c_hand needs Tunnel_MiddenPompkelder_Bediening_KnopHand_Waarnemer.ingedrukt\" of the top level scope of the specification failed.", e, state);
        }
