...
/** Event "Tunnel_MiddenTunnelKanaal_BedieningDynamischeVluchtrouteIndicatie_Modus_c_hand". */
public final class Event_Tunnel_MiddenTunnelKanaal_BedieningDynamischeVluchtrouteIndicatie_Modus_c_hand extends RuntimeEvent<State> {
    public Event_Tunnel_MiddenTunnelKanaal_BedieningDynamischeVluchtrouteIndicatie_Modus_c_hand() {
        super("Tunnel_MiddenTunnelKanaal_BedieningDynamischeVluchtrouteIndicatie_Modus_c_hand", 291, true);
    }
    ...
    public boolean fillData(State state) {
        ...
        proceed = Aut_sup35.fillSyncData_evt_Tunnel_MiddenTunnelKanaal_BedieningDynamischeVluchtrouteIndicatie_Modus_c_hand(state);
        if (!proceed) return false;

        // Check automaton "sup43".
        proceed = Aut_sup43.fillSyncData_evt_Tunnel_MiddenTunnelKanaal_BedieningDynamischeVluchtrouteIndicatie_Modus_c_hand(state);
        if (!proceed) return false;

        // Check automaton "sup6".
        proceed = Aut_sup6.fillSyncData_evt_Tunnel_MiddenTunnelKanaal_BedieningDynamischeVluchtrouteIndicatie_Modus_c_hand(state);
        if (!proceed) return false;

        // Check automaton "Tunnel_MiddenTunnelKanaal_BedieningDynamischeVluchtrouteIndicatie_Modus".
        proceed = Aut_Tunnel_MiddenTunnelKanaal_BedieningDynamischeVluchtrouteIndicatie_Modus.fillSyncData_evt_Tunnel_MiddenTunnelKanaal_BedieningDynamischeVluchtrouteIndicatie_Modus_c_hand(state);
        if (!proceed) return false;
    ...
    public boolean allowedByInvs(State state) {
        // Invariants for "".
        try {
            if (!(state.s_Tunnel_MiddenTunnelKanaal_BedieningDynamischeVluchtrouteIndicatie_KnopHand_Waarnemer.lp_Tunnel_MiddenTunnelKanaal_BedieningDynamischeVluchtrouteIndicatie_KnopHand_Waarnemer == 1)) return false;
        } catch (CifSimulatorException e) {
            throw new CifSimulatorException("Evaluation of invariant \"Tunnel_MiddenTunnelKanaal_BedieningDynamischeVluchtrouteIndicatie_Modus_c_hand needs Tunnel_MiddenTunnelKanaal_BedieningDynamischeVluchtrouteIndicatie_KnopHand_Waarnemer.ingedrukt\" of the top level scope of the specification failed.", e, state);
        }
