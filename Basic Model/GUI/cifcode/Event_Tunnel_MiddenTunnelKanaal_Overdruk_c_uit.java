...
/** Event "Tunnel_MiddenTunnelKanaal_Overdruk_c_uit". */
public final class Event_Tunnel_MiddenTunnelKanaal_Overdruk_c_uit extends RuntimeEvent<State> {
    public Event_Tunnel_MiddenTunnelKanaal_Overdruk_c_uit() {
        super("Tunnel_MiddenTunnelKanaal_Overdruk_c_uit", 348, true);
    }
    ...
    public boolean fillData(State state) {
        ...
        proceed = Aut_sup35.fillSyncData_evt_Tunnel_MiddenTunnelKanaal_Overdruk_c_uit(state);
        if (!proceed) return false;

        // Check automaton "sup41".
        proceed = Aut_sup41.fillSyncData_evt_Tunnel_MiddenTunnelKanaal_Overdruk_c_uit(state);
        if (!proceed) return false;

        // Check automaton "sup6".
        proceed = Aut_sup6.fillSyncData_evt_Tunnel_MiddenTunnelKanaal_Overdruk_c_uit(state);
        if (!proceed) return false;

        // Check automaton "Tunnel_MiddenTunnelKanaal_Overdruk".
        proceed = Aut_Tunnel_MiddenTunnelKanaal_Overdruk.fillSyncData_evt_Tunnel_MiddenTunnelKanaal_Overdruk_c_uit(state);
        if (!proceed) return false;
    ...
    public boolean allowedByInvs(State state) {
        // Invariants for "".
        try {
            if (!((((((state.s_Tunnel_MiddenTunnelKanaal_BedieningOverdruk_Modus.lp_Tunnel_MiddenTunnelKanaal_BedieningOverdruk_Modus == 0) && (state.s_Tunnel_Verkeersbuis1_StandVerkeersbuis.lp_Tunnel_Verkeersbuis1_StandVerkeersbuis == 0)) && (state.s_Tunnel_Verkeersbuis2_StandVerkeersbuis.lp_Tunnel_Verkeersbuis2_StandVerkeersbuis == 0)) && (state.s_Tunnel_Verkeersbuis1_Standby.lp_Tunnel_Verkeersbuis1_Standby == 0)) && (state.s_Tunnel_Verkeersbuis2_Standby.lp_Tunnel_Verkeersbuis2_Standby == 0)) || ((state.s_Tunnel_MiddenTunnelKanaal_BedieningOverdruk_Modus.lp_Tunnel_MiddenTunnelKanaal_BedieningOverdruk_Modus == 1) && (state.s_Tunnel_MiddenTunnelKanaal_KnopOverdrukUit_Waarnemer.lp_Tunnel_MiddenTunnelKanaal_KnopOverdrukUit_Waarnemer == 1)))) return false;
        } catch (CifSimulatorException e) {
            throw new CifSimulatorException("Evaluation of invariant \"Tunnel_MiddenTunnelKanaal_Overdruk_c_uit needs Tunnel_MiddenTunnelKanaal_BedieningOverdruk_Modus.auto and Tunnel_Verkeersbuis1_StandVerkeersbuis.operationeel and Tunnel_Verkeersbuis2_StandVerkeersbuis.operationeel and Tunnel_Verkeersbuis1_Standby.uit and Tunnel_Verkeersbuis2_Standby.uit or Tunnel_MiddenTunnelKanaal_BedieningOverdruk_Modus.hand and Tunnel_MiddenTunnelKanaal_KnopOverdrukUit_Waarnemer.ingedrukt\" of the top level scope of the specification failed.", e, state);
        }
