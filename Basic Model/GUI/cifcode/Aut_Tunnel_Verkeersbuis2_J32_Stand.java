...
/** Automaton "Tunnel_Verkeersbuis2_J32_Stand". */
public final class Aut_Tunnel_Verkeersbuis2_J32_Stand extends RuntimeSimpleAutomaton<State> {
    public Aut_Tunnel_Verkeersbuis2_J32_Stand() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_Tunnel_Verkeersbuis2_J32_Stand", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "Tunnel_Verkeersbuis2_J32_Stand";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_Tunnel_Verkeersbuis2_J32_Stand.lp_Tunnel_Verkeersbuis2_J32_Stand);
    }
    ...
    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis2_J32_Stand_c_aan(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(781).get(3);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(452);
        fillSyncData(aut, 781, rslt, state.s_Tunnel_Verkeersbuis2_J32_Stand.lp_Tunnel_Verkeersbuis2_J32_Stand);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis2_J32_Stand_c_uit(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(782).get(3);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(452);
        fillSyncData(aut, 782, rslt, state.s_Tunnel_Verkeersbuis2_J32_Stand.lp_Tunnel_Verkeersbuis2_J32_Stand);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_Tunnel_Verkeersbuis2_J32_Stand = target.s_Tunnel_Verkeersbuis2_J32_Stand.copy();
        target.s_Tunnel_Verkeersbuis2_J32_Stand.lp_Tunnel_Verkeersbuis2_J32_Stand = value;
    }
}
