...
/** Automaton "rb_overzicht_systemen_en_detecties_vluchtroute". */
public final class Aut_rb_overzicht_systemen_en_detecties_vluchtroute extends RuntimeSimpleAutomaton<State> {
    public Aut_rb_overzicht_systemen_en_detecties_vluchtroute() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_rb_overzicht_systemen_en_detecties_vluchtroute", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "rb_overzicht_systemen_en_detecties_vluchtroute";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_rb_overzicht_systemen_en_detecties_vluchtroute.lp_rb_overzicht_systemen_en_detecties_vluchtroute);
    }
    ...
    public static boolean fillSyncData_evt_rb_overzicht_systemen_en_detecties_vluchtroute_open_drop_down_menu(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(153).get(0);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(74);
        fillSyncData(aut, 153, rslt, state.s_rb_overzicht_systemen_en_detecties_vluchtroute.lp_rb_overzicht_systemen_en_detecties_vluchtroute);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_rb_overzicht_systemen_en_detecties_vluchtroute_sluit_drop_down_menu(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(154).get(0);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(74);
        fillSyncData(aut, 154, rslt, state.s_rb_overzicht_systemen_en_detecties_vluchtroute.lp_rb_overzicht_systemen_en_detecties_vluchtroute);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_sluit_alle_tabs(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(157).get(70);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(74);
        fillSyncData(aut, 157, rslt, state.s_rb_overzicht_systemen_en_detecties_vluchtroute.lp_rb_overzicht_systemen_en_detecties_vluchtroute);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_rb_overzicht_systemen_en_detecties_vluchtroute = target.s_rb_overzicht_systemen_en_detecties_vluchtroute.copy();
        target.s_rb_overzicht_systemen_en_detecties_vluchtroute.lp_rb_overzicht_systemen_en_detecties_vluchtroute = value;
    }
}
