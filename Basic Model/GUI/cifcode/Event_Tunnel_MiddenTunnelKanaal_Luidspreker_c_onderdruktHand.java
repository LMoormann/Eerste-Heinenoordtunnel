...
/** Event "Tunnel_MiddenTunnelKanaal_Luidspreker_c_onderdruktHand". */
public final class Event_Tunnel_MiddenTunnelKanaal_Luidspreker_c_onderdruktHand extends RuntimeEvent<State> {
    public Event_Tunnel_MiddenTunnelKanaal_Luidspreker_c_onderdruktHand() {
        super("Tunnel_MiddenTunnelKanaal_Luidspreker_c_onderdruktHand", 344, true);
    }
    ...
    public boolean fillData(State state) {
        ...
        proceed = Aut_sup6.fillSyncData_evt_Tunnel_MiddenTunnelKanaal_Luidspreker_c_onderdruktHand(state);
        if (!proceed) return false;

        // Check automaton "sup82".
        proceed = Aut_sup82.fillSyncData_evt_Tunnel_MiddenTunnelKanaal_Luidspreker_c_onderdruktHand(state);
        if (!proceed) return false;

        // Check automaton "sup85".
        proceed = Aut_sup85.fillSyncData_evt_Tunnel_MiddenTunnelKanaal_Luidspreker_c_onderdruktHand(state);
        if (!proceed) return false;

        // Check automaton "Tunnel_MiddenTunnelKanaal_Luidspreker".
        proceed = Aut_Tunnel_MiddenTunnelKanaal_Luidspreker.fillSyncData_evt_Tunnel_MiddenTunnelKanaal_Luidspreker_c_onderdruktHand(state);
        if (!proceed) return false;
    ...
    public boolean allowedByInvs(State state) {
        // Invariants for "".
        try {
            if (!(state.s_Tunnel_MiddenTunnelKanaal_KnopStopBoodschap_Waarnemer.lp_Tunnel_MiddenTunnelKanaal_KnopStopBoodschap_Waarnemer == 1)) return false;
        } catch (CifSimulatorException e) {
            throw new CifSimulatorException("Evaluation of invariant \"Tunnel_MiddenTunnelKanaal_Luidspreker_c_onderdruktHand needs Tunnel_MiddenTunnelKanaal_KnopStopBoodschap_Waarnemer.ingedrukt\" of the top level scope of the specification failed.", e, state);
        }
