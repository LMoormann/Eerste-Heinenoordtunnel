...
/** Automaton "Tunnel_MiddenPompkelder_Bediening_Modus". */
public final class Aut_Tunnel_MiddenPompkelder_Bediening_Modus extends RuntimeSimpleAutomaton<State> {
    public Aut_Tunnel_MiddenPompkelder_Bediening_Modus() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_Tunnel_MiddenPompkelder_Bediening_Modus", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "Tunnel_MiddenPompkelder_Bediening_Modus";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_Tunnel_MiddenPompkelder_Bediening_Modus.lp_Tunnel_MiddenPompkelder_Bediening_Modus);
    }
    ...
    public static boolean fillSyncData_evt_Tunnel_MiddenPompkelder_Bediening_Modus_c_auto(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(254).get(3);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(174);
        fillSyncData(aut, 254, rslt, state.s_Tunnel_MiddenPompkelder_Bediening_Modus.lp_Tunnel_MiddenPompkelder_Bediening_Modus);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_MiddenPompkelder_Bediening_Modus_c_hand(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(255).get(3);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(174);
        fillSyncData(aut, 255, rslt, state.s_Tunnel_MiddenPompkelder_Bediening_Modus.lp_Tunnel_MiddenPompkelder_Bediening_Modus);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_Tunnel_MiddenPompkelder_Bediening_Modus = target.s_Tunnel_MiddenPompkelder_Bediening_Modus.copy();
        target.s_Tunnel_MiddenPompkelder_Bediening_Modus.lp_Tunnel_MiddenPompkelder_Bediening_Modus = value;
    }
}
