...
/** Automaton "overzicht_systemen_en_detecties_drop_down_midden_pomp_kelder_stand_automaton". */
public final class Aut_overzicht_systemen_en_detecties_drop_down_midden_pomp_kelder_stand_automaton extends RuntimeSimpleAutomaton<State> {
    public Aut_overzicht_systemen_en_detecties_drop_down_midden_pomp_kelder_stand_automaton() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_overzicht_systemen_en_detecties_drop_down_midden_pomp_kelder_stand_automaton", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "overzicht_systemen_en_detecties_drop_down_midden_pomp_kelder_stand_automaton";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_overzicht_systemen_en_detecties_drop_down_midden_pomp_kelder_stand_automaton.lp_overzicht_systemen_en_detecties_drop_down_midden_pomp_kelder_stand_automaton);
    }
    ...
    public static boolean fillSyncData_evt_overzicht_systemen_en_detecties_drop_down_midden_pomp_kelder_stand_automaton_open_drop_down_menu(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(89).get(0);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(42);
        fillSyncData(aut, 89, rslt, state.s_overzicht_systemen_en_detecties_drop_down_midden_pomp_kelder_stand_automaton.lp_overzicht_systemen_en_detecties_drop_down_midden_pomp_kelder_stand_automaton);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_overzicht_systemen_en_detecties_drop_down_midden_pomp_kelder_stand_automaton_sluit_drop_down_menu(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(90).get(0);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(42);
        fillSyncData(aut, 90, rslt, state.s_overzicht_systemen_en_detecties_drop_down_midden_pomp_kelder_stand_automaton.lp_overzicht_systemen_en_detecties_drop_down_midden_pomp_kelder_stand_automaton);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_sluit_alle_tabs(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(157).get(38);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(42);
        fillSyncData(aut, 157, rslt, state.s_overzicht_systemen_en_detecties_drop_down_midden_pomp_kelder_stand_automaton.lp_overzicht_systemen_en_detecties_drop_down_midden_pomp_kelder_stand_automaton);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_overzicht_systemen_en_detecties_drop_down_midden_pomp_kelder_stand_automaton = target.s_overzicht_systemen_en_detecties_drop_down_midden_pomp_kelder_stand_automaton.copy();
        target.s_overzicht_systemen_en_detecties_drop_down_midden_pomp_kelder_stand_automaton.lp_overzicht_systemen_en_detecties_drop_down_midden_pomp_kelder_stand_automaton = value;
    }
}
