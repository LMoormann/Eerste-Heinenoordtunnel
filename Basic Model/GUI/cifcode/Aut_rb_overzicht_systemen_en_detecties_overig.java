...
/** Automaton "rb_overzicht_systemen_en_detecties_overig". */
public final class Aut_rb_overzicht_systemen_en_detecties_overig extends RuntimeSimpleAutomaton<State> {
    public Aut_rb_overzicht_systemen_en_detecties_overig() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_rb_overzicht_systemen_en_detecties_overig", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "rb_overzicht_systemen_en_detecties_overig";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_rb_overzicht_systemen_en_detecties_overig.lp_rb_overzicht_systemen_en_detecties_overig);
    }
    ...
    public static boolean fillSyncData_evt_rb_overzicht_systemen_en_detecties_overig_open_drop_down_menu(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(123).get(0);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(59);
        fillSyncData(aut, 123, rslt, state.s_rb_overzicht_systemen_en_detecties_overig.lp_rb_overzicht_systemen_en_detecties_overig);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_rb_overzicht_systemen_en_detecties_overig_sluit_drop_down_menu(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(124).get(0);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(59);
        fillSyncData(aut, 124, rslt, state.s_rb_overzicht_systemen_en_detecties_overig.lp_rb_overzicht_systemen_en_detecties_overig);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_sluit_alle_tabs(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(157).get(55);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(59);
        fillSyncData(aut, 157, rslt, state.s_rb_overzicht_systemen_en_detecties_overig.lp_rb_overzicht_systemen_en_detecties_overig);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_rb_overzicht_systemen_en_detecties_overig = target.s_rb_overzicht_systemen_en_detecties_overig.copy();
        target.s_rb_overzicht_systemen_en_detecties_overig.lp_rb_overzicht_systemen_en_detecties_overig = value;
    }
}
