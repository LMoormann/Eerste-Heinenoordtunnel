...
/** Automaton "rb_overzicht_systemen_en_detecties_verlichting". */
public final class Aut_rb_overzicht_systemen_en_detecties_verlichting extends RuntimeSimpleAutomaton<State> {
    public Aut_rb_overzicht_systemen_en_detecties_verlichting() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_rb_overzicht_systemen_en_detecties_verlichting", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "rb_overzicht_systemen_en_detecties_verlichting";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_rb_overzicht_systemen_en_detecties_verlichting.lp_rb_overzicht_systemen_en_detecties_verlichting);
    }
    ...
    public static boolean fillSyncData_evt_rb_overzicht_systemen_en_detecties_verlichting_open_drop_down_menu(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(135).get(0);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(65);
        fillSyncData(aut, 135, rslt, state.s_rb_overzicht_systemen_en_detecties_verlichting.lp_rb_overzicht_systemen_en_detecties_verlichting);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_rb_overzicht_systemen_en_detecties_verlichting_sluit_drop_down_menu(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(136).get(0);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(65);
        fillSyncData(aut, 136, rslt, state.s_rb_overzicht_systemen_en_detecties_verlichting.lp_rb_overzicht_systemen_en_detecties_verlichting);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_sluit_alle_tabs(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(157).get(61);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(65);
        fillSyncData(aut, 157, rslt, state.s_rb_overzicht_systemen_en_detecties_verlichting.lp_rb_overzicht_systemen_en_detecties_verlichting);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_rb_overzicht_systemen_en_detecties_verlichting = target.s_rb_overzicht_systemen_en_detecties_verlichting.copy();
        target.s_rb_overzicht_systemen_en_detecties_verlichting.lp_rb_overzicht_systemen_en_detecties_verlichting = value;
    }
}
