...
/** Automaton "Tunnel_Verkeersbuis1_Vluchtdeur_ContourVerlichting". */
public final class Aut_Tunnel_Verkeersbuis1_Vluchtdeur_ContourVerlichting extends RuntimeSimpleAutomaton<State> {
    public Aut_Tunnel_Verkeersbuis1_Vluchtdeur_ContourVerlichting() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_Tunnel_Verkeersbuis1_Vluchtdeur_ContourVerlichting", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "Tunnel_Verkeersbuis1_Vluchtdeur_ContourVerlichting";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_Tunnel_Verkeersbuis1_Vluchtdeur_ContourVerlichting.lp_Tunnel_Verkeersbuis1_Vluchtdeur_ContourVerlichting);
    }
    ...
    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis1_Vluchtdeur_ContourVerlichting_c_aan(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(687).get(2);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(400);
        fillSyncData(aut, 687, rslt, state.s_Tunnel_Verkeersbuis1_Vluchtdeur_ContourVerlichting.lp_Tunnel_Verkeersbuis1_Vluchtdeur_ContourVerlichting);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis1_Vluchtdeur_ContourVerlichting_c_uit(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(688).get(2);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(400);
        fillSyncData(aut, 688, rslt, state.s_Tunnel_Verkeersbuis1_Vluchtdeur_ContourVerlichting.lp_Tunnel_Verkeersbuis1_Vluchtdeur_ContourVerlichting);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_Tunnel_Verkeersbuis1_Vluchtdeur_ContourVerlichting = target.s_Tunnel_Verkeersbuis1_Vluchtdeur_ContourVerlichting.copy();
        target.s_Tunnel_Verkeersbuis1_Vluchtdeur_ContourVerlichting.lp_Tunnel_Verkeersbuis1_Vluchtdeur_ContourVerlichting = value;
    }
}
