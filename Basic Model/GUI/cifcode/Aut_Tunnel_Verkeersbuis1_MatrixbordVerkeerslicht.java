...
/** Automaton "Tunnel_Verkeersbuis1_MatrixbordVerkeerslicht". */
public final class Aut_Tunnel_Verkeersbuis1_MatrixbordVerkeerslicht extends RuntimeSimpleAutomaton<State> {
    public Aut_Tunnel_Verkeersbuis1_MatrixbordVerkeerslicht() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_Tunnel_Verkeersbuis1_MatrixbordVerkeerslicht", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "Tunnel_Verkeersbuis1_MatrixbordVerkeerslicht";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_Tunnel_Verkeersbuis1_MatrixbordVerkeerslicht.lp_Tunnel_Verkeersbuis1_MatrixbordVerkeerslicht);
    }
    ...
    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis1_MatrixbordVerkeerslicht_c_aan(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(581).get(2);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(360);
        fillSyncData(aut, 581, rslt, state.s_Tunnel_Verkeersbuis1_MatrixbordVerkeerslicht.lp_Tunnel_Verkeersbuis1_MatrixbordVerkeerslicht);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis1_MatrixbordVerkeerslicht_c_uit(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(582).get(2);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(360);
        fillSyncData(aut, 582, rslt, state.s_Tunnel_Verkeersbuis1_MatrixbordVerkeerslicht.lp_Tunnel_Verkeersbuis1_MatrixbordVerkeerslicht);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_Tunnel_Verkeersbuis1_MatrixbordVerkeerslicht = target.s_Tunnel_Verkeersbuis1_MatrixbordVerkeerslicht.copy();
        target.s_Tunnel_Verkeersbuis1_MatrixbordVerkeerslicht.lp_Tunnel_Verkeersbuis1_MatrixbordVerkeerslicht = value;
    }
}
