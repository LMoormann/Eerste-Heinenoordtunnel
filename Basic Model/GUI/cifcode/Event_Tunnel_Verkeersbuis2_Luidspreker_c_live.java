...
/** Event "Tunnel_Verkeersbuis2_Luidspreker_c_live". */
public final class Event_Tunnel_Verkeersbuis2_Luidspreker_c_live extends RuntimeEvent<State> {
    public Event_Tunnel_Verkeersbuis2_Luidspreker_c_live() {
        super("Tunnel_Verkeersbuis2_Luidspreker_c_live", 934, true);
    }
    ...
    public boolean fillData(State state) {
        ...
        proceed = Aut_sup6.fillSyncData_evt_Tunnel_Verkeersbuis2_Luidspreker_c_live(state);
        if (!proceed) return false;

        // Check automaton "sup72".
        proceed = Aut_sup72.fillSyncData_evt_Tunnel_Verkeersbuis2_Luidspreker_c_live(state);
        if (!proceed) return false;

        // Check automaton "sup82".
        proceed = Aut_sup82.fillSyncData_evt_Tunnel_Verkeersbuis2_Luidspreker_c_live(state);
        if (!proceed) return false;

        // Check automaton "sup84".
        proceed = Aut_sup84.fillSyncData_evt_Tunnel_Verkeersbuis2_Luidspreker_c_live(state);
        if (!proceed) return false;

        // Check automaton "Tunnel_Verkeersbuis2_Luidspreker".
        proceed = Aut_Tunnel_Verkeersbuis2_Luidspreker.fillSyncData_evt_Tunnel_Verkeersbuis2_Luidspreker_c_live(state);
        if (!proceed) return false;
    ...
    public boolean allowedByInvs(State state) {
        ...
        try {
            ...
            throw new CifSimulatorException("Evaluation of invariant \"Tunnel_Verkeersbuis2_Luidspreker_c_live needs Tunnel_Verkeersbuis2_Vluchtdeur_Geluidsbaken.uit\" of the top level scope of the specification failed.", e, state);
        }

        try {
            if (!(state.s_Tunnel_Verkeersbuis2_KnopStartLive_Waarnemer.lp_Tunnel_Verkeersbuis2_KnopStartLive_Waarnemer == 1)) return false;
        } catch (CifSimulatorException e) {
            throw new CifSimulatorException("Evaluation of invariant \"Tunnel_Verkeersbuis2_Luidspreker_c_live needs Tunnel_Verkeersbuis2_KnopStartLive_Waarnemer.ingedrukt\" of the top level scope of the specification failed.", e, state);
        }
