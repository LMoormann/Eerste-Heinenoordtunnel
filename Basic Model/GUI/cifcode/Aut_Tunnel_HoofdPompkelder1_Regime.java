...
/** Automaton "Tunnel_HoofdPompkelder1_Regime". */
public final class Aut_Tunnel_HoofdPompkelder1_Regime extends RuntimeSimpleAutomaton<State> {
    public Aut_Tunnel_HoofdPompkelder1_Regime() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_Tunnel_HoofdPompkelder1_Regime", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "Tunnel_HoofdPompkelder1_Regime";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_Tunnel_HoofdPompkelder1_Regime.lp_Tunnel_HoofdPompkelder1_Regime);
    }
    ...
    public static boolean fillSyncData_evt_Tunnel_HoofdPompkelder1_Regime_c_bergen(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(201).get(3);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(144);
        fillSyncData(aut, 201, rslt, state.s_Tunnel_HoofdPompkelder1_Regime.lp_Tunnel_HoofdPompkelder1_Regime);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_HoofdPompkelder1_Regime_c_leeghouden(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(202).get(3);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(144);
        fillSyncData(aut, 202, rslt, state.s_Tunnel_HoofdPompkelder1_Regime.lp_Tunnel_HoofdPompkelder1_Regime);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_HoofdPompkelder1_Regime_c_uit(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(203).get(3);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(144);
        fillSyncData(aut, 203, rslt, state.s_Tunnel_HoofdPompkelder1_Regime.lp_Tunnel_HoofdPompkelder1_Regime);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_Tunnel_HoofdPompkelder1_Regime = target.s_Tunnel_HoofdPompkelder1_Regime.copy();
        target.s_Tunnel_HoofdPompkelder1_Regime.lp_Tunnel_HoofdPompkelder1_Regime = value;
    }
}
