...
/** Automaton "Tunnel_Verkeersbuis1_VentilatieRichting". */
public final class Aut_Tunnel_Verkeersbuis1_VentilatieRichting extends RuntimeSimpleAutomaton<State> {
    public Aut_Tunnel_Verkeersbuis1_VentilatieRichting() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_Tunnel_Verkeersbuis1_VentilatieRichting", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "Tunnel_Verkeersbuis1_VentilatieRichting";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_Tunnel_Verkeersbuis1_VentilatieRichting.lp_Tunnel_Verkeersbuis1_VentilatieRichting);
    }
    ...
    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis1_VentilatieRichting_c_rijrichting(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(633).get(1);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(376);
        fillSyncData(aut, 633, rslt, state.s_Tunnel_Verkeersbuis1_VentilatieRichting.lp_Tunnel_Verkeersbuis1_VentilatieRichting);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis1_VentilatieRichting_c_tegenRijrichting(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(634).get(1);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(376);
        fillSyncData(aut, 634, rslt, state.s_Tunnel_Verkeersbuis1_VentilatieRichting.lp_Tunnel_Verkeersbuis1_VentilatieRichting);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_Tunnel_Verkeersbuis1_VentilatieRichting = target.s_Tunnel_Verkeersbuis1_VentilatieRichting.copy();
        target.s_Tunnel_Verkeersbuis1_VentilatieRichting.lp_Tunnel_Verkeersbuis1_VentilatieRichting = value;
    }
}
