...
/** Automaton "Tunnel_Verkeersbuis1_SOS". */
public final class Aut_Tunnel_Verkeersbuis1_SOS extends RuntimeSimpleAutomaton<State> {
    public Aut_Tunnel_Verkeersbuis1_SOS() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_Tunnel_Verkeersbuis1_SOS", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "Tunnel_Verkeersbuis1_SOS";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_Tunnel_Verkeersbuis1_SOS.lp_Tunnel_Verkeersbuis1_SOS);
    }
    ...
    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis1_SOS_u_aan(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(607).get(0);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(371);
        fillSyncData(aut, 607, rslt, state.s_Tunnel_Verkeersbuis1_SOS.lp_Tunnel_Verkeersbuis1_SOS);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis1_SOS_u_uit(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(608).get(0);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(371);
        fillSyncData(aut, 608, rslt, state.s_Tunnel_Verkeersbuis1_SOS.lp_Tunnel_Verkeersbuis1_SOS);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_Tunnel_Verkeersbuis1_SOS = target.s_Tunnel_Verkeersbuis1_SOS.copy();
        target.s_Tunnel_Verkeersbuis1_SOS.lp_Tunnel_Verkeersbuis1_SOS = value;
    }
}
