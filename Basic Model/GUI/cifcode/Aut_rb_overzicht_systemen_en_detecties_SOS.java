...
/** Automaton "rb_overzicht_systemen_en_detecties_SOS". */
public final class Aut_rb_overzicht_systemen_en_detecties_SOS extends RuntimeSimpleAutomaton<State> {
    public Aut_rb_overzicht_systemen_en_detecties_SOS() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_rb_overzicht_systemen_en_detecties_SOS", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "rb_overzicht_systemen_en_detecties_SOS";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_rb_overzicht_systemen_en_detecties_SOS.lp_rb_overzicht_systemen_en_detecties_SOS);
    }
    ...
    public static boolean fillSyncData_evt_rb_overzicht_systemen_en_detecties_SOS_open_drop_down_menu(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(125).get(0);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(60);
        fillSyncData(aut, 125, rslt, state.s_rb_overzicht_systemen_en_detecties_SOS.lp_rb_overzicht_systemen_en_detecties_SOS);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_rb_overzicht_systemen_en_detecties_SOS_sluit_drop_down_menu(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(126).get(0);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(60);
        fillSyncData(aut, 126, rslt, state.s_rb_overzicht_systemen_en_detecties_SOS.lp_rb_overzicht_systemen_en_detecties_SOS);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_sluit_alle_tabs(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(157).get(56);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(60);
        fillSyncData(aut, 157, rslt, state.s_rb_overzicht_systemen_en_detecties_SOS.lp_rb_overzicht_systemen_en_detecties_SOS);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_rb_overzicht_systemen_en_detecties_SOS = target.s_rb_overzicht_systemen_en_detecties_SOS.copy();
        target.s_rb_overzicht_systemen_en_detecties_SOS.lp_rb_overzicht_systemen_en_detecties_SOS = value;
    }
}
