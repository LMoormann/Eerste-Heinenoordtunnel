...
/** Automaton "Tunnel_Verkeersbuis1_HulppostkastA_SensorOpen". */
public final class Aut_Tunnel_Verkeersbuis1_HulppostkastA_SensorOpen extends RuntimeSimpleAutomaton<State> {
    public Aut_Tunnel_Verkeersbuis1_HulppostkastA_SensorOpen() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_Tunnel_Verkeersbuis1_HulppostkastA_SensorOpen", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "Tunnel_Verkeersbuis1_HulppostkastA_SensorOpen";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_Tunnel_Verkeersbuis1_HulppostkastA_SensorOpen.lp_Tunnel_Verkeersbuis1_HulppostkastA_SensorOpen);
    }
    ...
    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis1_HulppostkastA_SensorOpen_u_aan(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(415).get(1);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(266);
        fillSyncData(aut, 415, rslt, state.s_Tunnel_Verkeersbuis1_HulppostkastA_SensorOpen.lp_Tunnel_Verkeersbuis1_HulppostkastA_SensorOpen);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis1_HulppostkastA_SensorOpen_u_uit(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(416).get(1);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(266);
        fillSyncData(aut, 416, rslt, state.s_Tunnel_Verkeersbuis1_HulppostkastA_SensorOpen.lp_Tunnel_Verkeersbuis1_HulppostkastA_SensorOpen);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_Tunnel_Verkeersbuis1_HulppostkastA_SensorOpen = target.s_Tunnel_Verkeersbuis1_HulppostkastA_SensorOpen.copy();
        target.s_Tunnel_Verkeersbuis1_HulppostkastA_SensorOpen.lp_Tunnel_Verkeersbuis1_HulppostkastA_SensorOpen = value;
    }
}
