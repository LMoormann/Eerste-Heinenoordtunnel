...
/** Automaton "lb_overzicht_systemen_en_detecties_buis". */
public final class Aut_lb_overzicht_systemen_en_detecties_buis extends RuntimeSimpleAutomaton<State> {
    public Aut_lb_overzicht_systemen_en_detecties_buis() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_lb_overzicht_systemen_en_detecties_buis", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "lb_overzicht_systemen_en_detecties_buis";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_lb_overzicht_systemen_en_detecties_buis.lp_lb_overzicht_systemen_en_detecties_buis);
    }
    ...
    public static boolean fillSyncData_evt_lb_overzicht_systemen_en_detecties_buis_open_drop_down_menu(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(25).get(0);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(10);
        fillSyncData(aut, 25, rslt, state.s_lb_overzicht_systemen_en_detecties_buis.lp_lb_overzicht_systemen_en_detecties_buis);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_lb_overzicht_systemen_en_detecties_buis_sluit_drop_down_menu(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(26).get(0);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(10);
        fillSyncData(aut, 26, rslt, state.s_lb_overzicht_systemen_en_detecties_buis.lp_lb_overzicht_systemen_en_detecties_buis);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_sluit_alle_tabs(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(157).get(6);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(10);
        fillSyncData(aut, 157, rslt, state.s_lb_overzicht_systemen_en_detecties_buis.lp_lb_overzicht_systemen_en_detecties_buis);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_lb_overzicht_systemen_en_detecties_buis = target.s_lb_overzicht_systemen_en_detecties_buis.copy();
        target.s_lb_overzicht_systemen_en_detecties_buis.lp_lb_overzicht_systemen_en_detecties_buis = value;
    }
}
