...
/** Automaton "Tunnel_Verkeersbuis2_Afsluitboom1_ActuatorNeer". */
public final class Aut_Tunnel_Verkeersbuis2_Afsluitboom1_ActuatorNeer extends RuntimeSimpleAutomaton<State> {
    public Aut_Tunnel_Verkeersbuis2_Afsluitboom1_ActuatorNeer() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_Tunnel_Verkeersbuis2_Afsluitboom1_ActuatorNeer", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "Tunnel_Verkeersbuis2_Afsluitboom1_ActuatorNeer";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_Tunnel_Verkeersbuis2_Afsluitboom1_ActuatorNeer.lp_Tunnel_Verkeersbuis2_Afsluitboom1_ActuatorNeer);
    }
    ...
    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis2_Afsluitboom1_ActuatorNeer_c_aan(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(709).get(4);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(412);
        fillSyncData(aut, 709, rslt, state.s_Tunnel_Verkeersbuis2_Afsluitboom1_ActuatorNeer.lp_Tunnel_Verkeersbuis2_Afsluitboom1_ActuatorNeer);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis2_Afsluitboom1_ActuatorNeer_c_uit(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(710).get(4);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(412);
        fillSyncData(aut, 710, rslt, state.s_Tunnel_Verkeersbuis2_Afsluitboom1_ActuatorNeer.lp_Tunnel_Verkeersbuis2_Afsluitboom1_ActuatorNeer);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis2_Afsluitboom1_ActuatorNeer_c_uitNood(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(711).get(4);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(412);
        fillSyncData(aut, 711, rslt, state.s_Tunnel_Verkeersbuis2_Afsluitboom1_ActuatorNeer.lp_Tunnel_Verkeersbuis2_Afsluitboom1_ActuatorNeer);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_Tunnel_Verkeersbuis2_Afsluitboom1_ActuatorNeer = target.s_Tunnel_Verkeersbuis2_Afsluitboom1_ActuatorNeer.copy();
        target.s_Tunnel_Verkeersbuis2_Afsluitboom1_ActuatorNeer.lp_Tunnel_Verkeersbuis2_Afsluitboom1_ActuatorNeer = value;
    }
}
