...
public final class Aut_geselecteerde_tab_tab_systemen_en_detecties extends RuntimeSimpleAutomaton<State> {
    public Aut_geselecteerde_tab_tab_systemen_en_detecties() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_geselecteerde_tab_tab_systemen_en_detecties", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "geselecteerde_tab_tab_systemen_en_detecties";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_geselecteerde_tab_tab_systemen_en_detecties.lp_geselecteerde_tab_tab_systemen_en_detecties);
    }
    ...
    public static boolean fillSyncData_evt_click_tab_linkerbuis(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(11).get(0);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(3);
        fillSyncData(aut, 11, rslt, state.s_geselecteerde_tab_tab_systemen_en_detecties.lp_geselecteerde_tab_tab_systemen_en_detecties);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_click_tab_rechterbuis(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(13).get(0);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(3);
        fillSyncData(aut, 13, rslt, state.s_geselecteerde_tab_tab_systemen_en_detecties.lp_geselecteerde_tab_tab_systemen_en_detecties);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_geselecteerde_tab_tab_systemen_en_detecties = target.s_geselecteerde_tab_tab_systemen_en_detecties.copy();
        target.s_geselecteerde_tab_tab_systemen_en_detecties.lp_geselecteerde_tab_tab_systemen_en_detecties = value;
    }
}
