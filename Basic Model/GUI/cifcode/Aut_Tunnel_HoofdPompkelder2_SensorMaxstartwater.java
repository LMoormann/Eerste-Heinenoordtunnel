...
/** Automaton "Tunnel_HoofdPompkelder2_SensorMaxstartwater". */
public final class Aut_Tunnel_HoofdPompkelder2_SensorMaxstartwater extends RuntimeSimpleAutomaton<State> {
    public Aut_Tunnel_HoofdPompkelder2_SensorMaxstartwater() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_Tunnel_HoofdPompkelder2_SensorMaxstartwater", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "Tunnel_HoofdPompkelder2_SensorMaxstartwater";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_Tunnel_HoofdPompkelder2_SensorMaxstartwater.lp_Tunnel_HoofdPompkelder2_SensorMaxstartwater);
    }
    ...
    public static boolean fillSyncData_evt_Tunnel_HoofdPompkelder2_SensorMaxstartwater_u_aan(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(244).get(2);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(168);
        fillSyncData(aut, 244, rslt, state.s_Tunnel_HoofdPompkelder2_SensorMaxstartwater.lp_Tunnel_HoofdPompkelder2_SensorMaxstartwater);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_HoofdPompkelder2_SensorMaxstartwater_u_uit(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(245).get(2);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(168);
        fillSyncData(aut, 245, rslt, state.s_Tunnel_HoofdPompkelder2_SensorMaxstartwater.lp_Tunnel_HoofdPompkelder2_SensorMaxstartwater);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_Tunnel_HoofdPompkelder2_SensorMaxstartwater = target.s_Tunnel_HoofdPompkelder2_SensorMaxstartwater.copy();
        target.s_Tunnel_HoofdPompkelder2_SensorMaxstartwater.lp_Tunnel_HoofdPompkelder2_SensorMaxstartwater = value;
    }
}
