...
/** Event "Tunnel_MiddenPompkelder_SensorStartwater_u_uit". */
public final class Event_Tunnel_MiddenPompkelder_SensorStartwater_u_uit extends RuntimeEvent<State> {
    public Event_Tunnel_MiddenPompkelder_SensorStartwater_u_uit() {
        super("Tunnel_MiddenPompkelder_SensorStartwater_u_uit", 283, false);
    }
    ...
    public boolean fillData(State state) {
        ...
        proceed = Aut_event_disabler.fillSyncData_evt_Tunnel_MiddenPompkelder_SensorStartwater_u_uit(state);
        if (!proceed) return false;

        // Check automaton "Tunnel_MiddenPompkelder_FysiekeRelatie".
        proceed = Aut_Tunnel_MiddenPompkelder_FysiekeRelatie.fillSyncData_evt_Tunnel_MiddenPompkelder_SensorStartwater_u_uit(state);
        if (!proceed) return false;

        // Check automaton "Tunnel_MiddenPompkelder_SensorStartwater".
        proceed = Aut_Tunnel_MiddenPompkelder_SensorStartwater.fillSyncData_evt_Tunnel_MiddenPompkelder_SensorStartwater_u_uit(state);
        if (!proceed) return false;
