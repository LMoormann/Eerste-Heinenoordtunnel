...
/** Automaton "Tunnel_MiddenPompkelder_SensorMaxstartwater". */
public final class Aut_Tunnel_MiddenPompkelder_SensorMaxstartwater extends RuntimeSimpleAutomaton<State> {
    public Aut_Tunnel_MiddenPompkelder_SensorMaxstartwater() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_Tunnel_MiddenPompkelder_SensorMaxstartwater", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "Tunnel_MiddenPompkelder_SensorMaxstartwater";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_Tunnel_MiddenPompkelder_SensorMaxstartwater.lp_Tunnel_MiddenPompkelder_SensorMaxstartwater);
    }
    ...
    public static boolean fillSyncData_evt_Tunnel_MiddenPompkelder_SensorMaxstartwater_u_aan(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(280).get(2);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(189);
        fillSyncData(aut, 280, rslt, state.s_Tunnel_MiddenPompkelder_SensorMaxstartwater.lp_Tunnel_MiddenPompkelder_SensorMaxstartwater);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_MiddenPompkelder_SensorMaxstartwater_u_uit(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(281).get(2);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(189);
        fillSyncData(aut, 281, rslt, state.s_Tunnel_MiddenPompkelder_SensorMaxstartwater.lp_Tunnel_MiddenPompkelder_SensorMaxstartwater);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_Tunnel_MiddenPompkelder_SensorMaxstartwater = target.s_Tunnel_MiddenPompkelder_SensorMaxstartwater.copy();
        target.s_Tunnel_MiddenPompkelder_SensorMaxstartwater.lp_Tunnel_MiddenPompkelder_SensorMaxstartwater = value;
    }
}
