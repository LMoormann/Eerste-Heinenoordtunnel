...
/** Automaton "lb_overzicht_systemen_en_detecties_overdruk_veilige_ruimte". */
public final class Aut_lb_overzicht_systemen_en_detecties_overdruk_veilige_ruimte extends RuntimeSimpleAutomaton<State> {
    public Aut_lb_overzicht_systemen_en_detecties_overdruk_veilige_ruimte() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_lb_overzicht_systemen_en_detecties_overdruk_veilige_ruimte", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "lb_overzicht_systemen_en_detecties_overdruk_veilige_ruimte";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_lb_overzicht_systemen_en_detecties_overdruk_veilige_ruimte.lp_lb_overzicht_systemen_en_detecties_overdruk_veilige_ruimte);
    }
    ...
    public static boolean fillSyncData_evt_lb_overzicht_systemen_en_detecties_overdruk_veilige_ruimte_open_drop_down_menu(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(43).get(0);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(19);
        fillSyncData(aut, 43, rslt, state.s_lb_overzicht_systemen_en_detecties_overdruk_veilige_ruimte.lp_lb_overzicht_systemen_en_detecties_overdruk_veilige_ruimte);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_lb_overzicht_systemen_en_detecties_overdruk_veilige_ruimte_sluit_drop_down_menu(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(44).get(0);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(19);
        fillSyncData(aut, 44, rslt, state.s_lb_overzicht_systemen_en_detecties_overdruk_veilige_ruimte.lp_lb_overzicht_systemen_en_detecties_overdruk_veilige_ruimte);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_sluit_alle_tabs(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(157).get(15);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(19);
        fillSyncData(aut, 157, rslt, state.s_lb_overzicht_systemen_en_detecties_overdruk_veilige_ruimte.lp_lb_overzicht_systemen_en_detecties_overdruk_veilige_ruimte);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_lb_overzicht_systemen_en_detecties_overdruk_veilige_ruimte = target.s_lb_overzicht_systemen_en_detecties_overdruk_veilige_ruimte.copy();
        target.s_lb_overzicht_systemen_en_detecties_overdruk_veilige_ruimte.lp_lb_overzicht_systemen_en_detecties_overdruk_veilige_ruimte = value;
    }
}
