...
/** Automaton "rb_overzicht_systemen_en_detecties_verkeer". */
public final class Aut_rb_overzicht_systemen_en_detecties_verkeer extends RuntimeSimpleAutomaton<State> {
    public Aut_rb_overzicht_systemen_en_detecties_verkeer() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_rb_overzicht_systemen_en_detecties_verkeer", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "rb_overzicht_systemen_en_detecties_verkeer";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_rb_overzicht_systemen_en_detecties_verkeer.lp_rb_overzicht_systemen_en_detecties_verkeer);
    }
    ...
    public static boolean fillSyncData_evt_rb_overzicht_systemen_en_detecties_verkeer_open_drop_down_menu(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(133).get(0);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(64);
        fillSyncData(aut, 133, rslt, state.s_rb_overzicht_systemen_en_detecties_verkeer.lp_rb_overzicht_systemen_en_detecties_verkeer);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_rb_overzicht_systemen_en_detecties_verkeer_sluit_drop_down_menu(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(134).get(0);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(64);
        fillSyncData(aut, 134, rslt, state.s_rb_overzicht_systemen_en_detecties_verkeer.lp_rb_overzicht_systemen_en_detecties_verkeer);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_sluit_alle_tabs(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(157).get(60);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(64);
        fillSyncData(aut, 157, rslt, state.s_rb_overzicht_systemen_en_detecties_verkeer.lp_rb_overzicht_systemen_en_detecties_verkeer);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_rb_overzicht_systemen_en_detecties_verkeer = target.s_rb_overzicht_systemen_en_detecties_verkeer.copy();
        target.s_rb_overzicht_systemen_en_detecties_verkeer.lp_rb_overzicht_systemen_en_detecties_verkeer = value;
    }
}
