...
/** Automaton "Tunnel_MiddenTunnelKanaal_KnopVerlichtingAan_Knop". */
public final class Aut_Tunnel_MiddenTunnelKanaal_KnopVerlichtingAan_Knop extends RuntimeSimpleAutomaton<State> {
    public Aut_Tunnel_MiddenTunnelKanaal_KnopVerlichtingAan_Knop() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_Tunnel_MiddenTunnelKanaal_KnopVerlichtingAan_Knop", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "Tunnel_MiddenTunnelKanaal_KnopVerlichtingAan_Knop";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_Tunnel_MiddenTunnelKanaal_KnopVerlichtingAan_Knop.lp_Tunnel_MiddenTunnelKanaal_KnopVerlichtingAan_Knop);
    }
    ...
    public static boolean fillSyncData_evt_Tunnel_MiddenTunnelKanaal_KnopVerlichtingAan_Knop_u_indrukken(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(335).get(0);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(223);
        fillSyncData(aut, 335, rslt, state.s_Tunnel_MiddenTunnelKanaal_KnopVerlichtingAan_Knop.lp_Tunnel_MiddenTunnelKanaal_KnopVerlichtingAan_Knop);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_Tunnel_MiddenTunnelKanaal_KnopVerlichtingAan_Knop = target.s_Tunnel_MiddenTunnelKanaal_KnopVerlichtingAan_Knop.copy();
        target.s_Tunnel_MiddenTunnelKanaal_KnopVerlichtingAan_Knop.lp_Tunnel_MiddenTunnelKanaal_KnopVerlichtingAan_Knop = value;
    }
}
