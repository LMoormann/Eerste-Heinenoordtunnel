...
/** Automaton "Tunnel_Verkeersbuis2_Verkeerslicht2_Stand". */
public final class Aut_Tunnel_Verkeersbuis2_Verkeerslicht2_Stand extends RuntimeSimpleAutomaton<State> {
    public Aut_Tunnel_Verkeersbuis2_Verkeerslicht2_Stand() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_Tunnel_Verkeersbuis2_Verkeerslicht2_Stand", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "Tunnel_Verkeersbuis2_Verkeerslicht2_Stand";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_Tunnel_Verkeersbuis2_Verkeerslicht2_Stand.lp_Tunnel_Verkeersbuis2_Verkeerslicht2_Stand);
    }
    ...
    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis2_Verkeerslicht2_Stand_c_geel(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(1002).get(4);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(564);
        fillSyncData(aut, 1002, rslt, state.s_Tunnel_Verkeersbuis2_Verkeerslicht2_Stand.lp_Tunnel_Verkeersbuis2_Verkeerslicht2_Stand);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis2_Verkeerslicht2_Stand_c_knipper(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(1003).get(4);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(564);
        fillSyncData(aut, 1003, rslt, state.s_Tunnel_Verkeersbuis2_Verkeerslicht2_Stand.lp_Tunnel_Verkeersbuis2_Verkeerslicht2_Stand);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis2_Verkeerslicht2_Stand_c_rood(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(1004).get(4);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(564);
        fillSyncData(aut, 1004, rslt, state.s_Tunnel_Verkeersbuis2_Verkeerslicht2_Stand.lp_Tunnel_Verkeersbuis2_Verkeerslicht2_Stand);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis2_Verkeerslicht2_Stand_c_uit(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(1005).get(4);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(564);
        fillSyncData(aut, 1005, rslt, state.s_Tunnel_Verkeersbuis2_Verkeerslicht2_Stand.lp_Tunnel_Verkeersbuis2_Verkeerslicht2_Stand);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_Tunnel_Verkeersbuis2_Verkeerslicht2_Stand = target.s_Tunnel_Verkeersbuis2_Verkeerslicht2_Stand.copy();
        target.s_Tunnel_Verkeersbuis2_Verkeerslicht2_Stand.lp_Tunnel_Verkeersbuis2_Verkeerslicht2_Stand = value;
    }
}
