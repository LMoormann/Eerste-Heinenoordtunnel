...
/** Event "Tunnel_MiddenTunnelKanaal_Luidspreker_c_onderdrukt". */
public final class Event_Tunnel_MiddenTunnelKanaal_Luidspreker_c_onderdrukt extends RuntimeEvent<State> {
    public Event_Tunnel_MiddenTunnelKanaal_Luidspreker_c_onderdrukt() {
        super("Tunnel_MiddenTunnelKanaal_Luidspreker_c_onderdrukt", 343, true);
    }
    ...
    public boolean fillData(State state) {
        ...
        proceed = Aut_sup6.fillSyncData_evt_Tunnel_MiddenTunnelKanaal_Luidspreker_c_onderdrukt(state);
        if (!proceed) return false;

        // Check automaton "sup82".
        proceed = Aut_sup82.fillSyncData_evt_Tunnel_MiddenTunnelKanaal_Luidspreker_c_onderdrukt(state);
        if (!proceed) return false;

        // Check automaton "sup85".
        proceed = Aut_sup85.fillSyncData_evt_Tunnel_MiddenTunnelKanaal_Luidspreker_c_onderdrukt(state);
        if (!proceed) return false;

        // Check automaton "Tunnel_MiddenTunnelKanaal_Luidspreker".
        proceed = Aut_Tunnel_MiddenTunnelKanaal_Luidspreker.fillSyncData_evt_Tunnel_MiddenTunnelKanaal_Luidspreker_c_onderdrukt(state);
        if (!proceed) return false;
    ...
    public boolean allowedByInvs(State state) {
        // Invariants for "".
        try {
            if (!((state.s_Tunnel_Verkeersbuis1_WisselTimerAudio.lp_Tunnel_Verkeersbuis1_WisselTimerAudio == 2) || (state.s_Tunnel_Verkeersbuis2_WisselTimerAudio.lp_Tunnel_Verkeersbuis2_WisselTimerAudio == 2))) return false;
        } catch (CifSimulatorException e) {
            throw new CifSimulatorException("Evaluation of invariant \"Tunnel_MiddenTunnelKanaal_Luidspreker_c_onderdrukt needs Tunnel_Verkeersbuis1_WisselTimerAudio.aan2 or Tunnel_Verkeersbuis2_WisselTimerAudio.aan2\" of the top level scope of the specification failed.", e, state);
        }
