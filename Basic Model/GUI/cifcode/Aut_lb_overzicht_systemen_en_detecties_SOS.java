...
/** Automaton "lb_overzicht_systemen_en_detecties_SOS". */
public final class Aut_lb_overzicht_systemen_en_detecties_SOS extends RuntimeSimpleAutomaton<State> {
    public Aut_lb_overzicht_systemen_en_detecties_SOS() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_lb_overzicht_systemen_en_detecties_SOS", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "lb_overzicht_systemen_en_detecties_SOS";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_lb_overzicht_systemen_en_detecties_SOS.lp_lb_overzicht_systemen_en_detecties_SOS);
    }
    ...
    public static boolean fillSyncData_evt_lb_overzicht_systemen_en_detecties_SOS_open_drop_down_menu(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(49).get(0);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(22);
        fillSyncData(aut, 49, rslt, state.s_lb_overzicht_systemen_en_detecties_SOS.lp_lb_overzicht_systemen_en_detecties_SOS);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_lb_overzicht_systemen_en_detecties_SOS_sluit_drop_down_menu(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(50).get(0);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(22);
        fillSyncData(aut, 50, rslt, state.s_lb_overzicht_systemen_en_detecties_SOS.lp_lb_overzicht_systemen_en_detecties_SOS);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_sluit_alle_tabs(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(157).get(18);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(22);
        fillSyncData(aut, 157, rslt, state.s_lb_overzicht_systemen_en_detecties_SOS.lp_lb_overzicht_systemen_en_detecties_SOS);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_lb_overzicht_systemen_en_detecties_SOS = target.s_lb_overzicht_systemen_en_detecties_SOS.copy();
        target.s_lb_overzicht_systemen_en_detecties_SOS.lp_lb_overzicht_systemen_en_detecties_SOS = value;
    }
}
