...
/** Automaton "Tunnel_Verkeersbuis2_Evacuatie". */
public final class Aut_Tunnel_Verkeersbuis2_Evacuatie extends RuntimeSimpleAutomaton<State> {
    public Aut_Tunnel_Verkeersbuis2_Evacuatie() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_Tunnel_Verkeersbuis2_Evacuatie", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "Tunnel_Verkeersbuis2_Evacuatie";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_Tunnel_Verkeersbuis2_Evacuatie.lp_Tunnel_Verkeersbuis2_Evacuatie);
    }
    ...
    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis2_Evacuatie_c_aan(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(765).get(5);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(444);
        fillSyncData(aut, 765, rslt, state.s_Tunnel_Verkeersbuis2_Evacuatie.lp_Tunnel_Verkeersbuis2_Evacuatie);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis2_Evacuatie_c_uit(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(766).get(5);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(444);
        fillSyncData(aut, 766, rslt, state.s_Tunnel_Verkeersbuis2_Evacuatie.lp_Tunnel_Verkeersbuis2_Evacuatie);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_Tunnel_Verkeersbuis2_Evacuatie = target.s_Tunnel_Verkeersbuis2_Evacuatie.copy();
        target.s_Tunnel_Verkeersbuis2_Evacuatie.lp_Tunnel_Verkeersbuis2_Evacuatie = value;
    }
}
