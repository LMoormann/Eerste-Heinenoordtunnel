...
/** Automaton "Tunnel_Verkeersbuis2_Verkeerslicht3_Stand". */
public final class Aut_Tunnel_Verkeersbuis2_Verkeerslicht3_Stand extends RuntimeSimpleAutomaton<State> {
    public Aut_Tunnel_Verkeersbuis2_Verkeerslicht3_Stand() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_Tunnel_Verkeersbuis2_Verkeerslicht3_Stand", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "Tunnel_Verkeersbuis2_Verkeerslicht3_Stand";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_Tunnel_Verkeersbuis2_Verkeerslicht3_Stand.lp_Tunnel_Verkeersbuis2_Verkeerslicht3_Stand);
    }
    ...
    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis2_Verkeerslicht3_Stand_c_geel(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(1011).get(4);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(568);
        fillSyncData(aut, 1011, rslt, state.s_Tunnel_Verkeersbuis2_Verkeerslicht3_Stand.lp_Tunnel_Verkeersbuis2_Verkeerslicht3_Stand);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis2_Verkeerslicht3_Stand_c_knipper(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(1012).get(4);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(568);
        fillSyncData(aut, 1012, rslt, state.s_Tunnel_Verkeersbuis2_Verkeerslicht3_Stand.lp_Tunnel_Verkeersbuis2_Verkeerslicht3_Stand);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis2_Verkeerslicht3_Stand_c_rood(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(1013).get(4);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(568);
        fillSyncData(aut, 1013, rslt, state.s_Tunnel_Verkeersbuis2_Verkeerslicht3_Stand.lp_Tunnel_Verkeersbuis2_Verkeerslicht3_Stand);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis2_Verkeerslicht3_Stand_c_uit(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(1014).get(4);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(568);
        fillSyncData(aut, 1014, rslt, state.s_Tunnel_Verkeersbuis2_Verkeerslicht3_Stand.lp_Tunnel_Verkeersbuis2_Verkeerslicht3_Stand);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_Tunnel_Verkeersbuis2_Verkeerslicht3_Stand = target.s_Tunnel_Verkeersbuis2_Verkeerslicht3_Stand.copy();
        target.s_Tunnel_Verkeersbuis2_Verkeerslicht3_Stand.lp_Tunnel_Verkeersbuis2_Verkeerslicht3_Stand = value;
    }
}
