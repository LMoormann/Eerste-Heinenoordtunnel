...
/** Automaton "rb_overzicht_systemen_en_detecties_noodtelefoon". */
public final class Aut_rb_overzicht_systemen_en_detecties_noodtelefoon extends RuntimeSimpleAutomaton<State> {
    public Aut_rb_overzicht_systemen_en_detecties_noodtelefoon() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_rb_overzicht_systemen_en_detecties_noodtelefoon", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "rb_overzicht_systemen_en_detecties_noodtelefoon";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_rb_overzicht_systemen_en_detecties_noodtelefoon.lp_rb_overzicht_systemen_en_detecties_noodtelefoon);
    }
    ...
    public static boolean fillSyncData_evt_rb_overzicht_systemen_en_detecties_noodtelefoon_open_drop_down_menu(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(117).get(0);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(56);
        fillSyncData(aut, 117, rslt, state.s_rb_overzicht_systemen_en_detecties_noodtelefoon.lp_rb_overzicht_systemen_en_detecties_noodtelefoon);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_rb_overzicht_systemen_en_detecties_noodtelefoon_sluit_drop_down_menu(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(118).get(0);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(56);
        fillSyncData(aut, 118, rslt, state.s_rb_overzicht_systemen_en_detecties_noodtelefoon.lp_rb_overzicht_systemen_en_detecties_noodtelefoon);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_sluit_alle_tabs(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(157).get(52);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(56);
        fillSyncData(aut, 157, rslt, state.s_rb_overzicht_systemen_en_detecties_noodtelefoon.lp_rb_overzicht_systemen_en_detecties_noodtelefoon);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_rb_overzicht_systemen_en_detecties_noodtelefoon = target.s_rb_overzicht_systemen_en_detecties_noodtelefoon.copy();
        target.s_rb_overzicht_systemen_en_detecties_noodtelefoon.lp_rb_overzicht_systemen_en_detecties_noodtelefoon = value;
    }
}
