...
/** Automaton "Tunnel_Verkeersbuis1_StandVerkeersbuis". */
public final class Aut_Tunnel_Verkeersbuis1_StandVerkeersbuis extends RuntimeSimpleAutomaton<State> {
    public Aut_Tunnel_Verkeersbuis1_StandVerkeersbuis() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_Tunnel_Verkeersbuis1_StandVerkeersbuis", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "Tunnel_Verkeersbuis1_StandVerkeersbuis";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_Tunnel_Verkeersbuis1_StandVerkeersbuis.lp_Tunnel_Verkeersbuis1_StandVerkeersbuis);
    }
    ...
    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis1_StandVerkeersbuis_c_calamiteit(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(611).get(5);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(373);
        fillSyncData(aut, 611, rslt, state.s_Tunnel_Verkeersbuis1_StandVerkeersbuis.lp_Tunnel_Verkeersbuis1_StandVerkeersbuis);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis1_StandVerkeersbuis_c_herstel(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(612).get(5);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(373);
        fillSyncData(aut, 612, rslt, state.s_Tunnel_Verkeersbuis1_StandVerkeersbuis.lp_Tunnel_Verkeersbuis1_StandVerkeersbuis);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis1_StandVerkeersbuis_c_ondersteunend(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(613).get(5);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(373);
        fillSyncData(aut, 613, rslt, state.s_Tunnel_Verkeersbuis1_StandVerkeersbuis.lp_Tunnel_Verkeersbuis1_StandVerkeersbuis);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis1_StandVerkeersbuis_c_operationeel(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(614).get(5);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(373);
        fillSyncData(aut, 614, rslt, state.s_Tunnel_Verkeersbuis1_StandVerkeersbuis.lp_Tunnel_Verkeersbuis1_StandVerkeersbuis);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_Tunnel_Verkeersbuis1_StandVerkeersbuis = target.s_Tunnel_Verkeersbuis1_StandVerkeersbuis.copy();
        target.s_Tunnel_Verkeersbuis1_StandVerkeersbuis.lp_Tunnel_Verkeersbuis1_StandVerkeersbuis = value;
    }
}
