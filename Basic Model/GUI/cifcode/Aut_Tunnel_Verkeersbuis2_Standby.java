...
/** Automaton "Tunnel_Verkeersbuis2_Standby". */
public final class Aut_Tunnel_Verkeersbuis2_Standby extends RuntimeSimpleAutomaton<State> {
    public Aut_Tunnel_Verkeersbuis2_Standby() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_Tunnel_Verkeersbuis2_Standby", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "Tunnel_Verkeersbuis2_Standby";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_Tunnel_Verkeersbuis2_Standby.lp_Tunnel_Verkeersbuis2_Standby);
    }
    ...
    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis2_Standby_c_aan(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(967).get(5);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(554);
        fillSyncData(aut, 967, rslt, state.s_Tunnel_Verkeersbuis2_Standby.lp_Tunnel_Verkeersbuis2_Standby);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis2_Standby_c_uit(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(968).get(5);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(554);
        fillSyncData(aut, 968, rslt, state.s_Tunnel_Verkeersbuis2_Standby.lp_Tunnel_Verkeersbuis2_Standby);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_Tunnel_Verkeersbuis2_Standby = target.s_Tunnel_Verkeersbuis2_Standby.copy();
        target.s_Tunnel_Verkeersbuis2_Standby.lp_Tunnel_Verkeersbuis2_Standby = value;
    }
}
