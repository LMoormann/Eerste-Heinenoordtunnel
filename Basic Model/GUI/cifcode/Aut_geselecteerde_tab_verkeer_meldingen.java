...
public final class Aut_geselecteerde_tab_verkeer_meldingen extends RuntimeSimpleAutomaton<State> {
    public Aut_geselecteerde_tab_verkeer_meldingen() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_geselecteerde_tab_verkeer_meldingen", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "geselecteerde_tab_verkeer_meldingen";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_geselecteerde_tab_verkeer_meldingen.lp_geselecteerde_tab_verkeer_meldingen);
    }
    ...
    public static boolean fillSyncData_evt_click_tab_meldingen(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(12).get(0);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(4);
        fillSyncData(aut, 12, rslt, state.s_geselecteerde_tab_verkeer_meldingen.lp_geselecteerde_tab_verkeer_meldingen);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_click_tab_verkeer(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(14).get(0);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(4);
        fillSyncData(aut, 14, rslt, state.s_geselecteerde_tab_verkeer_meldingen.lp_geselecteerde_tab_verkeer_meldingen);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_geselecteerde_tab_verkeer_meldingen = target.s_geselecteerde_tab_verkeer_meldingen.copy();
        target.s_geselecteerde_tab_verkeer_meldingen.lp_geselecteerde_tab_verkeer_meldingen = value;
    }
}
