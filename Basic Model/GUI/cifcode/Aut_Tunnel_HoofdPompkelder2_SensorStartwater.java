...
/** Automaton "Tunnel_HoofdPompkelder2_SensorStartwater". */
public final class Aut_Tunnel_HoofdPompkelder2_SensorStartwater extends RuntimeSimpleAutomaton<State> {
    public Aut_Tunnel_HoofdPompkelder2_SensorStartwater() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_Tunnel_HoofdPompkelder2_SensorStartwater", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "Tunnel_HoofdPompkelder2_SensorStartwater";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_Tunnel_HoofdPompkelder2_SensorStartwater.lp_Tunnel_HoofdPompkelder2_SensorStartwater);
    }
    ...
    public static boolean fillSyncData_evt_Tunnel_HoofdPompkelder2_SensorStartwater_u_aan(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(246).get(2);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(169);
        fillSyncData(aut, 246, rslt, state.s_Tunnel_HoofdPompkelder2_SensorStartwater.lp_Tunnel_HoofdPompkelder2_SensorStartwater);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_HoofdPompkelder2_SensorStartwater_u_uit(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(247).get(2);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(169);
        fillSyncData(aut, 247, rslt, state.s_Tunnel_HoofdPompkelder2_SensorStartwater.lp_Tunnel_HoofdPompkelder2_SensorStartwater);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_Tunnel_HoofdPompkelder2_SensorStartwater = target.s_Tunnel_HoofdPompkelder2_SensorStartwater.copy();
        target.s_Tunnel_HoofdPompkelder2_SensorStartwater.lp_Tunnel_HoofdPompkelder2_SensorStartwater = value;
    }
}
