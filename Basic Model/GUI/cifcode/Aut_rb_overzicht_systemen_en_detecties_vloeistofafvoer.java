...
/** Automaton "rb_overzicht_systemen_en_detecties_vloeistofafvoer". */
public final class Aut_rb_overzicht_systemen_en_detecties_vloeistofafvoer extends RuntimeSimpleAutomaton<State> {
    public Aut_rb_overzicht_systemen_en_detecties_vloeistofafvoer() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_rb_overzicht_systemen_en_detecties_vloeistofafvoer", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "rb_overzicht_systemen_en_detecties_vloeistofafvoer";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_rb_overzicht_systemen_en_detecties_vloeistofafvoer.lp_rb_overzicht_systemen_en_detecties_vloeistofafvoer);
    }
    ...
    public static boolean fillSyncData_evt_rb_overzicht_systemen_en_detecties_vloeistofafvoer_open_drop_down_menu(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(143).get(0);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(69);
        fillSyncData(aut, 143, rslt, state.s_rb_overzicht_systemen_en_detecties_vloeistofafvoer.lp_rb_overzicht_systemen_en_detecties_vloeistofafvoer);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_rb_overzicht_systemen_en_detecties_vloeistofafvoer_sluit_drop_down_menu(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(144).get(0);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(69);
        fillSyncData(aut, 144, rslt, state.s_rb_overzicht_systemen_en_detecties_vloeistofafvoer.lp_rb_overzicht_systemen_en_detecties_vloeistofafvoer);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_sluit_alle_tabs(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(157).get(65);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(69);
        fillSyncData(aut, 157, rslt, state.s_rb_overzicht_systemen_en_detecties_vloeistofafvoer.lp_rb_overzicht_systemen_en_detecties_vloeistofafvoer);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_rb_overzicht_systemen_en_detecties_vloeistofafvoer = target.s_rb_overzicht_systemen_en_detecties_vloeistofafvoer.copy();
        target.s_rb_overzicht_systemen_en_detecties_vloeistofafvoer.lp_rb_overzicht_systemen_en_detecties_vloeistofafvoer = value;
    }
}
