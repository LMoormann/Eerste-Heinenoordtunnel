...
/** Automaton "Tunnel_Verkeersbuis2_Verkeerslicht1_Stand". */
public final class Aut_Tunnel_Verkeersbuis2_Verkeerslicht1_Stand extends RuntimeSimpleAutomaton<State> {
    public Aut_Tunnel_Verkeersbuis2_Verkeerslicht1_Stand() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_Tunnel_Verkeersbuis2_Verkeerslicht1_Stand", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "Tunnel_Verkeersbuis2_Verkeerslicht1_Stand";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_Tunnel_Verkeersbuis2_Verkeerslicht1_Stand.lp_Tunnel_Verkeersbuis2_Verkeerslicht1_Stand);
    }
    ...
    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis2_Verkeerslicht1_Stand_c_geel(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(993).get(4);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(560);
        fillSyncData(aut, 993, rslt, state.s_Tunnel_Verkeersbuis2_Verkeerslicht1_Stand.lp_Tunnel_Verkeersbuis2_Verkeerslicht1_Stand);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis2_Verkeerslicht1_Stand_c_knipper(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(994).get(4);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(560);
        fillSyncData(aut, 994, rslt, state.s_Tunnel_Verkeersbuis2_Verkeerslicht1_Stand.lp_Tunnel_Verkeersbuis2_Verkeerslicht1_Stand);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis2_Verkeerslicht1_Stand_c_rood(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(995).get(4);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(560);
        fillSyncData(aut, 995, rslt, state.s_Tunnel_Verkeersbuis2_Verkeerslicht1_Stand.lp_Tunnel_Verkeersbuis2_Verkeerslicht1_Stand);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis2_Verkeerslicht1_Stand_c_uit(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(996).get(4);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(560);
        fillSyncData(aut, 996, rslt, state.s_Tunnel_Verkeersbuis2_Verkeerslicht1_Stand.lp_Tunnel_Verkeersbuis2_Verkeerslicht1_Stand);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_Tunnel_Verkeersbuis2_Verkeerslicht1_Stand = target.s_Tunnel_Verkeersbuis2_Verkeerslicht1_Stand.copy();
        target.s_Tunnel_Verkeersbuis2_Verkeerslicht1_Stand.lp_Tunnel_Verkeersbuis2_Verkeerslicht1_Stand = value;
    }
}
