...
/** Automaton "Tunnel_Verkeersbuis2_Afsluitboom1_SensorOpen". */
public final class Aut_Tunnel_Verkeersbuis2_Afsluitboom1_SensorOpen extends RuntimeSimpleAutomaton<State> {
    public Aut_Tunnel_Verkeersbuis2_Afsluitboom1_SensorOpen() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_Tunnel_Verkeersbuis2_Afsluitboom1_SensorOpen", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "Tunnel_Verkeersbuis2_Afsluitboom1_SensorOpen";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_Tunnel_Verkeersbuis2_Afsluitboom1_SensorOpen.lp_Tunnel_Verkeersbuis2_Afsluitboom1_SensorOpen);
    }
    ...
    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis2_Afsluitboom1_SensorOpen_u_aan(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(719).get(2);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(417);
        fillSyncData(aut, 719, rslt, state.s_Tunnel_Verkeersbuis2_Afsluitboom1_SensorOpen.lp_Tunnel_Verkeersbuis2_Afsluitboom1_SensorOpen);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis2_Afsluitboom1_SensorOpen_u_uit(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(720).get(2);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(417);
        fillSyncData(aut, 720, rslt, state.s_Tunnel_Verkeersbuis2_Afsluitboom1_SensorOpen.lp_Tunnel_Verkeersbuis2_Afsluitboom1_SensorOpen);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_Tunnel_Verkeersbuis2_Afsluitboom1_SensorOpen = target.s_Tunnel_Verkeersbuis2_Afsluitboom1_SensorOpen.copy();
        target.s_Tunnel_Verkeersbuis2_Afsluitboom1_SensorOpen.lp_Tunnel_Verkeersbuis2_Afsluitboom1_SensorOpen = value;
    }
}
