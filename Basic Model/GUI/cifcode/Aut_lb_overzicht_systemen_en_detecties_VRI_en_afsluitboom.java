...
/** Automaton "lb_overzicht_systemen_en_detecties_VRI_en_afsluitboom". */
public final class Aut_lb_overzicht_systemen_en_detecties_VRI_en_afsluitboom extends RuntimeSimpleAutomaton<State> {
    public Aut_lb_overzicht_systemen_en_detecties_VRI_en_afsluitboom() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_lb_overzicht_systemen_en_detecties_VRI_en_afsluitboom", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "lb_overzicht_systemen_en_detecties_VRI_en_afsluitboom";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_lb_overzicht_systemen_en_detecties_VRI_en_afsluitboom.lp_lb_overzicht_systemen_en_detecties_VRI_en_afsluitboom);
    }
    ...
    public static boolean fillSyncData_evt_lb_overzicht_systemen_en_detecties_VRI_en_afsluitboom_open_drop_down_menu(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(79).get(0);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(37);
        fillSyncData(aut, 79, rslt, state.s_lb_overzicht_systemen_en_detecties_VRI_en_afsluitboom.lp_lb_overzicht_systemen_en_detecties_VRI_en_afsluitboom);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_lb_overzicht_systemen_en_detecties_VRI_en_afsluitboom_sluit_drop_down_menu(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(80).get(0);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(37);
        fillSyncData(aut, 80, rslt, state.s_lb_overzicht_systemen_en_detecties_VRI_en_afsluitboom.lp_lb_overzicht_systemen_en_detecties_VRI_en_afsluitboom);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_sluit_alle_tabs(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(157).get(33);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(37);
        fillSyncData(aut, 157, rslt, state.s_lb_overzicht_systemen_en_detecties_VRI_en_afsluitboom.lp_lb_overzicht_systemen_en_detecties_VRI_en_afsluitboom);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_lb_overzicht_systemen_en_detecties_VRI_en_afsluitboom = target.s_lb_overzicht_systemen_en_detecties_VRI_en_afsluitboom.copy();
        target.s_lb_overzicht_systemen_en_detecties_VRI_en_afsluitboom.lp_lb_overzicht_systemen_en_detecties_VRI_en_afsluitboom = value;
    }
}
