...
/** Automaton "lb_overzicht_systemen_en_detecties_drop_down_richting_ventilatie". */
public final class Aut_lb_overzicht_systemen_en_detecties_drop_down_richting_ventilatie extends RuntimeSimpleAutomaton<State> {
    public Aut_lb_overzicht_systemen_en_detecties_drop_down_richting_ventilatie() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_lb_overzicht_systemen_en_detecties_drop_down_richting_ventilatie", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "lb_overzicht_systemen_en_detecties_drop_down_richting_ventilatie";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_lb_overzicht_systemen_en_detecties_drop_down_richting_ventilatie.lp_lb_overzicht_systemen_en_detecties_drop_down_richting_ventilatie);
    }
    ...
    public static boolean fillSyncData_evt_lb_overzicht_systemen_en_detecties_drop_down_richting_ventilatie_open_drop_down_menu(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(31).get(0);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(13);
        fillSyncData(aut, 31, rslt, state.s_lb_overzicht_systemen_en_detecties_drop_down_richting_ventilatie.lp_lb_overzicht_systemen_en_detecties_drop_down_richting_ventilatie);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_lb_overzicht_systemen_en_detecties_drop_down_richting_ventilatie_sluit_drop_down_menu(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(32).get(0);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(13);
        fillSyncData(aut, 32, rslt, state.s_lb_overzicht_systemen_en_detecties_drop_down_richting_ventilatie.lp_lb_overzicht_systemen_en_detecties_drop_down_richting_ventilatie);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_sluit_alle_tabs(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(157).get(9);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(13);
        fillSyncData(aut, 157, rslt, state.s_lb_overzicht_systemen_en_detecties_drop_down_richting_ventilatie.lp_lb_overzicht_systemen_en_detecties_drop_down_richting_ventilatie);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_lb_overzicht_systemen_en_detecties_drop_down_richting_ventilatie = target.s_lb_overzicht_systemen_en_detecties_drop_down_richting_ventilatie.copy();
        target.s_lb_overzicht_systemen_en_detecties_drop_down_richting_ventilatie.lp_lb_overzicht_systemen_en_detecties_drop_down_richting_ventilatie = value;
    }
}
