...
/** Automaton "lb_overzicht_systemen_en_detecties_blusvoorziening". */
public final class Aut_lb_overzicht_systemen_en_detecties_blusvoorziening extends RuntimeSimpleAutomaton<State> {
    public Aut_lb_overzicht_systemen_en_detecties_blusvoorziening() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_lb_overzicht_systemen_en_detecties_blusvoorziening", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "lb_overzicht_systemen_en_detecties_blusvoorziening";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_lb_overzicht_systemen_en_detecties_blusvoorziening.lp_lb_overzicht_systemen_en_detecties_blusvoorziening);
    }
    ...
    public static boolean fillSyncData_evt_lb_overzicht_systemen_en_detecties_blusvoorziening_open_drop_down_menu(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(21).get(0);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(8);
        fillSyncData(aut, 21, rslt, state.s_lb_overzicht_systemen_en_detecties_blusvoorziening.lp_lb_overzicht_systemen_en_detecties_blusvoorziening);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_lb_overzicht_systemen_en_detecties_blusvoorziening_sluit_drop_down_menu(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(22).get(0);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(8);
        fillSyncData(aut, 22, rslt, state.s_lb_overzicht_systemen_en_detecties_blusvoorziening.lp_lb_overzicht_systemen_en_detecties_blusvoorziening);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_sluit_alle_tabs(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(157).get(4);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(8);
        fillSyncData(aut, 157, rslt, state.s_lb_overzicht_systemen_en_detecties_blusvoorziening.lp_lb_overzicht_systemen_en_detecties_blusvoorziening);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_lb_overzicht_systemen_en_detecties_blusvoorziening = target.s_lb_overzicht_systemen_en_detecties_blusvoorziening.copy();
        target.s_lb_overzicht_systemen_en_detecties_blusvoorziening.lp_lb_overzicht_systemen_en_detecties_blusvoorziening = value;
    }
}
