...
/** Automaton "lb_overzicht_systemen_en_detecties_overig". */
public final class Aut_lb_overzicht_systemen_en_detecties_overig extends RuntimeSimpleAutomaton<State> {
    public Aut_lb_overzicht_systemen_en_detecties_overig() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_lb_overzicht_systemen_en_detecties_overig", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "lb_overzicht_systemen_en_detecties_overig";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_lb_overzicht_systemen_en_detecties_overig.lp_lb_overzicht_systemen_en_detecties_overig);
    }
    ...
    public static boolean fillSyncData_evt_lb_overzicht_systemen_en_detecties_overig_open_drop_down_menu(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(47).get(0);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(21);
        fillSyncData(aut, 47, rslt, state.s_lb_overzicht_systemen_en_detecties_overig.lp_lb_overzicht_systemen_en_detecties_overig);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_lb_overzicht_systemen_en_detecties_overig_sluit_drop_down_menu(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(48).get(0);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(21);
        fillSyncData(aut, 48, rslt, state.s_lb_overzicht_systemen_en_detecties_overig.lp_lb_overzicht_systemen_en_detecties_overig);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_sluit_alle_tabs(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(157).get(17);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(21);
        fillSyncData(aut, 157, rslt, state.s_lb_overzicht_systemen_en_detecties_overig.lp_lb_overzicht_systemen_en_detecties_overig);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_lb_overzicht_systemen_en_detecties_overig = target.s_lb_overzicht_systemen_en_detecties_overig.copy();
        target.s_lb_overzicht_systemen_en_detecties_overig.lp_lb_overzicht_systemen_en_detecties_overig = value;
    }
}
