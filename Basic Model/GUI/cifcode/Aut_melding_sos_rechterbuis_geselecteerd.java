...
/** Automaton "melding_sos_rechterbuis_geselecteerd". */
public final class Aut_melding_sos_rechterbuis_geselecteerd extends RuntimeSimpleAutomaton<State> {
    public Aut_melding_sos_rechterbuis_geselecteerd() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_melding_sos_rechterbuis_geselecteerd", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "melding_sos_rechterbuis_geselecteerd";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_melding_sos_rechterbuis_geselecteerd.lp_melding_sos_rechterbuis_geselecteerd);
    }
    ...
    public static boolean fillSyncData_evt_melding_sos_rechterbuis_geselecteerd_open_drop_down_menu(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(81).get(0);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(38);
        fillSyncData(aut, 81, rslt, state.s_melding_sos_rechterbuis_geselecteerd.lp_melding_sos_rechterbuis_geselecteerd);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_melding_sos_rechterbuis_geselecteerd_sluit_drop_down_menu(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(82).get(0);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(38);
        fillSyncData(aut, 82, rslt, state.s_melding_sos_rechterbuis_geselecteerd.lp_melding_sos_rechterbuis_geselecteerd);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_sluit_alle_tabs(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(157).get(34);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(38);
        fillSyncData(aut, 157, rslt, state.s_melding_sos_rechterbuis_geselecteerd.lp_melding_sos_rechterbuis_geselecteerd);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_melding_sos_rechterbuis_geselecteerd = target.s_melding_sos_rechterbuis_geselecteerd.copy();
        target.s_melding_sos_rechterbuis_geselecteerd.lp_melding_sos_rechterbuis_geselecteerd = value;
    }
}
