...
/** Automaton "Tunnel_Verkeersbuis2_Vluchtdeur_Geluidsbaken". */
public final class Aut_Tunnel_Verkeersbuis2_Vluchtdeur_Geluidsbaken extends RuntimeSimpleAutomaton<State> {
    public Aut_Tunnel_Verkeersbuis2_Vluchtdeur_Geluidsbaken() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_Tunnel_Verkeersbuis2_Vluchtdeur_Geluidsbaken", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "Tunnel_Verkeersbuis2_Vluchtdeur_Geluidsbaken";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_Tunnel_Verkeersbuis2_Vluchtdeur_Geluidsbaken.lp_Tunnel_Verkeersbuis2_Vluchtdeur_Geluidsbaken);
    }
    ...
    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis2_Vluchtdeur_Geluidsbaken_c_aan(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(1047).get(4);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(583);
        fillSyncData(aut, 1047, rslt, state.s_Tunnel_Verkeersbuis2_Vluchtdeur_Geluidsbaken.lp_Tunnel_Verkeersbuis2_Vluchtdeur_Geluidsbaken);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis2_Vluchtdeur_Geluidsbaken_c_onderdrukt(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(1048).get(4);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(583);
        fillSyncData(aut, 1048, rslt, state.s_Tunnel_Verkeersbuis2_Vluchtdeur_Geluidsbaken.lp_Tunnel_Verkeersbuis2_Vluchtdeur_Geluidsbaken);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis2_Vluchtdeur_Geluidsbaken_c_uit(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(1049).get(4);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(583);
        fillSyncData(aut, 1049, rslt, state.s_Tunnel_Verkeersbuis2_Vluchtdeur_Geluidsbaken.lp_Tunnel_Verkeersbuis2_Vluchtdeur_Geluidsbaken);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_Tunnel_Verkeersbuis2_Vluchtdeur_Geluidsbaken = target.s_Tunnel_Verkeersbuis2_Vluchtdeur_Geluidsbaken.copy();
        target.s_Tunnel_Verkeersbuis2_Vluchtdeur_Geluidsbaken.lp_Tunnel_Verkeersbuis2_Vluchtdeur_Geluidsbaken = value;
    }
}
