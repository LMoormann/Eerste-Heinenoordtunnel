...
/** Automaton "lb_bedienmenu_voor_verkeerslichtenstanden". */
public final class Aut_lb_bedienmenu_voor_verkeerslichtenstanden extends RuntimeSimpleAutomaton<State> {
    public Aut_lb_bedienmenu_voor_verkeerslichtenstanden() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_lb_bedienmenu_voor_verkeerslichtenstanden", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "lb_bedienmenu_voor_verkeerslichtenstanden";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_lb_bedienmenu_voor_verkeerslichtenstanden.lp_lb_bedienmenu_voor_verkeerslichtenstanden);
    }
    ...
    public static boolean fillSyncData_evt_lb_bedienmenu_voor_verkeerslichtenstanden_open_drop_down_menu(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(15).get(0);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(5);
        fillSyncData(aut, 15, rslt, state.s_lb_bedienmenu_voor_verkeerslichtenstanden.lp_lb_bedienmenu_voor_verkeerslichtenstanden);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_lb_bedienmenu_voor_verkeerslichtenstanden_sluit_drop_down_menu(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(16).get(0);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(5);
        fillSyncData(aut, 16, rslt, state.s_lb_bedienmenu_voor_verkeerslichtenstanden.lp_lb_bedienmenu_voor_verkeerslichtenstanden);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_sluit_alle_tabs(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(157).get(1);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(5);
        fillSyncData(aut, 157, rslt, state.s_lb_bedienmenu_voor_verkeerslichtenstanden.lp_lb_bedienmenu_voor_verkeerslichtenstanden);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_lb_bedienmenu_voor_verkeerslichtenstanden = target.s_lb_bedienmenu_voor_verkeerslichtenstanden.copy();
        target.s_lb_bedienmenu_voor_verkeerslichtenstanden.lp_lb_bedienmenu_voor_verkeerslichtenstanden = value;
    }
}
