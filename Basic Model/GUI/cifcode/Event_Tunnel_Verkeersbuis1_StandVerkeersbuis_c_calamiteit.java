...
/** Event "Tunnel_Verkeersbuis1_StandVerkeersbuis_c_calamiteit". */
public final class Event_Tunnel_Verkeersbuis1_StandVerkeersbuis_c_calamiteit extends RuntimeEvent<State> {
    public Event_Tunnel_Verkeersbuis1_StandVerkeersbuis_c_calamiteit() {
        super("Tunnel_Verkeersbuis1_StandVerkeersbuis_c_calamiteit", 611, true);
    }
    ...
    public boolean fillData(State state) {
        ...
        proceed = Aut_sup31.fillSyncData_evt_Tunnel_Verkeersbuis1_StandVerkeersbuis_c_calamiteit(state);
        if (!proceed) return false;

        // Check automaton "sup35".
        proceed = Aut_sup35.fillSyncData_evt_Tunnel_Verkeersbuis1_StandVerkeersbuis_c_calamiteit(state);
        if (!proceed) return false;

        // Check automaton "sup39".
        proceed = Aut_sup39.fillSyncData_evt_Tunnel_Verkeersbuis1_StandVerkeersbuis_c_calamiteit(state);
        if (!proceed) return false;

        // Check automaton "sup6".
        proceed = Aut_sup6.fillSyncData_evt_Tunnel_Verkeersbuis1_StandVerkeersbuis_c_calamiteit(state);
        if (!proceed) return false;

        // Check automaton "sup7".
        proceed = Aut_sup7.fillSyncData_evt_Tunnel_Verkeersbuis1_StandVerkeersbuis_c_calamiteit(state);
        if (!proceed) return false;

        // Check automaton "Tunnel_Verkeersbuis1_StandVerkeersbuis".
        proceed = Aut_Tunnel_Verkeersbuis1_StandVerkeersbuis.fillSyncData_evt_Tunnel_Verkeersbuis1_StandVerkeersbuis_c_calamiteit(state);
        if (!proceed) return false;
    ...
    public boolean allowedByInvs(State state) {
        ...
        try {
            ...
            throw new CifSimulatorException("Evaluation of invariant \"Tunnel_Verkeersbuis2_StandVerkeersbuis.calamiteit disables Tunnel_Verkeersbuis1_StandVerkeersbuis_c_calamiteit\" of the top level scope of the specification failed.", e, state);
        }

        try {
            if (!(((state.s_Tunnel_Verkeersbuis1_KnopCalamiteit_Waarnemer.lp_Tunnel_Verkeersbuis1_KnopCalamiteit_Waarnemer == 1) || (state.s_Tunnel_Verkeersbuis1_PopUp_KnopBevestigen_Waarnemer.lp_Tunnel_Verkeersbuis1_PopUp_KnopBevestigen_Waarnemer == 1)) || (state.s_Tunnel_Verkeersbuis1_PopUp_Timer.lp_Tunnel_Verkeersbuis1_PopUp_Timer == 2))) return false;
        } catch (CifSimulatorException e) {
            throw new CifSimulatorException("Evaluation of invariant \"Tunnel_Verkeersbuis1_StandVerkeersbuis_c_calamiteit needs Tunnel_Verkeersbuis1_KnopCalamiteit_Waarnemer.ingedrukt or Tunnel_Verkeersbuis1_PopUp_KnopBevestigen_Waarnemer.ingedrukt or Tunnel_Verkeersbuis1_PopUp_Timer.afgelopen\" of the top level scope of the specification failed.", e, state);
        }
