...
/** Automaton "Tunnel_MiddenPompkelder_SensorLaagwater". */
public final class Aut_Tunnel_MiddenPompkelder_SensorLaagwater extends RuntimeSimpleAutomaton<State> {
    public Aut_Tunnel_MiddenPompkelder_SensorLaagwater() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_Tunnel_MiddenPompkelder_SensorLaagwater", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "Tunnel_MiddenPompkelder_SensorLaagwater";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_Tunnel_MiddenPompkelder_SensorLaagwater.lp_Tunnel_MiddenPompkelder_SensorLaagwater);
    }
    ...
    public static boolean fillSyncData_evt_Tunnel_MiddenPompkelder_SensorLaagwater_u_aan(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(278).get(1);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(188);
        fillSyncData(aut, 278, rslt, state.s_Tunnel_MiddenPompkelder_SensorLaagwater.lp_Tunnel_MiddenPompkelder_SensorLaagwater);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_MiddenPompkelder_SensorLaagwater_u_uit(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(279).get(2);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(188);
        fillSyncData(aut, 279, rslt, state.s_Tunnel_MiddenPompkelder_SensorLaagwater.lp_Tunnel_MiddenPompkelder_SensorLaagwater);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_Tunnel_MiddenPompkelder_SensorLaagwater = target.s_Tunnel_MiddenPompkelder_SensorLaagwater.copy();
        target.s_Tunnel_MiddenPompkelder_SensorLaagwater.lp_Tunnel_MiddenPompkelder_SensorLaagwater = value;
    }
}
