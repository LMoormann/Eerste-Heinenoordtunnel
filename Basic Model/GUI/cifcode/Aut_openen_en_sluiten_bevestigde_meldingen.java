...
/** Automaton "openen_en_sluiten_bevestigde_meldingen". */
public final class Aut_openen_en_sluiten_bevestigde_meldingen extends RuntimeSimpleAutomaton<State> {
    public Aut_openen_en_sluiten_bevestigde_meldingen() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_openen_en_sluiten_bevestigde_meldingen", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "openen_en_sluiten_bevestigde_meldingen";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_openen_en_sluiten_bevestigde_meldingen.lp_openen_en_sluiten_bevestigde_meldingen);
    }
    ...
    public static boolean fillSyncData_evt_openen_en_sluiten_bevestigde_meldingen_open_drop_down_menu(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(83).get(0);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(39);
        fillSyncData(aut, 83, rslt, state.s_openen_en_sluiten_bevestigde_meldingen.lp_openen_en_sluiten_bevestigde_meldingen);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_openen_en_sluiten_bevestigde_meldingen_sluit_drop_down_menu(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(84).get(0);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(39);
        fillSyncData(aut, 84, rslt, state.s_openen_en_sluiten_bevestigde_meldingen.lp_openen_en_sluiten_bevestigde_meldingen);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_sluit_alle_tabs(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(157).get(35);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(39);
        fillSyncData(aut, 157, rslt, state.s_openen_en_sluiten_bevestigde_meldingen.lp_openen_en_sluiten_bevestigde_meldingen);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_openen_en_sluiten_bevestigde_meldingen = target.s_openen_en_sluiten_bevestigde_meldingen.copy();
        target.s_openen_en_sluiten_bevestigde_meldingen.lp_openen_en_sluiten_bevestigde_meldingen = value;
    }
}
