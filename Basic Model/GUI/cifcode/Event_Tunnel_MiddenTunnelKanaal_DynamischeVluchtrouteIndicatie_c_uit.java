...
/** Event "Tunnel_MiddenTunnelKanaal_DynamischeVluchtrouteIndicatie_c_uit". */
public final class Event_Tunnel_MiddenTunnelKanaal_DynamischeVluchtrouteIndicatie_c_uit extends RuntimeEvent<State> {
    public Event_Tunnel_MiddenTunnelKanaal_DynamischeVluchtrouteIndicatie_c_uit() {
        super("Tunnel_MiddenTunnelKanaal_DynamischeVluchtrouteIndicatie_c_uit", 310, true);
    }
    ...
    public boolean fillData(State state) {
        ...
        proceed = Aut_sup35.fillSyncData_evt_Tunnel_MiddenTunnelKanaal_DynamischeVluchtrouteIndicatie_c_uit(state);
        if (!proceed) return false;

        // Check automaton "sup43".
        proceed = Aut_sup43.fillSyncData_evt_Tunnel_MiddenTunnelKanaal_DynamischeVluchtrouteIndicatie_c_uit(state);
        if (!proceed) return false;

        // Check automaton "sup6".
        proceed = Aut_sup6.fillSyncData_evt_Tunnel_MiddenTunnelKanaal_DynamischeVluchtrouteIndicatie_c_uit(state);
        if (!proceed) return false;

        // Check automaton "Tunnel_MiddenTunnelKanaal_DynamischeVluchtrouteIndicatie".
        proceed = Aut_Tunnel_MiddenTunnelKanaal_DynamischeVluchtrouteIndicatie.fillSyncData_evt_Tunnel_MiddenTunnelKanaal_DynamischeVluchtrouteIndicatie_c_uit(state);
        if (!proceed) return false;
    ...
    public boolean allowedByInvs(State state) {
        // Invariants for "".
        try {
            if (!((((state.s_Tunnel_MiddenTunnelKanaal_BedieningDynamischeVluchtrouteIndicatie_Modus.lp_Tunnel_MiddenTunnelKanaal_BedieningDynamischeVluchtrouteIndicatie_Modus == 0) && (state.s_Tunnel_Verkeersbuis1_Evacuatie.lp_Tunnel_Verkeersbuis1_Evacuatie == 0)) && (state.s_Tunnel_Verkeersbuis2_Evacuatie.lp_Tunnel_Verkeersbuis2_Evacuatie == 0)) || ((state.s_Tunnel_MiddenTunnelKanaal_BedieningDynamischeVluchtrouteIndicatie_Modus.lp_Tunnel_MiddenTunnelKanaal_BedieningDynamischeVluchtrouteIndicatie_Modus == 1) && (state.s_Tunnel_MiddenTunnelKanaal_KnopDynamischeVluchtrouteIndicatieUit_Waarnemer.lp_Tunnel_MiddenTunnelKanaal_KnopDynamischeVluchtrouteIndicatieUit_Waarnemer == 1)))) return false;
        } catch (CifSimulatorException e) {
            throw new CifSimulatorException("Evaluation of invariant \"Tunnel_MiddenTunnelKanaal_DynamischeVluchtrouteIndicatie_c_uit needs Tunnel_MiddenTunnelKanaal_BedieningDynamischeVluchtrouteIndicatie_Modus.auto and Tunnel_Verkeersbuis1_Evacuatie.uit and Tunnel_Verkeersbuis2_Evacuatie.uit or Tunnel_MiddenTunnelKanaal_BedieningDynamischeVluchtrouteIndicatie_Modus.hand and Tunnel_MiddenTunnelKanaal_KnopDynamischeVluchtrouteIndicatieUit_Waarnemer.ingedrukt\" of the top level scope of the specification failed.", e, state);
        }
