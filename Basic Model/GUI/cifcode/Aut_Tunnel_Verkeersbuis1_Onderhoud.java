...
/** Automaton "Tunnel_Verkeersbuis1_Onderhoud". */
public final class Aut_Tunnel_Verkeersbuis1_Onderhoud extends RuntimeSimpleAutomaton<State> {
    public Aut_Tunnel_Verkeersbuis1_Onderhoud() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_Tunnel_Verkeersbuis1_Onderhoud", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "Tunnel_Verkeersbuis1_Onderhoud";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_Tunnel_Verkeersbuis1_Onderhoud.lp_Tunnel_Verkeersbuis1_Onderhoud);
    }
    ...
    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis1_Onderhoud_c_aan(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(583).get(5);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(361);
        fillSyncData(aut, 583, rslt, state.s_Tunnel_Verkeersbuis1_Onderhoud.lp_Tunnel_Verkeersbuis1_Onderhoud);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis1_Onderhoud_c_uit(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(584).get(5);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(361);
        fillSyncData(aut, 584, rslt, state.s_Tunnel_Verkeersbuis1_Onderhoud.lp_Tunnel_Verkeersbuis1_Onderhoud);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_Tunnel_Verkeersbuis1_Onderhoud = target.s_Tunnel_Verkeersbuis1_Onderhoud.copy();
        target.s_Tunnel_Verkeersbuis1_Onderhoud.lp_Tunnel_Verkeersbuis1_Onderhoud = value;
    }
}
