...
/** Automaton "Tunnel_Verkeersbuis2_Afsluitboom1_ActuatorOp". */
public final class Aut_Tunnel_Verkeersbuis2_Afsluitboom1_ActuatorOp extends RuntimeSimpleAutomaton<State> {
    public Aut_Tunnel_Verkeersbuis2_Afsluitboom1_ActuatorOp() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_Tunnel_Verkeersbuis2_Afsluitboom1_ActuatorOp", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "Tunnel_Verkeersbuis2_Afsluitboom1_ActuatorOp";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_Tunnel_Verkeersbuis2_Afsluitboom1_ActuatorOp.lp_Tunnel_Verkeersbuis2_Afsluitboom1_ActuatorOp);
    }
    ...
    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis2_Afsluitboom1_ActuatorOp_c_aan(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(712).get(4);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(413);
        fillSyncData(aut, 712, rslt, state.s_Tunnel_Verkeersbuis2_Afsluitboom1_ActuatorOp.lp_Tunnel_Verkeersbuis2_Afsluitboom1_ActuatorOp);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis2_Afsluitboom1_ActuatorOp_c_uit(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(713).get(4);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(413);
        fillSyncData(aut, 713, rslt, state.s_Tunnel_Verkeersbuis2_Afsluitboom1_ActuatorOp.lp_Tunnel_Verkeersbuis2_Afsluitboom1_ActuatorOp);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis2_Afsluitboom1_ActuatorOp_c_uitNood(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(714).get(4);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(413);
        fillSyncData(aut, 714, rslt, state.s_Tunnel_Verkeersbuis2_Afsluitboom1_ActuatorOp.lp_Tunnel_Verkeersbuis2_Afsluitboom1_ActuatorOp);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_Tunnel_Verkeersbuis2_Afsluitboom1_ActuatorOp = target.s_Tunnel_Verkeersbuis2_Afsluitboom1_ActuatorOp.copy();
        target.s_Tunnel_Verkeersbuis2_Afsluitboom1_ActuatorOp.lp_Tunnel_Verkeersbuis2_Afsluitboom1_ActuatorOp = value;
    }
}
