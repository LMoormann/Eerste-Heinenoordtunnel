...
/** Automaton "Tunnel_Verkeersbuis1_Standby". */
public final class Aut_Tunnel_Verkeersbuis1_Standby extends RuntimeSimpleAutomaton<State> {
    public Aut_Tunnel_Verkeersbuis1_Standby() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_Tunnel_Verkeersbuis1_Standby", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "Tunnel_Verkeersbuis1_Standby";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_Tunnel_Verkeersbuis1_Standby.lp_Tunnel_Verkeersbuis1_Standby);
    }
    ...
    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis1_Standby_c_aan(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(609).get(5);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(372);
        fillSyncData(aut, 609, rslt, state.s_Tunnel_Verkeersbuis1_Standby.lp_Tunnel_Verkeersbuis1_Standby);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis1_Standby_c_uit(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(610).get(5);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(372);
        fillSyncData(aut, 610, rslt, state.s_Tunnel_Verkeersbuis1_Standby.lp_Tunnel_Verkeersbuis1_Standby);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_Tunnel_Verkeersbuis1_Standby = target.s_Tunnel_Verkeersbuis1_Standby.copy();
        target.s_Tunnel_Verkeersbuis1_Standby.lp_Tunnel_Verkeersbuis1_Standby = value;
    }
}
