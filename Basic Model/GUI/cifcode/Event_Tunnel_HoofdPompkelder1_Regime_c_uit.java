...
/** Event "Tunnel_HoofdPompkelder1_Regime_c_uit". */
public final class Event_Tunnel_HoofdPompkelder1_Regime_c_uit extends RuntimeEvent<State> {
    public Event_Tunnel_HoofdPompkelder1_Regime_c_uit() {
        super("Tunnel_HoofdPompkelder1_Regime_c_uit", 203, true);
    }
    ...
    public boolean fillData(State state) {
        ...
        proceed = Aut_sup6.fillSyncData_evt_Tunnel_HoofdPompkelder1_Regime_c_uit(state);
        if (!proceed) return false;

        // Check automaton "sup89".
        proceed = Aut_sup89.fillSyncData_evt_Tunnel_HoofdPompkelder1_Regime_c_uit(state);
        if (!proceed) return false;

        // Check automaton "sup92".
        proceed = Aut_sup92.fillSyncData_evt_Tunnel_HoofdPompkelder1_Regime_c_uit(state);
        if (!proceed) return false;

        // Check automaton "Tunnel_HoofdPompkelder1_Regime".
        proceed = Aut_Tunnel_HoofdPompkelder1_Regime.fillSyncData_evt_Tunnel_HoofdPompkelder1_Regime_c_uit(state);
        if (!proceed) return false;
    ...
    public boolean allowedByInvs(State state) {
        // Invariants for "".
        try {
            if (!((state.s_Tunnel_HoofdPompkelder1_Bediening_Modus.lp_Tunnel_HoofdPompkelder1_Bediening_Modus == 1) && (state.s_Tunnel_HoofdPompkelder1_KnopUit_Waarnemer.lp_Tunnel_HoofdPompkelder1_KnopUit_Waarnemer == 1))) return false;
        } catch (CifSimulatorException e) {
            throw new CifSimulatorException("Evaluation of invariant \"Tunnel_HoofdPompkelder1_Regime_c_uit needs Tunnel_HoofdPompkelder1_Bediening_Modus.hand and Tunnel_HoofdPompkelder1_KnopUit_Waarnemer.ingedrukt\" of the top level scope of the specification failed.", e, state);
        }
