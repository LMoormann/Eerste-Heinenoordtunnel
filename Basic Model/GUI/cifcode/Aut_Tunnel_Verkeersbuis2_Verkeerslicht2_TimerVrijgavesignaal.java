...
/** Automaton "Tunnel_Verkeersbuis2_Verkeerslicht2_TimerVrijgavesignaal". */
public final class Aut_Tunnel_Verkeersbuis2_Verkeerslicht2_TimerVrijgavesignaal extends RuntimeSimpleAutomaton<State> {
    public Aut_Tunnel_Verkeersbuis2_Verkeerslicht2_TimerVrijgavesignaal() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_Tunnel_Verkeersbuis2_Verkeerslicht2_TimerVrijgavesignaal", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "Tunnel_Verkeersbuis2_Verkeerslicht2_TimerVrijgavesignaal";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_Tunnel_Verkeersbuis2_Verkeerslicht2_TimerVrijgavesignaal.lp_Tunnel_Verkeersbuis2_Verkeerslicht2_TimerVrijgavesignaal);
    }
    ...
    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis2_Verkeerslicht2_TimerVrijgavesignaal_c_aan(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(1006).get(4);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(565);
        fillSyncData(aut, 1006, rslt, state.s_Tunnel_Verkeersbuis2_Verkeerslicht2_TimerVrijgavesignaal.lp_Tunnel_Verkeersbuis2_Verkeerslicht2_TimerVrijgavesignaal);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis2_Verkeerslicht2_TimerVrijgavesignaal_c_reset(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(1007).get(4);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(565);
        fillSyncData(aut, 1007, rslt, state.s_Tunnel_Verkeersbuis2_Verkeerslicht2_TimerVrijgavesignaal.lp_Tunnel_Verkeersbuis2_Verkeerslicht2_TimerVrijgavesignaal);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis2_Verkeerslicht2_TimerVrijgavesignaal_u_timeout(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(1008).get(1);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(565);
        fillSyncData(aut, 1008, rslt, state.s_Tunnel_Verkeersbuis2_Verkeerslicht2_TimerVrijgavesignaal.lp_Tunnel_Verkeersbuis2_Verkeerslicht2_TimerVrijgavesignaal);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_Tunnel_Verkeersbuis2_Verkeerslicht2_TimerVrijgavesignaal = target.s_Tunnel_Verkeersbuis2_Verkeerslicht2_TimerVrijgavesignaal.copy();
        target.s_Tunnel_Verkeersbuis2_Verkeerslicht2_TimerVrijgavesignaal.lp_Tunnel_Verkeersbuis2_Verkeerslicht2_TimerVrijgavesignaal = value;
    }
}
