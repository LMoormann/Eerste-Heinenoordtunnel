...
/** Automaton "Tunnel_BrandBlusInstallatie_SensorLaag". */
public final class Aut_Tunnel_BrandBlusInstallatie_SensorLaag extends RuntimeSimpleAutomaton<State> {
    public Aut_Tunnel_BrandBlusInstallatie_SensorLaag() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_Tunnel_BrandBlusInstallatie_SensorLaag", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "Tunnel_BrandBlusInstallatie_SensorLaag";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_Tunnel_BrandBlusInstallatie_SensorLaag.lp_Tunnel_BrandBlusInstallatie_SensorLaag);
    }
    ...
    public static boolean fillSyncData_evt_Tunnel_BrandBlusInstallatie_SensorLaag_u_aan(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(178).get(1);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(129);
        fillSyncData(aut, 178, rslt, state.s_Tunnel_BrandBlusInstallatie_SensorLaag.lp_Tunnel_BrandBlusInstallatie_SensorLaag);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_BrandBlusInstallatie_SensorLaag_u_uit(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(179).get(2);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(129);
        fillSyncData(aut, 179, rslt, state.s_Tunnel_BrandBlusInstallatie_SensorLaag.lp_Tunnel_BrandBlusInstallatie_SensorLaag);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_Tunnel_BrandBlusInstallatie_SensorLaag = target.s_Tunnel_BrandBlusInstallatie_SensorLaag.copy();
        target.s_Tunnel_BrandBlusInstallatie_SensorLaag.lp_Tunnel_BrandBlusInstallatie_SensorLaag = value;
    }
}
