...
/** Event "Tunnel_Verkeersbuis2_Evacuatie_c_aan". */
public final class Event_Tunnel_Verkeersbuis2_Evacuatie_c_aan extends RuntimeEvent<State> {
    public Event_Tunnel_Verkeersbuis2_Evacuatie_c_aan() {
        super("Tunnel_Verkeersbuis2_Evacuatie_c_aan", 765, true);
    }
    ...
    public boolean fillData(State state) {
        ...
        proceed = Aut_sup48.fillSyncData_evt_Tunnel_Verkeersbuis2_Evacuatie_c_aan(state);
        if (!proceed) return false;

        // Check automaton "sup6".
        proceed = Aut_sup6.fillSyncData_evt_Tunnel_Verkeersbuis2_Evacuatie_c_aan(state);
        if (!proceed) return false;

        // Check automaton "sup72".
        proceed = Aut_sup72.fillSyncData_evt_Tunnel_Verkeersbuis2_Evacuatie_c_aan(state);
        if (!proceed) return false;

        // Check automaton "sup76".
        proceed = Aut_sup76.fillSyncData_evt_Tunnel_Verkeersbuis2_Evacuatie_c_aan(state);
        if (!proceed) return false;

        // Check automaton "sup80".
        proceed = Aut_sup80.fillSyncData_evt_Tunnel_Verkeersbuis2_Evacuatie_c_aan(state);
        if (!proceed) return false;

        // Check automaton "Tunnel_Verkeersbuis2_Evacuatie".
        proceed = Aut_Tunnel_Verkeersbuis2_Evacuatie.fillSyncData_evt_Tunnel_Verkeersbuis2_Evacuatie_c_aan(state);
        if (!proceed) return false;
    ...
    public boolean allowedByInvs(State state) {
        // Invariants for "".
        try {
            if (!(state.s_Tunnel_Verkeersbuis1_StandVerkeersbuis.lp_Tunnel_Verkeersbuis1_StandVerkeersbuis == 3)) return false;
        } catch (CifSimulatorException e) {
            throw new CifSimulatorException("Evaluation of invariant \"Tunnel_Verkeersbuis2_Evacuatie_c_aan needs Tunnel_Verkeersbuis1_StandVerkeersbuis.ondersteunend\" of the top level scope of the specification failed.", e, state);
        }

        try {
            if (!(state.s_Tunnel_Verkeersbuis2_KnopEvacuatie_Waarnemer.lp_Tunnel_Verkeersbuis2_KnopEvacuatie_Waarnemer == 1)) return false;
        } catch (CifSimulatorException e) {
            throw new CifSimulatorException("Evaluation of invariant \"Tunnel_Verkeersbuis2_Evacuatie_c_aan needs Tunnel_Verkeersbuis2_KnopEvacuatie_Waarnemer.ingedrukt\" of the top level scope of the specification failed.", e, state);
        }
