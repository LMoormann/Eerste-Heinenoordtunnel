...
/** Automaton "Tunnel_MiddenTunnelKanaal_BedieningVerlichting_KnopHand_Knop". */
public final class Aut_Tunnel_MiddenTunnelKanaal_BedieningVerlichting_KnopHand_Knop extends RuntimeSimpleAutomaton<State> {
    public Aut_Tunnel_MiddenTunnelKanaal_BedieningVerlichting_KnopHand_Knop() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_Tunnel_MiddenTunnelKanaal_BedieningVerlichting_KnopHand_Knop", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "Tunnel_MiddenTunnelKanaal_BedieningVerlichting_KnopHand_Knop";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_Tunnel_MiddenTunnelKanaal_BedieningVerlichting_KnopHand_Knop.lp_Tunnel_MiddenTunnelKanaal_BedieningVerlichting_KnopHand_Knop);
    }
    ...
    public static boolean fillSyncData_evt_Tunnel_MiddenTunnelKanaal_BedieningVerlichting_KnopHand_Knop_u_indrukken(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(303).get(0);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(203);
        fillSyncData(aut, 303, rslt, state.s_Tunnel_MiddenTunnelKanaal_BedieningVerlichting_KnopHand_Knop.lp_Tunnel_MiddenTunnelKanaal_BedieningVerlichting_KnopHand_Knop);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_Tunnel_MiddenTunnelKanaal_BedieningVerlichting_KnopHand_Knop = target.s_Tunnel_MiddenTunnelKanaal_BedieningVerlichting_KnopHand_Knop.copy();
        target.s_Tunnel_MiddenTunnelKanaal_BedieningVerlichting_KnopHand_Knop.lp_Tunnel_MiddenTunnelKanaal_BedieningVerlichting_KnopHand_Knop = value;
    }
}
