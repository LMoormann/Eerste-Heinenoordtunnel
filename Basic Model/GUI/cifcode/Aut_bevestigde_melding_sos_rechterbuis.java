...
/** Automaton "bevestigde_melding_sos_rechterbuis". */
public final class Aut_bevestigde_melding_sos_rechterbuis extends RuntimeSimpleAutomaton<State> {
    public Aut_bevestigde_melding_sos_rechterbuis() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_bevestigde_melding_sos_rechterbuis", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "bevestigde_melding_sos_rechterbuis";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_bevestigde_melding_sos_rechterbuis.lp_bevestigde_melding_sos_rechterbuis);
    }
    ...
    public static boolean fillSyncData_evt_bevestigde_melding_sos_rechterbuis_open_drop_down_menu(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(9).get(0);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(1);
        fillSyncData(aut, 9, rslt, state.s_bevestigde_melding_sos_rechterbuis.lp_bevestigde_melding_sos_rechterbuis);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_bevestigde_melding_sos_rechterbuis_sluit_drop_down_menu(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(10).get(0);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(1);
        fillSyncData(aut, 10, rslt, state.s_bevestigde_melding_sos_rechterbuis.lp_bevestigde_melding_sos_rechterbuis);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_sluit_alle_tabs(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(157).get(0);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(1);
        fillSyncData(aut, 157, rslt, state.s_bevestigde_melding_sos_rechterbuis.lp_bevestigde_melding_sos_rechterbuis);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_bevestigde_melding_sos_rechterbuis = target.s_bevestigde_melding_sos_rechterbuis.copy();
        target.s_bevestigde_melding_sos_rechterbuis.lp_bevestigde_melding_sos_rechterbuis = value;
    }
}
