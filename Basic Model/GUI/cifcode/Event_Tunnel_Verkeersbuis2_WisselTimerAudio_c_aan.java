...
/** Event "Tunnel_Verkeersbuis2_WisselTimerAudio_c_aan". */
public final class Event_Tunnel_Verkeersbuis2_WisselTimerAudio_c_aan extends RuntimeEvent<State> {
    public Event_Tunnel_Verkeersbuis2_WisselTimerAudio_c_aan() {
        super("Tunnel_Verkeersbuis2_WisselTimerAudio_c_aan", 1064, true);
    }
    ...
    public boolean fillData(State state) {
        ...
        proceed = Aut_sup6.fillSyncData_evt_Tunnel_Verkeersbuis2_WisselTimerAudio_c_aan(state);
        if (!proceed) return false;

        // Check automaton "sup72".
        proceed = Aut_sup72.fillSyncData_evt_Tunnel_Verkeersbuis2_WisselTimerAudio_c_aan(state);
        if (!proceed) return false;

        // Check automaton "sup82".
        proceed = Aut_sup82.fillSyncData_evt_Tunnel_Verkeersbuis2_WisselTimerAudio_c_aan(state);
        if (!proceed) return false;

        // Check automaton "sup84".
        proceed = Aut_sup84.fillSyncData_evt_Tunnel_Verkeersbuis2_WisselTimerAudio_c_aan(state);
        if (!proceed) return false;

        // Check automaton "Tunnel_Verkeersbuis2_WisselTimerAudio".
        proceed = Aut_Tunnel_Verkeersbuis2_WisselTimerAudio.fillSyncData_evt_Tunnel_Verkeersbuis2_WisselTimerAudio_c_aan(state);
        if (!proceed) return false;
    ...
    public boolean allowedByInvs(State state) {
        // Invariants for "".
        try {
            if (!(((state.s_Tunnel_Verkeersbuis2_Evacuatie.lp_Tunnel_Verkeersbuis2_Evacuatie == 1) || (((!(state.s_Tunnel_Verkeersbuis2_Luidspreker.lp_Tunnel_Verkeersbuis2_Luidspreker == 0)) && (state.s_Tunnel_Verkeersbuis2_Vluchtdeur_BedieningGeluidsbaken_Modus.lp_Tunnel_Verkeersbuis2_Vluchtdeur_BedieningGeluidsbaken_Modus == 1)) && (state.s_Tunnel_Verkeersbuis2_Vluchtdeur_KnopGeluidsbakenAan_Waarnemer.lp_Tunnel_Verkeersbuis2_Vluchtdeur_KnopGeluidsbakenAan_Waarnemer == 1))) || ((!(state.s_Tunnel_Verkeersbuis2_Vluchtdeur_Geluidsbaken.lp_Tunnel_Verkeersbuis2_Vluchtdeur_Geluidsbaken == 0)) && ((state.s_Tunnel_Verkeersbuis2_KnopStartLive_Waarnemer.lp_Tunnel_Verkeersbuis2_KnopStartLive_Waarnemer == 1) || (state.s_Tunnel_Verkeersbuis2_KnopStartBoodschap_Waarnemer.lp_Tunnel_Verkeersbuis2_KnopStartBoodschap_Waarnemer == 1))))) return false;
        } catch (CifSimulatorException e) {
            throw new CifSimulatorException("Evaluation of invariant \"Tunnel_Verkeersbuis2_WisselTimerAudio_c_aan needs Tunnel_Verkeersbuis2_Evacuatie.aan or not Tunnel_Verkeersbuis2_Luidspreker.uit and Tunnel_Verkeersbuis2_Vluchtdeur_BedieningGeluidsbaken_Modus.hand and Tunnel_Verkeersbuis2_Vluchtdeur_KnopGeluidsbakenAan_Waarnemer.ingedrukt or not Tunnel_Verkeersbuis2_Vluchtdeur_Geluidsbaken.uit and (Tunnel_Verkeersbuis2_KnopStartLive_Waarnemer.ingedrukt or Tunnel_Verkeersbuis2_KnopStartBoodschap_Waarnemer.ingedrukt)\" of the top level scope of the specification failed.", e, state);
        }
