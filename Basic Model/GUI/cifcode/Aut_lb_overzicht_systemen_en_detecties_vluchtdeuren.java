...
/** Automaton "lb_overzicht_systemen_en_detecties_vluchtdeuren". */
public final class Aut_lb_overzicht_systemen_en_detecties_vluchtdeuren extends RuntimeSimpleAutomaton<State> {
    public Aut_lb_overzicht_systemen_en_detecties_vluchtdeuren() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_lb_overzicht_systemen_en_detecties_vluchtdeuren", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "lb_overzicht_systemen_en_detecties_vluchtdeuren";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_lb_overzicht_systemen_en_detecties_vluchtdeuren.lp_lb_overzicht_systemen_en_detecties_vluchtdeuren);
    }
    ...
    public static boolean fillSyncData_evt_lb_overzicht_systemen_en_detecties_vluchtdeuren_open_drop_down_menu(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(69).get(0);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(32);
        fillSyncData(aut, 69, rslt, state.s_lb_overzicht_systemen_en_detecties_vluchtdeuren.lp_lb_overzicht_systemen_en_detecties_vluchtdeuren);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_lb_overzicht_systemen_en_detecties_vluchtdeuren_sluit_drop_down_menu(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(70).get(0);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(32);
        fillSyncData(aut, 70, rslt, state.s_lb_overzicht_systemen_en_detecties_vluchtdeuren.lp_lb_overzicht_systemen_en_detecties_vluchtdeuren);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_sluit_alle_tabs(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(157).get(28);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(32);
        fillSyncData(aut, 157, rslt, state.s_lb_overzicht_systemen_en_detecties_vluchtdeuren.lp_lb_overzicht_systemen_en_detecties_vluchtdeuren);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_lb_overzicht_systemen_en_detecties_vluchtdeuren = target.s_lb_overzicht_systemen_en_detecties_vluchtdeuren.copy();
        target.s_lb_overzicht_systemen_en_detecties_vluchtdeuren.lp_lb_overzicht_systemen_en_detecties_vluchtdeuren = value;
    }
}
