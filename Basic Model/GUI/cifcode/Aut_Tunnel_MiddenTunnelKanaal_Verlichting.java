...
/** Automaton "Tunnel_MiddenTunnelKanaal_Verlichting". */
public final class Aut_Tunnel_MiddenTunnelKanaal_Verlichting extends RuntimeSimpleAutomaton<State> {
    public Aut_Tunnel_MiddenTunnelKanaal_Verlichting() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_Tunnel_MiddenTunnelKanaal_Verlichting", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "Tunnel_MiddenTunnelKanaal_Verlichting";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_Tunnel_MiddenTunnelKanaal_Verlichting.lp_Tunnel_MiddenTunnelKanaal_Verlichting);
    }
    ...
    public static boolean fillSyncData_evt_Tunnel_MiddenTunnelKanaal_Verlichting_c_aan(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(349).get(2);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(229);
        fillSyncData(aut, 349, rslt, state.s_Tunnel_MiddenTunnelKanaal_Verlichting.lp_Tunnel_MiddenTunnelKanaal_Verlichting);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_MiddenTunnelKanaal_Verlichting_c_uit(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(350).get(2);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(229);
        fillSyncData(aut, 350, rslt, state.s_Tunnel_MiddenTunnelKanaal_Verlichting.lp_Tunnel_MiddenTunnelKanaal_Verlichting);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_Tunnel_MiddenTunnelKanaal_Verlichting = target.s_Tunnel_MiddenTunnelKanaal_Verlichting.copy();
        target.s_Tunnel_MiddenTunnelKanaal_Verlichting.lp_Tunnel_MiddenTunnelKanaal_Verlichting = value;
    }
}
