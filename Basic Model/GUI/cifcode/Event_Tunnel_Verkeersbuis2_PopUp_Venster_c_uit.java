...
/** Event "Tunnel_Verkeersbuis2_PopUp_Venster_c_uit". */
public final class Event_Tunnel_Verkeersbuis2_PopUp_Venster_c_uit extends RuntimeEvent<State> {
    public Event_Tunnel_Verkeersbuis2_PopUp_Venster_c_uit() {
        super("Tunnel_Verkeersbuis2_PopUp_Venster_c_uit", 953, true);
    }
    ...
    public boolean fillData(State state) {
        ...
        proceed = Aut_sup48.fillSyncData_evt_Tunnel_Verkeersbuis2_PopUp_Venster_c_uit(state);
        if (!proceed) return false;

        // Check automaton "sup6".
        proceed = Aut_sup6.fillSyncData_evt_Tunnel_Verkeersbuis2_PopUp_Venster_c_uit(state);
        if (!proceed) return false;

        // Check automaton "sup72".
        proceed = Aut_sup72.fillSyncData_evt_Tunnel_Verkeersbuis2_PopUp_Venster_c_uit(state);
        if (!proceed) return false;

        // Check automaton "sup76".
        proceed = Aut_sup76.fillSyncData_evt_Tunnel_Verkeersbuis2_PopUp_Venster_c_uit(state);
        if (!proceed) return false;

        // Check automaton "sup80".
        proceed = Aut_sup80.fillSyncData_evt_Tunnel_Verkeersbuis2_PopUp_Venster_c_uit(state);
        if (!proceed) return false;

        // Check automaton "Tunnel_Verkeersbuis2_PopUp_Venster".
        proceed = Aut_Tunnel_Verkeersbuis2_PopUp_Venster.fillSyncData_evt_Tunnel_Verkeersbuis2_PopUp_Venster_c_uit(state);
        if (!proceed) return false;
    ...
    public boolean allowedByInvs(State state) {
        // Invariants for "".
        try {
            if (!(((state.s_Tunnel_Verkeersbuis2_PopUp_KnopBevestigen_Waarnemer.lp_Tunnel_Verkeersbuis2_PopUp_KnopBevestigen_Waarnemer == 1) || (state.s_Tunnel_Verkeersbuis2_PopUp_KnopAfwijzen_Waarnemer.lp_Tunnel_Verkeersbuis2_PopUp_KnopAfwijzen_Waarnemer == 1)) || (state.s_Tunnel_Verkeersbuis2_PopUp_Timer.lp_Tunnel_Verkeersbuis2_PopUp_Timer == 2))) return false;
        } catch (CifSimulatorException e) {
            throw new CifSimulatorException("Evaluation of invariant \"Tunnel_Verkeersbuis2_PopUp_Venster_c_uit needs Tunnel_Verkeersbuis2_PopUp_KnopBevestigen_Waarnemer.ingedrukt or Tunnel_Verkeersbuis2_PopUp_KnopAfwijzen_Waarnemer.ingedrukt or Tunnel_Verkeersbuis2_PopUp_Timer.afgelopen\" of the top level scope of the specification failed.", e, state);
        }
