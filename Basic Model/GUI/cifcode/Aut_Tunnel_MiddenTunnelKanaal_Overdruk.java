...
/** Automaton "Tunnel_MiddenTunnelKanaal_Overdruk". */
public final class Aut_Tunnel_MiddenTunnelKanaal_Overdruk extends RuntimeSimpleAutomaton<State> {
    public Aut_Tunnel_MiddenTunnelKanaal_Overdruk() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_Tunnel_MiddenTunnelKanaal_Overdruk", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "Tunnel_MiddenTunnelKanaal_Overdruk";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_Tunnel_MiddenTunnelKanaal_Overdruk.lp_Tunnel_MiddenTunnelKanaal_Overdruk);
    }
    ...
    public static boolean fillSyncData_evt_Tunnel_MiddenTunnelKanaal_Overdruk_c_links(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(346).get(3);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(228);
        fillSyncData(aut, 346, rslt, state.s_Tunnel_MiddenTunnelKanaal_Overdruk.lp_Tunnel_MiddenTunnelKanaal_Overdruk);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_MiddenTunnelKanaal_Overdruk_c_rechts(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(347).get(3);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(228);
        fillSyncData(aut, 347, rslt, state.s_Tunnel_MiddenTunnelKanaal_Overdruk.lp_Tunnel_MiddenTunnelKanaal_Overdruk);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_MiddenTunnelKanaal_Overdruk_c_uit(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(348).get(3);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(228);
        fillSyncData(aut, 348, rslt, state.s_Tunnel_MiddenTunnelKanaal_Overdruk.lp_Tunnel_MiddenTunnelKanaal_Overdruk);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_Tunnel_MiddenTunnelKanaal_Overdruk = target.s_Tunnel_MiddenTunnelKanaal_Overdruk.copy();
        target.s_Tunnel_MiddenTunnelKanaal_Overdruk.lp_Tunnel_MiddenTunnelKanaal_Overdruk = value;
    }
}
