...
/** Automaton "Tunnel_MiddenPompkelder_KnopLeeghouden_Knop". */
public final class Aut_Tunnel_MiddenPompkelder_KnopLeeghouden_Knop extends RuntimeSimpleAutomaton<State> {
    public Aut_Tunnel_MiddenPompkelder_KnopLeeghouden_Knop() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_Tunnel_MiddenPompkelder_KnopLeeghouden_Knop", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "Tunnel_MiddenPompkelder_KnopLeeghouden_Knop";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_Tunnel_MiddenPompkelder_KnopLeeghouden_Knop.lp_Tunnel_MiddenPompkelder_KnopLeeghouden_Knop);
    }
    ...
    public static boolean fillSyncData_evt_Tunnel_MiddenPompkelder_KnopLeeghouden_Knop_u_indrukken(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(259).get(0);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(178);
        fillSyncData(aut, 259, rslt, state.s_Tunnel_MiddenPompkelder_KnopLeeghouden_Knop.lp_Tunnel_MiddenPompkelder_KnopLeeghouden_Knop);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_Tunnel_MiddenPompkelder_KnopLeeghouden_Knop = target.s_Tunnel_MiddenPompkelder_KnopLeeghouden_Knop.copy();
        target.s_Tunnel_MiddenPompkelder_KnopLeeghouden_Knop.lp_Tunnel_MiddenPompkelder_KnopLeeghouden_Knop = value;
    }
}
