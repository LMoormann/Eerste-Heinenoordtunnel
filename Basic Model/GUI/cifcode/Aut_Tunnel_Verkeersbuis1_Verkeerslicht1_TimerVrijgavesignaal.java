...
/** Automaton "Tunnel_Verkeersbuis1_Verkeerslicht1_TimerVrijgavesignaal". */
public final class Aut_Tunnel_Verkeersbuis1_Verkeerslicht1_TimerVrijgavesignaal extends RuntimeSimpleAutomaton<State> {
    public Aut_Tunnel_Verkeersbuis1_Verkeerslicht1_TimerVrijgavesignaal() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_Tunnel_Verkeersbuis1_Verkeerslicht1_TimerVrijgavesignaal", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "Tunnel_Verkeersbuis1_Verkeerslicht1_TimerVrijgavesignaal";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_Tunnel_Verkeersbuis1_Verkeerslicht1_TimerVrijgavesignaal.lp_Tunnel_Verkeersbuis1_Verkeerslicht1_TimerVrijgavesignaal);
    }
    ...
    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis1_Verkeerslicht1_TimerVrijgavesignaal_c_aan(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(639).get(4);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(379);
        fillSyncData(aut, 639, rslt, state.s_Tunnel_Verkeersbuis1_Verkeerslicht1_TimerVrijgavesignaal.lp_Tunnel_Verkeersbuis1_Verkeerslicht1_TimerVrijgavesignaal);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis1_Verkeerslicht1_TimerVrijgavesignaal_c_reset(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(640).get(4);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(379);
        fillSyncData(aut, 640, rslt, state.s_Tunnel_Verkeersbuis1_Verkeerslicht1_TimerVrijgavesignaal.lp_Tunnel_Verkeersbuis1_Verkeerslicht1_TimerVrijgavesignaal);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis1_Verkeerslicht1_TimerVrijgavesignaal_u_timeout(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(641).get(1);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(379);
        fillSyncData(aut, 641, rslt, state.s_Tunnel_Verkeersbuis1_Verkeerslicht1_TimerVrijgavesignaal.lp_Tunnel_Verkeersbuis1_Verkeerslicht1_TimerVrijgavesignaal);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_Tunnel_Verkeersbuis1_Verkeerslicht1_TimerVrijgavesignaal = target.s_Tunnel_Verkeersbuis1_Verkeerslicht1_TimerVrijgavesignaal.copy();
        target.s_Tunnel_Verkeersbuis1_Verkeerslicht1_TimerVrijgavesignaal.lp_Tunnel_Verkeersbuis1_Verkeerslicht1_TimerVrijgavesignaal = value;
    }
}
