...
/** Automaton "Tunnel_Verkeersbuis1_Luidspreker". */
public final class Aut_Tunnel_Verkeersbuis1_Luidspreker extends RuntimeSimpleAutomaton<State> {
    public Aut_Tunnel_Verkeersbuis1_Luidspreker() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_Tunnel_Verkeersbuis1_Luidspreker", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "Tunnel_Verkeersbuis1_Luidspreker";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_Tunnel_Verkeersbuis1_Luidspreker.lp_Tunnel_Verkeersbuis1_Luidspreker);
    }
    ...
    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis1_Luidspreker_c_boodschap(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(575).get(3);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(358);
        fillSyncData(aut, 575, rslt, state.s_Tunnel_Verkeersbuis1_Luidspreker.lp_Tunnel_Verkeersbuis1_Luidspreker);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis1_Luidspreker_c_live(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(576).get(3);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(358);
        fillSyncData(aut, 576, rslt, state.s_Tunnel_Verkeersbuis1_Luidspreker.lp_Tunnel_Verkeersbuis1_Luidspreker);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis1_Luidspreker_c_onderdrukt(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(577).get(3);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(358);
        fillSyncData(aut, 577, rslt, state.s_Tunnel_Verkeersbuis1_Luidspreker.lp_Tunnel_Verkeersbuis1_Luidspreker);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis1_Luidspreker_c_uit(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(578).get(3);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(358);
        fillSyncData(aut, 578, rslt, state.s_Tunnel_Verkeersbuis1_Luidspreker.lp_Tunnel_Verkeersbuis1_Luidspreker);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_Tunnel_Verkeersbuis1_Luidspreker = target.s_Tunnel_Verkeersbuis1_Luidspreker.copy();
        target.s_Tunnel_Verkeersbuis1_Luidspreker.lp_Tunnel_Verkeersbuis1_Luidspreker = value;
    }
}
