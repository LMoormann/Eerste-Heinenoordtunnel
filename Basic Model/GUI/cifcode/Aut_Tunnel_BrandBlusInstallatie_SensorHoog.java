...
/** Automaton "Tunnel_BrandBlusInstallatie_SensorHoog". */
public final class Aut_Tunnel_BrandBlusInstallatie_SensorHoog extends RuntimeSimpleAutomaton<State> {
    public Aut_Tunnel_BrandBlusInstallatie_SensorHoog() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_Tunnel_BrandBlusInstallatie_SensorHoog", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "Tunnel_BrandBlusInstallatie_SensorHoog";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_Tunnel_BrandBlusInstallatie_SensorHoog.lp_Tunnel_BrandBlusInstallatie_SensorHoog);
    }
    ...
    public static boolean fillSyncData_evt_Tunnel_BrandBlusInstallatie_SensorHoog_u_aan(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(176).get(2);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(128);
        fillSyncData(aut, 176, rslt, state.s_Tunnel_BrandBlusInstallatie_SensorHoog.lp_Tunnel_BrandBlusInstallatie_SensorHoog);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_BrandBlusInstallatie_SensorHoog_u_uit(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(177).get(1);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(128);
        fillSyncData(aut, 177, rslt, state.s_Tunnel_BrandBlusInstallatie_SensorHoog.lp_Tunnel_BrandBlusInstallatie_SensorHoog);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_Tunnel_BrandBlusInstallatie_SensorHoog = target.s_Tunnel_BrandBlusInstallatie_SensorHoog.copy();
        target.s_Tunnel_BrandBlusInstallatie_SensorHoog.lp_Tunnel_BrandBlusInstallatie_SensorHoog = value;
    }
}
