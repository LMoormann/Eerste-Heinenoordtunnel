...
/** Automaton "Tunnel_Verkeersbuis2_J32_TimerVrijgavesignaal". */
public final class Aut_Tunnel_Verkeersbuis2_J32_TimerVrijgavesignaal extends RuntimeSimpleAutomaton<State> {
    public Aut_Tunnel_Verkeersbuis2_J32_TimerVrijgavesignaal() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_Tunnel_Verkeersbuis2_J32_TimerVrijgavesignaal", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "Tunnel_Verkeersbuis2_J32_TimerVrijgavesignaal";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_Tunnel_Verkeersbuis2_J32_TimerVrijgavesignaal.lp_Tunnel_Verkeersbuis2_J32_TimerVrijgavesignaal);
    }
    ...
    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis2_J32_TimerVrijgavesignaal_c_aan(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(783).get(2);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(453);
        fillSyncData(aut, 783, rslt, state.s_Tunnel_Verkeersbuis2_J32_TimerVrijgavesignaal.lp_Tunnel_Verkeersbuis2_J32_TimerVrijgavesignaal);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis2_J32_TimerVrijgavesignaal_c_reset(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(784).get(2);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(453);
        fillSyncData(aut, 784, rslt, state.s_Tunnel_Verkeersbuis2_J32_TimerVrijgavesignaal.lp_Tunnel_Verkeersbuis2_J32_TimerVrijgavesignaal);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis2_J32_TimerVrijgavesignaal_u_timeout(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(785).get(1);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(453);
        fillSyncData(aut, 785, rslt, state.s_Tunnel_Verkeersbuis2_J32_TimerVrijgavesignaal.lp_Tunnel_Verkeersbuis2_J32_TimerVrijgavesignaal);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_Tunnel_Verkeersbuis2_J32_TimerVrijgavesignaal = target.s_Tunnel_Verkeersbuis2_J32_TimerVrijgavesignaal.copy();
        target.s_Tunnel_Verkeersbuis2_J32_TimerVrijgavesignaal.lp_Tunnel_Verkeersbuis2_J32_TimerVrijgavesignaal = value;
    }
}
