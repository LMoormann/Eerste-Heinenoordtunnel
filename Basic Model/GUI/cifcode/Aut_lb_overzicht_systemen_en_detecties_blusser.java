...
/** Automaton "lb_overzicht_systemen_en_detecties_blusser". */
public final class Aut_lb_overzicht_systemen_en_detecties_blusser extends RuntimeSimpleAutomaton<State> {
    public Aut_lb_overzicht_systemen_en_detecties_blusser() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_lb_overzicht_systemen_en_detecties_blusser", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "lb_overzicht_systemen_en_detecties_blusser";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_lb_overzicht_systemen_en_detecties_blusser.lp_lb_overzicht_systemen_en_detecties_blusser);
    }
    ...
    public static boolean fillSyncData_evt_lb_overzicht_systemen_en_detecties_blusser_open_drop_down_menu(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(19).get(0);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(7);
        fillSyncData(aut, 19, rslt, state.s_lb_overzicht_systemen_en_detecties_blusser.lp_lb_overzicht_systemen_en_detecties_blusser);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_lb_overzicht_systemen_en_detecties_blusser_sluit_drop_down_menu(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(20).get(0);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(7);
        fillSyncData(aut, 20, rslt, state.s_lb_overzicht_systemen_en_detecties_blusser.lp_lb_overzicht_systemen_en_detecties_blusser);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_sluit_alle_tabs(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(157).get(3);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(7);
        fillSyncData(aut, 157, rslt, state.s_lb_overzicht_systemen_en_detecties_blusser.lp_lb_overzicht_systemen_en_detecties_blusser);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_lb_overzicht_systemen_en_detecties_blusser = target.s_lb_overzicht_systemen_en_detecties_blusser.copy();
        target.s_lb_overzicht_systemen_en_detecties_blusser.lp_lb_overzicht_systemen_en_detecties_blusser = value;
    }
}
