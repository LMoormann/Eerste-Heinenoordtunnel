...
/** Automaton "Tunnel_Verkeersbuis2_SOS". */
public final class Aut_Tunnel_Verkeersbuis2_SOS extends RuntimeSimpleAutomaton<State> {
    public Aut_Tunnel_Verkeersbuis2_SOS() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_Tunnel_Verkeersbuis2_SOS", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "Tunnel_Verkeersbuis2_SOS";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_Tunnel_Verkeersbuis2_SOS.lp_Tunnel_Verkeersbuis2_SOS);
    }
    ...
    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis2_SOS_u_aan(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(965).get(0);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(553);
        fillSyncData(aut, 965, rslt, state.s_Tunnel_Verkeersbuis2_SOS.lp_Tunnel_Verkeersbuis2_SOS);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis2_SOS_u_uit(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(966).get(1);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(553);
        fillSyncData(aut, 966, rslt, state.s_Tunnel_Verkeersbuis2_SOS.lp_Tunnel_Verkeersbuis2_SOS);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_Tunnel_Verkeersbuis2_SOS = target.s_Tunnel_Verkeersbuis2_SOS.copy();
        target.s_Tunnel_Verkeersbuis2_SOS.lp_Tunnel_Verkeersbuis2_SOS = value;
    }
}
