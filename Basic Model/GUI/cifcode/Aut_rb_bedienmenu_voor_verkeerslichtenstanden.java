...
/** Automaton "rb_bedienmenu_voor_verkeerslichtenstanden". */
public final class Aut_rb_bedienmenu_voor_verkeerslichtenstanden extends RuntimeSimpleAutomaton<State> {
    public Aut_rb_bedienmenu_voor_verkeerslichtenstanden() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_rb_bedienmenu_voor_verkeerslichtenstanden", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "rb_bedienmenu_voor_verkeerslichtenstanden";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_rb_bedienmenu_voor_verkeerslichtenstanden.lp_rb_bedienmenu_voor_verkeerslichtenstanden);
    }
    ...
    public static boolean fillSyncData_evt_rb_bedienmenu_voor_verkeerslichtenstanden_open_drop_down_menu(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(91).get(0);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(43);
        fillSyncData(aut, 91, rslt, state.s_rb_bedienmenu_voor_verkeerslichtenstanden.lp_rb_bedienmenu_voor_verkeerslichtenstanden);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_rb_bedienmenu_voor_verkeerslichtenstanden_sluit_drop_down_menu(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(92).get(0);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(43);
        fillSyncData(aut, 92, rslt, state.s_rb_bedienmenu_voor_verkeerslichtenstanden.lp_rb_bedienmenu_voor_verkeerslichtenstanden);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_sluit_alle_tabs(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(157).get(39);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(43);
        fillSyncData(aut, 157, rslt, state.s_rb_bedienmenu_voor_verkeerslichtenstanden.lp_rb_bedienmenu_voor_verkeerslichtenstanden);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_rb_bedienmenu_voor_verkeerslichtenstanden = target.s_rb_bedienmenu_voor_verkeerslichtenstanden.copy();
        target.s_rb_bedienmenu_voor_verkeerslichtenstanden.lp_rb_bedienmenu_voor_verkeerslichtenstanden = value;
    }
}
