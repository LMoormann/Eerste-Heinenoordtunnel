...
/** Event "Tunnel_MiddenPompkelder_Pomp1_c_aan". */
public final class Event_Tunnel_MiddenPompkelder_Pomp1_c_aan extends RuntimeEvent<State> {
    public Event_Tunnel_MiddenPompkelder_Pomp1_c_aan() {
        super("Tunnel_MiddenPompkelder_Pomp1_c_aan", 265, true);
    }
    ...
    public boolean fillData(State state) {
        ...
        proceed = Aut_sup86.fillSyncData_evt_Tunnel_MiddenPompkelder_Pomp1_c_aan(state);
        if (!proceed) return false;

        // Check automaton "Tunnel_MiddenPompkelder_Pomp1".
        proceed = Aut_Tunnel_MiddenPompkelder_Pomp1.fillSyncData_evt_Tunnel_MiddenPompkelder_Pomp1_c_aan(state);
        if (!proceed) return false;
    ...
    public boolean allowedByInvs(State state) {
        // Invariants for "".
        try {
            if (!(((state.s_Tunnel_MiddenPompkelder_Regime.lp_Tunnel_MiddenPompkelder_Regime == 1) && (state.s_Tunnel_MiddenPompkelder_SensorHooghoogwater.lp_Tunnel_MiddenPompkelder_SensorHooghoogwater == 1)) || ((state.s_Tunnel_MiddenPompkelder_Regime.lp_Tunnel_MiddenPompkelder_Regime == 0) && (state.s_Tunnel_MiddenPompkelder_SensorStartwater.lp_Tunnel_MiddenPompkelder_SensorStartwater == 1)))) return false;
        } catch (CifSimulatorException e) {
            throw new CifSimulatorException("Evaluation of invariant \"Tunnel_MiddenPompkelder_Pomp1_c_aan needs Tunnel_MiddenPompkelder_Regime.bergen and Tunnel_MiddenPompkelder_SensorHooghoogwater.aan or Tunnel_MiddenPompkelder_Regime.leeghouden and Tunnel_MiddenPompkelder_SensorStartwater.aan\" of the top level scope of the specification failed.", e, state);
        }
