...
/** Automaton "Tunnel_MiddenPompkelder_SensorStartwater". */
public final class Aut_Tunnel_MiddenPompkelder_SensorStartwater extends RuntimeSimpleAutomaton<State> {
    public Aut_Tunnel_MiddenPompkelder_SensorStartwater() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_Tunnel_MiddenPompkelder_SensorStartwater", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "Tunnel_MiddenPompkelder_SensorStartwater";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_Tunnel_MiddenPompkelder_SensorStartwater.lp_Tunnel_MiddenPompkelder_SensorStartwater);
    }
    ...
    public static boolean fillSyncData_evt_Tunnel_MiddenPompkelder_SensorStartwater_u_aan(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(282).get(2);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(190);
        fillSyncData(aut, 282, rslt, state.s_Tunnel_MiddenPompkelder_SensorStartwater.lp_Tunnel_MiddenPompkelder_SensorStartwater);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_MiddenPompkelder_SensorStartwater_u_uit(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(283).get(2);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(190);
        fillSyncData(aut, 283, rslt, state.s_Tunnel_MiddenPompkelder_SensorStartwater.lp_Tunnel_MiddenPompkelder_SensorStartwater);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_Tunnel_MiddenPompkelder_SensorStartwater = target.s_Tunnel_MiddenPompkelder_SensorStartwater.copy();
        target.s_Tunnel_MiddenPompkelder_SensorStartwater.lp_Tunnel_MiddenPompkelder_SensorStartwater = value;
    }
}
