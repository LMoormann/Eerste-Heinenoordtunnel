...
/** Automaton "rb_overzicht_systemen_en_detecties_overdruk_veilige_ruimte". */
public final class Aut_rb_overzicht_systemen_en_detecties_overdruk_veilige_ruimte extends RuntimeSimpleAutomaton<State> {
    public Aut_rb_overzicht_systemen_en_detecties_overdruk_veilige_ruimte() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_rb_overzicht_systemen_en_detecties_overdruk_veilige_ruimte", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "rb_overzicht_systemen_en_detecties_overdruk_veilige_ruimte";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_rb_overzicht_systemen_en_detecties_overdruk_veilige_ruimte.lp_rb_overzicht_systemen_en_detecties_overdruk_veilige_ruimte);
    }
    ...
    public static boolean fillSyncData_evt_rb_overzicht_systemen_en_detecties_overdruk_veilige_ruimte_open_drop_down_menu(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(119).get(0);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(57);
        fillSyncData(aut, 119, rslt, state.s_rb_overzicht_systemen_en_detecties_overdruk_veilige_ruimte.lp_rb_overzicht_systemen_en_detecties_overdruk_veilige_ruimte);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_rb_overzicht_systemen_en_detecties_overdruk_veilige_ruimte_sluit_drop_down_menu(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(120).get(0);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(57);
        fillSyncData(aut, 120, rslt, state.s_rb_overzicht_systemen_en_detecties_overdruk_veilige_ruimte.lp_rb_overzicht_systemen_en_detecties_overdruk_veilige_ruimte);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_sluit_alle_tabs(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(157).get(53);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(57);
        fillSyncData(aut, 157, rslt, state.s_rb_overzicht_systemen_en_detecties_overdruk_veilige_ruimte.lp_rb_overzicht_systemen_en_detecties_overdruk_veilige_ruimte);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_rb_overzicht_systemen_en_detecties_overdruk_veilige_ruimte = target.s_rb_overzicht_systemen_en_detecties_overdruk_veilige_ruimte.copy();
        target.s_rb_overzicht_systemen_en_detecties_overdruk_veilige_ruimte.lp_rb_overzicht_systemen_en_detecties_overdruk_veilige_ruimte = value;
    }
}
