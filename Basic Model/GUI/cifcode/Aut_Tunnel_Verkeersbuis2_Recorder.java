...
/** Automaton "Tunnel_Verkeersbuis2_Recorder". */
public final class Aut_Tunnel_Verkeersbuis2_Recorder extends RuntimeSimpleAutomaton<State> {
    public Aut_Tunnel_Verkeersbuis2_Recorder() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_Tunnel_Verkeersbuis2_Recorder", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "Tunnel_Verkeersbuis2_Recorder";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_Tunnel_Verkeersbuis2_Recorder.lp_Tunnel_Verkeersbuis2_Recorder);
    }
    ...
    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis2_Recorder_c_aan(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(954).get(1);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(551);
        fillSyncData(aut, 954, rslt, state.s_Tunnel_Verkeersbuis2_Recorder.lp_Tunnel_Verkeersbuis2_Recorder);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis2_Recorder_c_uit(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(955).get(1);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(551);
        fillSyncData(aut, 955, rslt, state.s_Tunnel_Verkeersbuis2_Recorder.lp_Tunnel_Verkeersbuis2_Recorder);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_Tunnel_Verkeersbuis2_Recorder = target.s_Tunnel_Verkeersbuis2_Recorder.copy();
        target.s_Tunnel_Verkeersbuis2_Recorder.lp_Tunnel_Verkeersbuis2_Recorder = value;
    }
}
