...
/** Automaton "Tunnel_Verkeersbuis2_Onderhoud". */
public final class Aut_Tunnel_Verkeersbuis2_Onderhoud extends RuntimeSimpleAutomaton<State> {
    public Aut_Tunnel_Verkeersbuis2_Onderhoud() {
        super(getMonitorData(), Spec.MONITOR_EDGE, "cifcode.Aut_Tunnel_Verkeersbuis2_Onderhoud", SPEC.resourceClassLoader);
    }

    @Override
    public String getName() {
        return "Tunnel_Verkeersbuis2_Onderhoud";
    }

    @Override
    public String getCurLocName(State state) {
        return getLocName(state.s_Tunnel_Verkeersbuis2_Onderhoud.lp_Tunnel_Verkeersbuis2_Onderhoud);
    }
    ...
    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis2_Onderhoud_c_aan(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(941).get(5);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(543);
        fillSyncData(aut, 941, rslt, state.s_Tunnel_Verkeersbuis2_Onderhoud.lp_Tunnel_Verkeersbuis2_Onderhoud);
        return !rslt.isEmpty();
    }

    public static boolean fillSyncData_evt_Tunnel_Verkeersbuis2_Onderhoud_c_uit(State state) {
        List<RuntimeEdge<State>> rslt = SPEC.syncData.get(942).get(5);
        rslt.clear();
        RuntimeSimpleAutomaton<State> aut = (RuntimeSimpleAutomaton<State>)SPEC.automata.get(543);
        fillSyncData(aut, 942, rslt, state.s_Tunnel_Verkeersbuis2_Onderhoud.lp_Tunnel_Verkeersbuis2_Onderhoud);
        return !rslt.isEmpty();
    }
    ...
    protected void updateLocPointerValue(State target, int value) {
        target.s_Tunnel_Verkeersbuis2_Onderhoud = target.s_Tunnel_Verkeersbuis2_Onderhoud.copy();
        target.s_Tunnel_Verkeersbuis2_Onderhoud.lp_Tunnel_Verkeersbuis2_Onderhoud = value;
    }
}
